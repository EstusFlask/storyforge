...
| 8 | **AGENT-1 对话副驾与 Agent 团队** | 用一个主 Agent 对话组合调用现有能力，不让用户选择分 Agent | 单一主对话、五领域闭环、六角色路由、领域上下文与团队预算已完成；27.2b“整理本章”用一次调用形成六域证据候选、可恢复审核和 hash 守卫写回；下一单位进入后台一致性 Agent | Tool Registry 薄封装；前台写入必确认；后台默认只读；Canon 负责校验 | **27.2b ORGANIZE-CHAPTER COMPLETE（2026-07-27）** |
