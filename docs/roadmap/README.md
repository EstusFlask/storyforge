...
| 8 | **AGENT-1 对话副驾与 Agent 团队** | 用一个主 Agent 对话组合调用现有能力，不让用户选择分 Agent | 单一主对话、计划/任务/候选持久化及世界来源/灵感/角色/大纲/正文幕后编排已完成；继续 27.1-e、27.2b、整理本章 Agent、一致性 Agent、per-role 模型和预算 | Tool Registry 薄封装；前台写入必确认；后台默认只读；Canon 负责校验 | **27.1-d COMPLETE；继续团队与后台 Agent（2026-07-26）** |
