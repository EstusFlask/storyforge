...
| 8 | **AGENT-1 对话副驾与 Agent 团队** | 用一个主 Agent 对话组合调用现有能力，不让用户选择分 Agent | 单一主对话、计划/任务/候选持久化及世界来源/灵感/角色/大纲幕后编排已完成；继续正文领域、27.1-e、27.2b、整理本章 Agent、一致性 Agent、per-role 模型和预算 | Tool Registry 薄封装；前台写入必确认；后台默认只读；Canon 负责校验 | **MAIN-AGENT MVP COMPLETE；领域继续扩展（2026-07-26）** |
...
- 当前主 Agent 已提供泛化自然语言入口、计划与后台领域调度，可执行领域为世界来源、
  灵感反推、新增角色和新增大纲；正文、并行自治团队与长期后台 Agent 尚未交付，不把四个
  领域夸成全功能通用代理。
