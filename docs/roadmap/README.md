...
| 8 | **AGENT-1 对话副驾与 Agent 团队** | 用一个主 Agent 对话组合调用现有能力，不让用户选择分 Agent | 单一主对话、计划/任务/候选持久化及世界来源/灵感/角色幕后编排已完成；继续大纲/正文领域、27.1-e、27.2b、整理本章 Agent、一致性 Agent、per-role 模型和预算 | Tool Registry 薄封装；前台写入必确认；后台默认只读；Canon 负责校验 | **MAIN-AGENT MVP COMPLETE；领域继续扩展（2026-07-26）** |
| 9 | **FLOW-2 自由节点模式** | 让作者自由选择具体设定、前文和中间产物，组成可见可保存的创作数据流 | 独立 `nodeFlows/nodeRuns`、六类节点、动态端口、来源/预算筛选、逐节点输入输出、局部运行和确认写回已完成；继续 RAG-1 记录/字段选择器、模板、条件与脏下游 | 节点模式独立于分步骤 UI/数据模型；底层复用三注册表、共享 AI、Canon 与 adoption；图和运行可恢复、可导出 | **CORE MVP COMPLETE；RAG-1 ENHANCEMENT NEXT（2026-07-26）** |
...
10. ⏳ RAG-1 可见资料/检索管理 → AGENT-1 剩余领域 → SIM-1 共同运行时 → TTRPG-1 / CHATGAME-1 →
    PRODUCT-1 / PLATFORM-1：互动产品必须先共享 SIM 状态层，不为跑团和角色聊天各建一套引擎。
...
- **读与记录**：项目元素只经 `CONTEXT_SOURCES → assembleContext()`；`nodeRuns` 逐节点
  保存真实上游文本、预算、来源纳入/省略/裁剪、输出、错误与 gate。
- **写与生命周期**：作品仅在内容输出节点由作者明确确认后经 `adopt()` 写回；
  `nodeFlows/nodeRuns` 进入项目备份、导入、项目/世界删除和运行级联。
- **当前非范围**：RAG-1 记录/字段浏览器、条件、循环、并行、脚本/插件、图模板和 Agent
  自动生成图。它们不得通过节点直接查询 IndexedDB 或暗中写 Canon。
- **自动化证据**：`R-FLOW2-free-node-mode` 覆盖图校验、局部拓扑、实际输入输出持久化、
  刷新编辑、确认写回和删除级联；完整证据见 `VISUAL-WORKFLOW-DESIGN.md`。
...
- Phase 27.1-d 第二个领域扩展已交付角色生成：只经 `read_worldview/read_characters`
  ...
  状态卡或大纲。
- 角色闭环专项随全部 AGENT-1 测试合跑 7 文件/42 项；完整 CI 224 文件/792 项、Chromium
  20/20 通过。真实 Agnes 在隔离项目返回完整角色卡，作者编辑标记逐字写入既有角色面板，
  临时项目随后完整清理。
- 当前主 Agent 已提供泛化自然语言入口、计划与后台领域调度，但可执行领域仍只有世界来源、
  灵感反推和新增角色；大纲/正文、并行自治团队与长期后台 Agent 尚未交付，不把三个领域
  夸成全功能通用代理。
