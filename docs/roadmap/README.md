...
| 8 | **AGENT-1 对话副驾与 Agent 团队** | 用一个主 Agent 对话组合调用现有能力，不让用户选择分 Agent | 单一主对话、五领域闭环、六角色模型/API 路由、领域上下文档位与真实输入证据、跨调用团队预算和一次确定性 Canon 打回已完成；下一开发单位进入 27.2b 整理本章 Agent | Tool Registry 薄封装；前台写入必确认；后台默认只读；Canon 负责校验 | **27.1-e CURRENT SCOPE COMPLETE（2026-07-27）** |
