...
| 2 | **CANON-1 长期一致性与 Canon** | 把长篇一致性从提示词劝告升级为可度量、可追溯的软硬工程 | `CONSISTENCY-0`、`CONSISTENCY-2`、`CONSISTENCY-3`、长期一致性路线第 2–4 步、Phase 38 写后事实检测、Phase 39 故事线进度、`EDITOR-2`、`CF-20260703-8` | 覆盖地图、知识账本、世界宪法、时序/故事线、影响传播和编辑器出口闭环；明确硬/软/未覆盖 | **IN PROGRESS：CONSISTENCY-0/2/3 COMPLETE；时序/故事线 NEXT** |
...
3. CANON-1：✅ 覆盖基线 → ✅ 认知账本 → ✅ 世界宪法；下一步按覆盖地图进入存亡时序/故事线动态进度。
...
- `CONSISTENCY-COVERAGE-MAP.md` 的 6 个反例进入 `tests/canon/`：物品重复获得、认知边界、检索期世界隔离与设定互斥 2 类为活动测试；存亡时序 1 类为显式 `todo`。
...
- 当前诚实基线为 **5 类可执行、1 类未覆盖**；LLM 只负责候选抽取且仍可能漏，硬保证只覆盖已登记、已抽取、来源有效且已确认的主题。
