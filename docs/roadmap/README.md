...
| 2 | **CANON-1 长期一致性与 Canon** | 把长篇一致性从提示词劝告升级为可度量、可追溯的软硬工程 | `CONSISTENCY-0`、`CONSISTENCY-2`、`CONSISTENCY-3`、长期一致性路线第 2–4 步、Phase 38 写后事实检测、Phase 39 故事线进度、`EDITOR-2`、`CF-20260703-8` | 覆盖地图、知识账本、世界宪法、时序/故事线、影响传播和编辑器出口闭环；明确硬/软/未覆盖 | **IN PROGRESS：CONSISTENCY-0 COMPLETE；CONSISTENCY-2 NEXT** |
...
3. CANON-1：✅ 覆盖基线 → 认知账本 → 世界宪法，每一块连接反例和用户出口；当前进入认知账本。
...
- `CONSISTENCY-COVERAGE-MAP.md` 的 6 个反例进入 `tests/canon/`：物品重复获得与检索期世界隔离为活动测试，设定互斥、认知越界、存亡时序 4 类为显式 `todo`，不再以文档口头状态冒充覆盖。
- `check:canon-coverage` 进入 CI，要求 🟢 行有活动反例、🔴 行保留 `todo`，同时锁定活动测试必须调用真实判决/检索机制。
- 当前诚实基线为 **2 类可执行、4 类未覆盖**；硬检测与处置强度仍分开，现有 Canon finding 不自动修改正文。
