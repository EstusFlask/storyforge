...
15. ⏳ TTRPG-1 → CHATGAME-1 → PRODUCT-1 / PLATFORM-1：互动产品继续复用 SIM 状态层，
    不为跑团和角色聊天各建一套引擎。
...
- `SIM-1C` 已作为一个完整功能单位收口，当前主线转为 `TTRPG-1`；外部 UI/产品候选不得改变
  `TTRPG-1 → CHATGAME-1 → PRODUCT-1 / PLATFORM-1` 顺序。
