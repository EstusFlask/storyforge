...
现在打开 StoryForge 会先进入产品综合首页。世界引擎、分步骤写作、节点创作、跑团、角色聊天和文字游戏是并列的产品页签；世界引擎是其余功能可选择引用的运行基座，而不是这些功能的上级页面。

主要功能：

- 总览：继续最近的世界和分步骤作品，并进入六个并列功能页签。
- 世界引擎：从零创建可复用世界，或直接打开既有分步骤作品中的世界基础、角色、故事、大纲与状态实例；两种入口读取同一份本地数据，不需要同步，也不会拆分或删除原内容。
- 世界编号：每个项目持久化一个 `W-…@v1` 世界身份，可在本地工作区搜索和切换；世界引擎页支持生成和导入本地世界分享包，线上社区编号引用仍未开放。
- 本地世界发布：作者可声明署名、许可、内容警告和分步骤写作/跑团/角色聊天/文字游戏用途。分享包只携带注册表允许的世界设定、角色和规则表，带 SHA-256 完整性校验；导入会先预览，确认后创建新本地副本，不覆盖原项目。
- 分步骤写作：继续使用既有卷纲、章纲和正文工作流，旧项目无需迁移。
- 节点创作：直接打开独立 DAG 节点工作区。
- 跑团：只展示跑团存档，复用共享互动运行时，但不会混入沙盒、NPC 演进或角色聊天存档。
- 角色聊天：已提供单角色聊天 MVP。会话冻结世界与角色来源，支持流式回复、重新生成、检查点和分支；不会自动改写项目 Canon。
- 文字游戏：当前仍为 Labs 只读入口，不会创建游戏或改写项目数据。

原项目列表、文件夹恢复、导入导出和项目删除仍完整保留在 `/projects`，现有用户可以继续按原流程进入完整工作区。
...
当前边界：共享运行时已经支持作者选择世界、角色、地点、物品和规则并冻结为带来源 hash
的 Canon 快照；`TTRPG-1A/1B/1C` 已覆盖单机战役主持、战斗资源与遭遇、长期战役摘要、
任务、统一世界时钟、NPC 日程和分支续接。角色聊天已接入单角色聊天 MVP；文字游戏仍是 Labs 入口；多人联机
以及聊天长期记忆属于后续功能单位。
