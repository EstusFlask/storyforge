...
- 正规剧本（实验实现）：分支内部已有整本、卷或章节范围改编代码，但旧入口读取可变 Work，已在总纲对齐后从默认界面隔离；新产品专属 SourceSelection 完成前不能新建正式剧本。
- 页漫（实验实现）：分支内部已有从短篇或长篇派生漫画的代码，但旧创建入口同样已隔离；新产品专属 SourceSelection 完成前不能新建正式漫画。

当前实验代码中的剧本和漫画共用一条改编准备流程：来源快照 → 改编 Brief → 结构计划 → 作者确认 → 正式生产。该共用产品协议将按对齐审计拆除，不能视为目标架构。以下工作台说明仅记录已有实验草稿的内部能力，不代表默认界面仍允许新建或产品已经正式发布。
