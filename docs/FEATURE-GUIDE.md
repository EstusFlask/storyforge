...
首页“作品创作”现在统一管理四种生产路径，但不会复制四套世界、角色和正文系统：
...
- 正规剧本：在世界引擎的作品管理中选择小说，点击“把当前小说改成剧本或漫画”。可选整本、卷或章节范围，确认来源预览后创建独立剧本 Work，源小说不会被覆盖。
- 页漫：使用相同改编入口建立独立漫画 Work，可从短篇或长篇小说派生。

剧本和漫画共用一条受治理的改编准备流程：来源快照 → 改编 Brief → 结构计划 → 作者确认 → 正式生产。漫画在结构计划后还必须确认全局视觉圣经。AI 结果始终先成为可编辑候选，作者确认后才原子写入正式场景或页格；若源小说后来变化，系统会标记来源过期并阻止继续新增，已有成品仍可检查和导出，作者处理影响并确认同步后才能继续生产。
