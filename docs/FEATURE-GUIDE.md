...
StoryForge 是一个本地优先的人工智能叙事创作与世界运行系统。世界引擎负责可复用的世界事实、叙事结构和运行实例；长篇小说创作在其上提供分步骤模式与节点模式，主助手为分步骤模式提供对话式辅助。跑团、角色聊天和文字游戏使用同一世界基座，但拥有相互隔离的运行状态。统一 Agent + Harness 让生成的输入、候选、问题、用量和确认过程可见、可恢复、可停止。

本文按真实界面结构编写。第二至第十章主要记录当前最完整的长篇小说分步骤模式，每个二级页签都有对应截图；世界引擎、节点模式、跑团、角色聊天和文字游戏的最新完成边界以第一章、[当前能力基线](./roadmap/CAPABILITY-BASELINE.md) 和应用实际界面为准。后续会为这些新入口补充同等详细的独立截图章节。

## 目录

- [一、首页](#一首页)
- [1.1 短篇、长篇与改编作品](#11-短篇长篇与改编作品)
- [二、著作信息](#二著作信息)
- [三、设定库](#三设定库)
- [四、创作区](#四创作区)
- [五、体验中心](#五体验中心)
...
现在打开 StoryForge 会先进入产品综合首页。为了让用户直接进入所需功能，世界引擎、分步骤写作、节点创作、跑团、角色聊天和文字游戏在导航中保持并列；数据与产品依赖仍有明确层级：世界引擎位于第一层，长篇小说位于第二层，分步骤模式与节点模式共同服务长篇创作，主助手嵌入分步骤模式提供对话式协助。跑团、角色聊天和文字游戏从世界快照建立独立实例。

主要功能：

- 总览：继续最近的世界和长篇作品，并进入各功能页签。
...
- 分步骤写作：长篇小说的主工作流，继续使用既有卷纲、章纲和正文能力，旧项目无需迁移。
- 节点创作：长篇小说的自由编排模式，直接打开独立有向无环图工作区，读取和写入同一份作品数据。
- 主助手：位于分步骤工作区右侧，以对话方式辅助世界、灵感、角色、大纲和正文任务；候选确认后才进入作品。
...
> 对齐状态（2026-08-22）：短篇仍是现有小说创作引擎的正式 workflow profile；下述剧本和
> 漫画能力是 `feat/short-screenplay-comic` 分支的实验实现，尚未满足从不可变
> `WorldRelease + WorldReleaseManifestV2` 冻结产品专属 SourceSelection 的总纲要求，不应合并、
> 发布或作为正式完成能力宣传。具体阻断、数据需求和后续施工门见
> [分支与世界引擎总纲对齐审计](./SHORT-SCREENPLAY-COMIC-WORLD-SOURCE-ALIGNMENT-AUDIT-20260822.md)。

本功能分支的首页“作品创作”目前展示四种生产路径，但不会复制四套世界、角色和正文系统：

- 短篇小说：新建时选择“短篇小说”，目标字数限定为 5,000～25,000 字；沿用原分步骤创作的数据和编辑器，只裁剪为单卷动态结构、章纲与正文主路径。短篇可显式扩写为长篇；已有长篇切换为短篇前会检查当前字数和目标字数，超出边界时拒绝切换，不会截断正文。
- 长篇小说：保留原分步骤模式和节点模式，旧项目默认按长篇兼容，无需迁移。
- 正规剧本（实验实现）：在世界引擎的作品管理中选择小说，点击“把当前小说改成剧本或漫画”。可选整本、卷或章节范围，确认来源预览后创建独立剧本 Work，源小说不会被覆盖；当前来源仍是可变 Work，不能作为正式生产入口。
- 页漫（实验实现）：使用相同改编入口建立独立漫画 Work，可从短篇或长篇小说派生；当前同样受冻结来源缺口阻断。

当前实验代码中的剧本和漫画共用一条改编准备流程：来源快照 → 改编 Brief → 结构计划 → 作者确认 → 正式生产。该共用产品协议将按对齐审计拆除，不能视为目标架构。漫画在结构计划后还必须确认全局视觉圣经。AI 结果始终先成为可编辑候选，作者确认后才原子写入正式场景或页格；若源小说后来变化，系统会标记来源过期并阻止继续新增，已有草稿仍可检查和导出，作者处理影响并确认同步后才能继续实验生产。

正规剧本工作台支持：

- 场景树排序、复制、拆分、合并和来源证据检查。
- `INT./EXT.`、地点、时间、动作、角色 cue、括注、对白、转场、镜头和作者注释等结构化块。
- 场景 CAS 保存、逐场审定与锁定；全部场景审定且来源一致后才能正式完稿。
- 从同一场景结构导出 Fountain、FDX 和打印版 PDF，避免多种导出格式内容漂移。

漫画工作台支持：

- 1～9 格页面模板、页/格排序、拆格、合格、跨页移动和可视化页面预览。
- 角色、地点、道具和风格视觉条目；角色只可绑定本作品已登场角色，地点只可绑定世界地理库条目。
- 每格镜头、可见动作、来源证据、连续性引用及备注、视觉 Prompt、负面 Prompt、成图裁切和旋转。
- 本地横排/竖排对白、独白、旁白与音效排字；文字不会交给图片模型生成。
- 上传图片或调用兼容图片服务生成候选，显式登记权利声明、能力限制和参考图关系；候选不会自动选中。
- 确定性 QA 检查页格布局、媒体完整性、分辨率、引用环、连续性和排字风险。正式导出要求每格已选片，可导出分镜脚本、结构 JSON、PNG/WebP ZIP、CBZ 和打印版 PDF。

剧本或漫画标记正式完稿后进入只读状态，并同步把对应 Work 标为完成。若需修改，必须点击“重新打开审校”；这样不会出现状态显示已完成、内容却被静默改写的情况。
...
这一页是传统分步骤工作区内的多世界结构管理入口，只在多世界模式开启后显示。产品综合首页中的“世界引擎”不要求开启多世界；两者读取同一份本地资料，但承担不同层级的界面职责。
...
- 新建项目时选择项目文件夹，或在“设置 → 项目存储工作区”查看、更换任意本地硬盘位置；项目内容、
  记忆与恢复证据默认共用该项目根目录。选择位置后不会自动写入，仍需手动检查、确认采纳或同步。
- 在硬盘直接维护章节 Markdown、Work 级故事核心和创作规则中文 YAML；外部修改仍需先查看字段候选与影响。
- 查看项目/硬盘三方差异、字段候选和下游影响计划，明确解决冲突。
- 从恢复胶囊重建浏览器数据，或在不支持文件夹 API 的浏览器导入/导出工作区包。
- 恢复时保留完整 Harness 历史；不能跨本地主键继续信任的 completed 凭据会明确标为待复核，而非静默冒充有效。
- 配置 GitHub Gist 云备份。
- 管理手动快照。
...
本地记忆工作区的完整操作、恢复、隐私、token 与停用说明见 [《本地记忆工作区：使用、恢复与隐私》](MEMORY-WORKSPACE-GUIDE.md)。
...
核心创作功能已经进入统一 Agent + Harness。对普通作者来说，不需要理解 Run Contract 或注册表；需要知道的是：

- 每次任务会冻结目标、相关资料、模型和执行版本，中断或刷新后可从同一运行记录恢复。
- 输出先成为候选，不会因为模型生成了内容就直接改写正式设定或正文。
- 候选可以是“可直接确认”“带警告可用”“需手工修整”或“安全阻断”；普通质量问题不会把整份草稿直接丢掉。
- 默认一次生成；只有系统准确定位到可修结构问题时，均衡/精修模式才允许最多一次定向修复。
- 授权、欠费、模型权限、过期输入、网络结果未知或相同失败不会自动重复调用。
- 候选会记录实际 token、延迟、修复原因和停止原因；模型价格未知时不会猜测金额。
- 资料不足时，系统可以提出可见的临时假设来推动人物目标、阻力、选择和变化，但不会把假设静默写进 Canon。

当前创作可靠性入口属于实验性社区观察能力。本轮 Agent + Harness / CREL 已阶段性开发完成，工程恢复、费用停止和可编辑交付已经收口；独立作者 A/B 与原预注册质量门由产品决策关闭，不再是待开发项，但历史结果没有因此变成通过。它不能被理解为“任何模型都能稳定生成完美长篇”，社区反馈将用于判断后续需要重开的具体问题。
...
flowchart LR
  ...
  s1["① 任务合同<br/>作者目标 + Skill + 读写权限<br/>预算 / Prompt / Tool 版本"]
  s2["② 上下文装配<br/><b>assembleContext()</b><br/>CONTEXT_SOURCES 按需读取<br/>World / Work 隔离 + 预算裁剪"]
  s3["③ 有界生成<br/><b>软 / AI</b><br/>默认一次；确定性可修时<br/>最多一次定向修复"]
  s4["④ 产物验证<br/>结构 / 作用域 / 引用 / stale<br/>保留草稿、片段、问题与用量"]
  s5["⑤ 作者决定<br/>编辑 / 选择 / 接受警告 / 拒绝<br/>确认前 Canon 零写入"]
  s6["⑥ 采纳落库<br/><b>adopt()</b><br/>FIELD_REGISTRY + ADOPTION_SCHEMAS<br/>类型 / FK / 去重 / CAS"]
  s7["⑦ 终态与记忆<br/>回读正式结果 + receipt<br/>章后抽取 / 摘要 / 检索"]
  s8["⑧ 修改传播<br/>正文 hash 失效 + 影响 DAG<br/>人工、确定性与生成式目标分流"]
  ledger["durable ledger / checkpoint<br/>刷新恢复，不重复已结算调用"]
...
正文生成不是“把 prompt 发给模型”这么简单。它先冻结任务、权限和预算，再由 `CONTEXT_SOURCES` 装配上下文；AI 输出进入分级候选和确定性验证，只有作者确认后才采纳。运行证据、恢复、反写记忆、终态回读和影响传播都沿同一 durable 链路完成。
...
1. 先在短篇或长篇小说中整理好要改编的正文范围。
2. 到世界引擎的作品管理，选择源小说并点击“把当前小说改成剧本或漫画”。
3. 选择整本、卷或章节，检查来源预览，填写目标规格后创建派生作品。
4. 依次确认 Brief 和结构计划；漫画再确认视觉圣经。
5. 在剧本场景树或漫画页格工作台生产、检查并逐项审定。
6. 通过正式 QA 后导出，再标记正式完稿。源小说变化时先查看过期提示，不要在未处理影响的情况下重新生成。
...
- 清理浏览器数据可能删除项目。
- 建议定期导出 JSON 完整备份。
- 当前完整备份格式为 v9；剧本场景、漫画页格、视觉条目、媒体元数据和图片二进制都随项目生命周期导出、导入、删除和引用重映射。导入会复核图片哈希、尺寸、字节数和引用环，失败时整次导入不落库。
- 删除漫画图片使用“先解除引用、再标记待删除、复查无引用后清除”的两阶段回收，避免页面或设定图仍引用时误删二进制。
- 重要项目建议在新建时选择项目文件夹，或到设置中补充项目存储位置，并在每轮外部编辑后手动检查和同步；
  它不会在后台自动覆盖文件。
