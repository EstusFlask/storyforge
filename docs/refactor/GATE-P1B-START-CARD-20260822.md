...
状态：机械门已通过；等待用户授权把 4178 origin 的已配置凭据复制到 4197 origin 后执行真实模型 sealed run
...
| grader max tokens | `1200` |
| 单次 grader timeout | `180000 ms` |
| checkpoint key | `storyforge-races-gateway-eval-v1` |
...
4178 与 4197 是不同浏览器 origin，凭据不会自动共享。虽然用户已授权项目内无限调用 API，但把敏感凭据从一个 origin 复制到另一个 origin 仍需一次明确的动作时确认。确认前只允许准备门禁，不允许读取或复制密钥。
