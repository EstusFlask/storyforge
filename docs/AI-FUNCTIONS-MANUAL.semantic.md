...
- **上下文与范围**:模型只经 `chapterContent` 登记源读取当前 Work 的全部已写正文；完整正式年表由治理层回读作 CAS，不进入模型 Prompt。章节顺序、Manifest、分块和提示词在 Run 中冻结。
- **长任务边界**:每个确定完成分块建立 checkpoint，刷新只续跑剩余调用；模型请求结果不可判定时禁止自动重试。任一分块协议失败不产生部分候选。
- **写回与恢复**:候选按 AdoptionSchema 的 `chapterId + title` 身份去重；作者确认后逐章事务执行“清旧 + 整批写新”，章内 order 压缩为 `0..n-1`。写入后、checkpoint 前中断可回读同值状态继续；空候选或空选择表示明确清理目标已写章。
- **终验**:正文、提示词、目标章正式结果或范围外正式年表变化都会阻断采纳或撤销旧 receipt；其它 Work 数据始终隔离。
