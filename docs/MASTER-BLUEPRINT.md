...
| TypeScript 生产源码 | 437 个文件 / 83698 行 | `tsconfig.json` |
| IndexedDB schema | v41 / 46 张 required tables | `schema.ts` / `REQUIRED_TABLES` |
| PROJECT_TABLES | 46 张表 | `project-tables.ts` |
| Prompt 主线 | 59 个 moduleKey / 204 条内置模板 | `PromptModuleKey` / `prompt-seeds*.ts` |
| CONTEXT_SOURCES | 37 个上下文源 | `context-sources.ts` |
| 写回治理 | 20 个通用 adopt target / 8 个领域扩展 | `adoption-schema.ts` |
<!-- project-metrics:end -->
