...
| TypeScript 生产源码 | 892 个文件 / 322210 行 | `tsconfig.json` |
| IndexedDB schema | v71 / 107 张 required tables | `schema.ts` / `REQUIRED_TABLES` |
| PROJECT_TABLES | 107 张表 | `project-tables.ts` |
| Prompt 主线 | 64 个 moduleKey / 209 条内置模板 | `PromptModuleKey` / `prompt-seeds*.ts` |
| CONTEXT_SOURCES | 79 个上下文源 | `context-sources.ts` |
| 写回治理 | 34 个通用 adopt target / 36 个领域扩展 | `adoption-schema.ts` |
<!-- project-metrics:end -->
