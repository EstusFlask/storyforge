...
| TypeScript 生产源码 | 825 个文件 / 258049 行 | `tsconfig.json` |
| IndexedDB schema | v69 / 91 张 required tables | `schema.ts` / `REQUIRED_TABLES` |
| PROJECT_TABLES | 91 张表 | `project-tables.ts` |
| Prompt 主线 | 65 个 moduleKey / 210 条内置模板 | `PromptModuleKey` / `prompt-seeds*.ts` |
| CONTEXT_SOURCES | 76 个上下文源 | `context-sources.ts` |
| 写回治理 | 37 个通用 adopt target / 29 个领域扩展 | `adoption-schema.ts` |
<!-- project-metrics:end -->
