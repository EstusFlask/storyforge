...
节点模式与分步骤模式的关系，类似 ComfyUI 与一个封装好的 Stable Diffusion 工作流：分步骤模式提供经过设计的标准操作路径，节点模式把同一生产过程公开为可查看、可填写、可连接、可复用的节点图。
...
世界引擎必须提供稳定、版本化、可查询的数据出口，使上层产品能够：

- 读取内容目录和完整度画像；
- 按作用域查询事实、实体、关系、时间线、主支线、章节和原文证据；
- 先读取摘要与索引，再按需渐进式披露详情和原文；
- 获得来源、版本、置信与冲突信息；
- 在预算不足时明确知道哪些来源未被读取，而不是把截断误当成内容不存在。

数据出口必须由注册表和契约驱动，不能由每个上层产品手写一份世界字段清单。

## 6. 世界引擎之上的衍生产品

### 6.1 基本关系

跑团、角色聊天、AI 小镇和各类文字游戏通过“世界编号 + 封存版本”引用世界引擎。用户随后补充该产品特有的目标与配置，并与该产品的主 Agent 沟通。确认开始后，主 Agent 负责完成设计与生产。

引用是从世界引擎到衍生产品的单向读取。衍生产品拥有自己的内容、状态、媒资、运行记录、记忆、版本和用户保存空间。

衍生产品中的故事发展、玩家选择、角色状态和私有设定不得自动反馈、合并或污染共享世界引擎。即使未来同一个世界被无数用户引用，每个用户的衍生内容也只属于其自己的产品实例。若未来需要“把某次衍生内容改编为新世界版本”，必须是一个显式、独立、由世界作者确认的创作流程，而不是运行时回写机制。

### 6.2 每个上层产品都是完整垂直产品

每个上层产品都必须具备自己的完整架构，而不是“一个生成 Agent + 一个运行 Agent”或世界引擎上的薄页面。其最小生命周期是：

```text
引用并锁定世界版本
→ 收集产品目标、参与者、规模、风格、限制和体验目标
→ 主 Agent 形成可审查的产品计划
→ 内容、规则、媒资等生产分支并行工作
→ 自动组装、绑定、验证、修复
→ 形成不可变的可运行产品版本
...
每个阶段都必须有明确的数据表或契约、状态机、Agent/Skill 或确定性服务、错误恢复、可观测运行证据和验收测试。

### 6.3 媒资生产属于上层产品

媒资生产是上层产品生产阶段的正式组成部分。主 Agent 应根据已经确定的产品内容自动拆解并调度角色、场景、UI、地图、音乐、环境音、音效、配音或其他资产的需求，完成生成、选择、修复、绑定和版本管理。

用户通常只需要表达目标和查看结果，不应被迫逐张生成、下载再手工绑定。用户可以检查、替换或重做资产，但这些是产品生产能力，不是世界引擎的资产管理功能。

### 6.4 体验规模与结束契约

上层产品必须允许用户选择是否设定体验目标：

- **有界体验**：可设置目标时长、章节或场景规模、分支宽度、结局数量、收束方式等；系统应在预算内形成可结束、可验证的完整体验。
- **持续体验**：用户可以选择不预设终点，由系统按有限、可保存、可恢复的回合或章节持续演化；用户可随时要求暂停、总结、生成结局或继续。

“无限演绎”不得实现为一次永不结束的 Agent 调用。每次运行仍必须具有有限目标、持久化检查点、预算边界和失败恢复。

### 6.5 跑团

跑团产品需要独立解决参与人数、角色卡、技能与规则、队伍与目标、初始情景、真人或 AI 主持、模型提供商、掷骰/判定、信息隔离、分队与并行场景、玩家关系、主持记忆、节奏与戏剧性、运行记录和安全恢复。

这些不是文档中的“边界说明”，而是必须逐项形成 UI、表结构、规则服务、Agent 能力、权限与可见性、状态机和测试的产品子系统。

### 6.6 角色聊天与 AI 小镇

单角色聊天、多角色聊天和 AI 小镇共享角色语义与对话基础，但不是同一规模的一个页面状态。

- 角色聊天重点处理人格、关系、记忆、场景、多人轮次、可见信息和长期会话连续性。
- AI 小镇还需要世界时间、居民日程、地点、群体关系、自治行为、事件传播、离线演化和多主体调度。

两者应具有明确的产品边界和可复用层，不能用一个不断膨胀的聊天组件同时承担全部责任。
...
- 以选择、资源和状态推进为核心的文字冒险；
- 以场景、演出、对白、分支和视听呈现为核心的 AVG；
- 以持续世界状态、探索、任务、角色自治和开放演化为核心的文字开放世界。

它们可以共享世界读取、叙事运行、媒资与保存设施，但必须分别定义玩法循环、内容生产、运行状态、结束条件、界面和质量验证。分类最终以真实不同的数据与运行需求为准，不以营销名称堆叠页面。
...
- 把当前最主要的分步骤长篇流程做到端到端真实可用。
- 完成 Harness、三注册表、渐进式上下文、版本/候选/采纳、长程一致性和真实数据生命周期。
- 使节点模式完整映射并复用长篇能力，不形成第二套实现。
