...
这是 StoryForge 从“浏览器里保存项目、按需检索前文”升级为“作者、Harness、IndexedDB 与本地硬盘共同维护长期创作状态”的一次完整工程更新。记忆不再只是模型可能召回的一段文本，而是可读、可核对、可恢复，并在安全边界内可由作者编辑的项目映像。更新已合入 `main`，应用语义版本仍保持 `3.9.1`；正式版本号与 Release tag 将在下一次独立发版流程中决定。

#### 作者可见的变化

- 新建项目时可以直接选择项目存储文件夹，也可以在“设置 → 项目存储工作区”随时查看、更换或重新授权任意本地硬盘位置；所选目录就是该项目的本地根目录。
- 浏览器 IndexedDB 与硬盘工作区形成双向映像，但任何检查和同步都必须由作者主动触发。绑定文件夹、页面进入、定时器和模型完成都不会静默覆盖另一侧。
- 项目内修改后，作者可检查差异和下游影响，再确认写入硬盘；外部编辑受支持文件后，系统会形成字段候选、执行作用域与当前值校验，再由作者确认写回项目。
- 冲突不会默认选边。系统区分项目改动、本地改动、双方冲突、缺失、额外与损坏，并在覆盖或删除前保留 history/trash 恢复证据。
- Workspace、World、Work、章节 Markdown 可读可编辑；故事核心和创作规则新增 Work 级中文 YAML。账本、回执及高风险结构保持只读恢复，不为追求“全表可编辑”破坏来源和历史。
- 浏览器数据丢失后，可从工作区校验 identity、manifest、逐文件 hash 和恢复胶囊，重建正式内容、引用与 Harness 历史；跨浏览器失效的旧终态凭据会保留并明确标记待复核，不伪装为仍然有效。

#### Harness 与长期一致性

- Harness 的生成候选、作者修改、采纳/拒绝、验证和成功/失败/取消终态统一沉淀为可追溯记忆；终态和 `memory.settlement.recorded` 在同一 IndexedDB 事务内提交，避免“流程完成但没有记忆回执”的半状态。
- Fast Guard / Deep Audit 已成为正式 durable Run：由作者显式触发，每次最多一次模型调用；报告只形成证据候选，不自动改正文或 Canon，正文变化后旧报告保留并标记 stale。
- 生成和审校优先读取已确认 Canon、角色认知、状态、物品、时间线、依赖和来源证据。embedding 只允许作为可删除、可重建、无事实裁决权的可选索引。
- 外部修改故事核心或创作规则后，影响计划会提示可能受影响的生成、审校与下游内容；系统不会因此自动批量改稿或连续调用模型。

#### 隐私、Token 与诚实边界

- 文件扫描、解析、hash、差异、冲突、影响计划、同步和恢复全部在本机完成，模型调用为 0，不消耗用户 AI token。只有作者另行触发 AI 生成或语义深审时才会计费。
- 本次完成的是“可恢复、可解释、由作者裁决的长期创作连续性”，不承诺 AI 永不遗忘、作品永无矛盾，也不包含云同步、多人协作、Git 自动提交或服务端托管。
- 本轮选择性开放低歧义语义内容后即停止扩张；新增可编辑投影仍须逐表通过稳定身份、作用域、采纳、影响、恢复和反例测试。
...
这是 StoryForge 从“各功能拼提示词后直接调用模型”转向“受治理、可恢复、可审计的统一创作运行时”的一次主架构升级。更新已合入 `main`，但应用语义版本仍保持 `3.9.1`；正式版本号与 Release tag 将在下一次独立发版流程中决定。

#### 对作者可见的变化

- AI 输出统一先成为候选：可预览、编辑、拒绝或接受警告，只有作者确认后才进入 Canon 或正文。
- 创作任务默认一次生成；确定性定位到可修问题时最多允许一次定向修复。相同失败不再盲目重试，授权、余额、权限、网络结果未知和过期输入不会自动继续烧费。
- 即使候选未完全达标，也会尽可能保留合法片段、原始草稿、问题位置、临时假设与调用证据，避免“花了钱但什么都拿不到”。
- 只有部分世界观、角色或灵感时，运行时可形成 `NarrativeBrief` 和显式临时假设来推动目标、阻力、选择与状态变化；假设不会静默写成正式设定。
- 每个候选可记录实际模型、用途、input/output/total token、延迟、修复原因和停止原因；没有登记价格的模型不会显示伪造金额。

#### 架构与数据治理

- 新增统一 Master Agent、领域 Skill Registry、闭集只读 Tool Registry、Run Contract、执行版本绑定和工作流目录；计划只能调用登记的 Skill、Context Source 与写目标。
- 新增 durable Harness：`agentRuns / agentRunEvents / agentRunCheckpoints / agentEvents` 保存事件账本、checkpoint、父子 lineage、候选 hash、终态回执与恢复证据，刷新或中断后不会重复已结算步骤。
- AI 读取继续唯一经过 `CONTEXT_SOURCES + assembleContext()`；正式写入继续唯一经过 `FIELD_REGISTRY + AdoptionSchema + adopt()`；所有运行表、导入取消、作用域与引用重映射继续由 `PROJECT_TABLES` 统一派生。
- 新增 World / Work owner gate、信息边界、上下文预算与压缩、严格候选 parser、确定性验证、stale/CAS 检查、有限 fan-out、有限重规划和反向影响处理 DAG。
- 故事核心、故事线、世界观字段、角色创建/补充/关系、灵感反推、角色驱动、卷章纲、细纲、正文、章后抽取、地点/物品/年表/伏笔/历史/参考分析/文风学习等核心入口已迁移或由统一入口登记审计；旧直连旁路已按风险收口。
- 创作可靠性 `CREL-0～14` 在现有 Harness 上增加分级产物、问题合同、1+1 调用上限、免费确定性归一化、一次定向修复、部分设定叙事脚手架、统一预览和本地结构化反馈。

#### 验证证据

- 合并前完整 `npm run ci` 通过：387 个测试文件、1,892 项测试，覆盖 required tables、AI Manual、AI 入口登记、架构、来源可达性、路线图、Agent freshness、Canon、依赖、lint、TypeScript、覆盖率、生产构建与 bundle budget。
- 独立浏览器数据中的 Chromium E2E 52/52 通过；没有使用或修改作者当前预览项目。
- CREL v6 第三轮 Development：可编辑/可采纳 100%，平均 1 次生成，语义 85.8%，推进 100%，机器门 PASS。
- 一次性 sealed held-out：可编辑/可采纳 100%，平均 1 次生成，语义 83.3%，推进 100%；token/可采纳倍率 1.514404，略高于预注册 1.5 门槛，因此机器门按协议 FAIL。
