...
test('真实 Chromium 解码透明 PNG 与 PCM WAV，并产出可审计媒体指标', async ({ page, browserName }) => {
  ...
  const measurement = await page.evaluate(async currentBrowserName => {
    ...
    const [{ verifyGameMediaRuntimeUrlsV1 }, { ensureGeneratedCharacterAlphaV1 }] = await Promise.all([
      importer('/storyforge/src/lib/game-production/media-runtime-verifier.ts'),
      importer('/storyforge/src/lib/game-production/character-alpha-matting.ts'),
    ])
    ...
    try {
      const runtimeMeasurement = await verifyGameMediaRuntimeUrlsV1({
        assets: [
          {
