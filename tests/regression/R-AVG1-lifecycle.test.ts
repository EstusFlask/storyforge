...
describe('AVG-1 · binary and registry lifecycle', () => {
  ...
  it('Blob 随严格项目备份往返并重映射，删草稿保留不可变发布与存档', async () => {
    ...
    const exported = await exportProjectJSON(projectId); expect(exported.avgMediaBlobs?.[0].data).toBeNull(); expect(exported.avgMediaBlobs?.[0]._blobObjectExportId).toEqual(expect.any(Number)); expect(exported.mediaBlobObjects?.[0].data).toMatch(/^data:/); expect(exported.avgPresentationModules).toHaveLength(1)
    const importedId = await importProjectJSON(exported); const importedAsset = await db.avgMediaAssets.where('projectId').equals(importedId).first(); const importedBlob = await db.avgMediaBlobs.where('projectId').equals(importedId).first(); const importedObject = await db.mediaBlobObjects.where('projectId').equals(importedId).first(); expect(importedBlob?.mediaAssetId).toBe(importedAsset?.id); expect(importedBlob?.blobObjectId).toBe(importedObject?.id); expect(importedBlob?.data).toBeNull(); expect(importedObject?.data).toBeInstanceOf(ArrayBuffer); expect(importedObject?.data?.byteLength).toBeGreaterThan(0)
    expect(await db.simulationSessions.where('projectId').equals(importedId).filter(row => row.kind === 'avg').count()).toBe(1)
    const corrupt = structuredClone(exported); const portable = corrupt.mediaBlobObjects![0].data; const [prefix, body] = portable.split(','); corrupt.mediaBlobObjects![0].data = `${prefix},${body[0] === 'A' ? 'B' : 'A'}${body.slice(1)}`; const projectCount = await db.projects.count(); await expect(importProjectJSON(corrupt)).rejects.toThrow('二进制哈希'); expect(await db.projects.count()).toBe(projectCount)
