...
describe('AVG-1 · author and player UI', () => {
  ...
  it('玩家以真实媒资推进 Beat，关闭图片和减少动态后仍可走到三个选择', async () => {
    ...
    await click(host, '退出游戏'); await waitFor(() => expect(useAvgGamePlayerStore.getState().selectedSessionId).toBeNull()); expect(host.textContent).toContain('作品详情'); expect(await db.simulationSessions.where('projectId').equals(owned.scope.projectId).count()).toBe(1); await click(host, '继续上次进度'); await waitFor(() => expect(host.textContent).toContain('第一章 · 失潮之夜'))
