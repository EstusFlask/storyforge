...
describe('R-15: character reference remap', () => {
  ...
  it('remaps merged character ids to the primary character and merges state cards by name', async () => {
    ...
    await db.transaction('rw', db.characters, db.characterRelations, db.detailedOutlines, db.stateCards, db.temporalFacts, db.itemLedger, db.knowledgeLedger, db.cultivationProgress, async () => {
