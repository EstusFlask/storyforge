...
function request(overrides: Partial<GameMediaRequestV1> = {}): GameMediaRequestV1 {
  return {
    schema: 'storyforge.game-media-request', version: 1, requestId: 'request.1',
...
describe('R-GAMEPROD-1E · central media provider adapters', () => {
  it('从 PNG/JPEG/WebP provider bytes 读取真实尺寸，不信任请求元数据', () => {
    ...
    expect(detectGameImageDimensionsV1(png)).toEqual({ width: 1024, height: 576 })
    expect(detectGameImageDimensionsV1(jpeg)).toEqual({ width: 1024, height: 576 })
    expect(detectGameImageDimensionsV1(webp.buffer)).toEqual({ width: 1024, height: 576 })
    expect(detectGameImageDimensionsV1(PNG)).toBeNull()
  })
  ...
  it('本地 procedural SFX 明确不能冒充商业音乐，未实现目录适配器也显式失败', async () => {
    ...
    expect(listGameMediaProviderCapabilitiesV1().map(item => item.adapterId)).toEqual([
    ...
    expect(() => resolveGameMediaProviderAdapterV1('existing-project-media.v1')).toThrow(/catalog-service/)
  })
})
