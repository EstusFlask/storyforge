...
import { verifyGameMediaRuntimeUrlsV1 } from '../../src/lib/game-production/media-runtime-verifier'
...
describe('R-GAMEPROD-1I · real browser media runtime verifier', () => {
  it('按稳定 assetKey 解码图片与音频，并冻结透明度、声道、采样率和本地音频分析', async () => {
    ...
    const result = await verifyGameMediaRuntimeUrlsV1({
      assets: [
