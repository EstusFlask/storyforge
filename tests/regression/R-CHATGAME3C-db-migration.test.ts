...
function name(): string {
  const value = `chatgame-v70-${Math.random()}`
  names.push(value)
  return value
}
...
class ChatGameV70DB extends Dexie {
  constructor(databaseName: string) {
    super(databaseName)
    this.version(69).stores({ projects: '++id, name' })
...
class ChatGameV71DB extends Dexie {
  constructor(databaseName: string) {
    super(databaseName)
    this.version(70).stores({
      projects: '++id, name',
...
describe('CHATGAME-3C · v69 -> v70 product source/Brief migration', () => {
  it('只新增三张空产品表并保留旧项目，唯一 revision 索引拒绝覆盖历史', async () => {
    ...
    const upgraded = track(new ChatGameV70DB(databaseName))
    await upgraded.open()
  ...
  it('v70 -> v71 保留冻结来源与 Brief，只新增空步骤/产物/媒资/发行表和 nullable 指针', async () => {
    const databaseName = name()
    const old = track(new ChatGameV70DB(databaseName))
    await old.open()
    ...
    const upgraded = track(new ChatGameV71DB(databaseName))
    await upgraded.open()
