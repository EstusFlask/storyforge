...
describe('R-HARNESS59 / WEH-0H · 正式 AI 入口机器绑定', () => {
  it('AST 守卫证明实际 member/alias/wrapper 调用均携带登记 entryId', () => {
    ...
    expect(output).toContain('32 bindings / 35 calls')
    expect(output).toContain('formal 15, auxiliary 13, evaluation 3, experimental 1')
  })

  it('唯一注册表严格解析且每项连接 Skill、执行器、候选和调用方', () => {
    expect(registry.version).toBe(2)
    expect(FORMAL_AI_ENTRY_REGISTRY_V1.entries).toHaveLength(32)
