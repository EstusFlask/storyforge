...
} from '../../src/lib/game-production/capabilities'
import {
  evaluateGameProductionAuthorizationReadinessV1,
  type GameProductionCapabilityReadinessV1,
} from '../../src/lib/game-production/service'
...
describe('R-GAMEPROD-1D · reuse existing text provider configuration', () => {
  const readiness = (overrides: Partial<GameProductionCapabilityReadinessV1> = {}): GameProductionCapabilityReadinessV1 => ({
    text: {
  ...
  it('Agnes 图片复用同一全局配置；只有真正缺失的冻结 capability 才在 Build 前阻断', () => {
    ...
    const agnesImageReady = evaluateGameProductionAuthorizationReadinessV1({
    ...
    const blocked = evaluateGameProductionAuthorizationReadinessV1({
    ...
    const allowed = evaluateGameProductionAuthorizationReadinessV1({
      brief: { capabilityRequirements: [requiredImage] },
      readiness: readiness({
  ...
  it('从现有配置生成去敏 binding receipt，不产生第二套 Key 配置', async () => {
    const resolved = await resolveConfiguredTextCapabilityV1({
      projectId: 1, category: 'game-production.content', requirementKey: 'text.runtime-package',
  ...
  it('正式调用沿用同一 resolved config，并把 key 只交给既有 chat 边界', async () => {
    ...
    const result = await runConfiguredProductionTextV1({
      projectId: 7, category: 'game-production.content', requirementKey: 'text.runtime-package',
    ...
    expect(runAI).toHaveBeenCalledWith(
      ...
      configured,
      { category: 'game-production.content', projectId: 7, maxTokens: 2_000 },
      undefined,
      undefined,
    )
