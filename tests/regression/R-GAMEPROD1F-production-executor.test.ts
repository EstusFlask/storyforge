...
import { prepareGameProductionAdoption } from '../../src/lib/game-production/adoption'
...
import type { ResolvedGameMediaCapabilityV1 } from '../../src/lib/game-production/media-transport'
import {
  createBuiltInProductionCapabilityBindingV1,
  createConfiguredGameProductionExecutorV1,
  isolateCharacterProviderPromptV1,
  parseGameMediaRequirementsArtifactV2,
  parseProductionModelJsonObjectV1,
  type ProductionTextRunnerV1,
} from '../../src/lib/game-production/production-executor'
import { runGameProductionUntilBlockedV1 } from '../../src/lib/game-production/scheduler'
...
import { beginGameProductionEvolutionV1, publishGameProductionV1, startGameProductionPreviewV1 } from '../../src/lib/game-production/service'
import {
...
async function fixture(qualityProfile: 'prototype' | 'commercial-candidate' = 'prototype') {
  ...
  const created = await executeGameProductionCommand({
    ...
    command: {
      ...
      worldReleaseId: release.id!, userText: '正式自动制作 AVG',
    },
  })
...
async function fixtureForProduct(productType: ProductionProductTypeV1) {
  ...
  const created = await executeGameProductionCommand({
    scope: owned.scope,
    command: {
      type: 'create-intent', commandId: `six.${productType}.intent`, productionKey: `six.${productType}`,
      worldReleaseId: release.id!, userText: `正式自动制作 ${productType}`,
    },
  })
  const saved = await executeGameProductionCommand({
    scope: owned.scope, productionId: created.productionId,
...
async function completeAvgBuildPreviewMainRoute(input: {
  ...
  for (let turn = 0; turn < 30; turn += 1) {
    let state = await readSimulationState(opened.sessionId)
    ...
    for (const beat of beats.slice(reachedIndex + 1)) {
      const base = await readSimulationStateVersion(opened.sessionId)
    ...
    state = await readSimulationState(opened.sessionId)
...
function modelOutputs(
  worldHash: string,
  productType: ProductionProductTypeV1 = 'avg',
  characterAnchorRef = 'character:0',
  playerRole = '扮演冻结 Brief 主角',
) {
  return {
    'content.design': {
      schema: 'storyforge.game-design-artifact', version: 1, title: '雾港抉择',
    ...
    'content.narrative': {
      schema: 'storyforge.game-narrative-artifact', version: 1, moduleKind: 'main', moduleTitle: '雾港抉择',
    ...
    'content.product-module': {
      schema: 'storyforge.game-product-module-artifact', version: 1, productType,
    ...
    'media.requirements': {
      schema: 'storyforge.game-media-requirements-artifact', version: 2,
      visual: [
...
async function relayCapabilities(brief: Awaited<ReturnType<typeof fixture>>['brief'], calls: RedactedMediaTransportRequestV1[]) {
  ...
  const resolved = new Map<string, ResolvedGameMediaCapabilityV1>()
  for (const requirement of brief.capabilityRequirements.filter(item => ['image', 'music', 'sfx'].includes(item.mediaClass))) {
    ...
    const capabilityHash = await hashGameProductionValueV2({ requirementKey: requirement.requirementKey, adapterId })
    const adapter = resolveGameMediaProviderAdapterV1(adapterId)
    resolved.set(requirement.requirementKey, {
      ...
      receipt: {
        ...
        receiptHash: await hashGameProductionValueV2({ requirementKey: requirement.requirementKey, capabilityHash }),
      },
    })
  }
  return resolved
...
describe('R-GAMEPROD-1F · provider JSON response normalization', () => {
...
describe('R-GAMEPROD-1F · configured formal production executor', () => {
  ...
  it('复用非密钥 binding，并行生成内容、视觉、音频，预览后完成三轮连续演化', async () => {
    ...
    const bindingHash = await hashGameProductionValueV2({ provider: 'existing-global-config' })
    ...
    expect(parseGameMediaRequirementsArtifactV2(anchoredScene, owned.brief).visual[0].characterAnchorRefs)
    ...
    expect(parseGameMediaRequirementsArtifactV2(weakenedConstraints, owned.brief).visual[1].hardConstraints)
      .toEqual([
        '不得写回世界正式表',
        '保持角色身份、年龄段与核心视觉特征',
        `角色定位：${owned.brief.intent.playerRole}`,
    ...
    expect(parseGameMediaRequirementsArtifactV2(backgroundSuggestion, owned.brief).visual[0].hardConstraints)
      .toEqual([])
    const forgedSceneAnchor = structuredClone(anchoredScene)
    forgedSceneAnchor.visual[0].characterAnchorRefs = ['character:999999']
    expect(() => parseGameMediaRequirementsArtifactV2(forgedSceneAnchor, owned.brief))
      .toThrow(/角色锚点未绑定 Brief 冻结角色/)
    const calls: string[] = []
    ...
    const executor = createConfiguredGameProductionExecutorV1({ production, brief: owned.brief, runText })
    ...
    expect(JSON.parse(build.compatibilityJson)).toMatchObject({
      schema: 'storyforge.game-build-compatibility', level: 'compatible', migrationPolicy: 'initial-session',
    })
    const packageArtifact = await db.gameBuildArtifacts
    ...
    expect(opened.productType).toBe('avg')
    expect(await db.simulationSessions.get(opened.sessionId)).toMatchObject({
      gameBuildId: build.id, gameReleaseId: null, kind: 'avg', runtimeSourceHash: build.packageHash,
    })
    const playable = await resolvePlayableGameSource({
      scope: owned.scope,
      source: { kind: 'build', gameBuildId: build.id!, expectedPreviewHash: build.previewHash },
    })
    ...
    const published = await publishGameProductionV1({
      scope: owned.scope, productionId: owned.productionId,
    })
    const releasedPlayable = await resolvePlayableGameSource({
      scope: owned.scope,
      source: { kind: 'release', gameReleaseId: published.receipt.gameReleaseId },
    })
    ...
    const evolution = await beginGameProductionEvolutionV1({
      scope: owned.scope, productionId: owned.productionId,
      userText: '承接守住庇护的结局，让原来的配角调查第二座信号塔。',
    })
    const nextBriefRow = await db.gameProductionBriefs
    ...
    const currentProduction = (await db.gameProductions.get(owned.productionId))!
    const authorized = await executeGameProductionCommand({
      scope: owned.scope, productionId: owned.productionId,
      command: {
    ...
    const nextBrief = parseGameProductionBriefV3(nextBriefRow!.briefJson)
    const evolvedProjection = await runGameProductionUntilBlockedV1({
      scope: owned.scope, productionId: owned.productionId,
      executor: createConfiguredGameProductionExecutorV1({
        production: (await db.gameProductions.get(owned.productionId))!, brief: nextBrief, runText,
      }),
      capabilityBindings,
    })
    expect(evolvedProjection.terminal).toBe(true)
    expect(await db.gameBuilds.get(evolvedProjection.buildId)).toMatchObject({
      buildNumber: 2, parentBuildNumber: 1, sourceGameReleaseId: published.receipt.gameReleaseId,
      status: 'release-ready',
    })
    expect(JSON.parse((await db.gameBuilds.get(evolvedProjection.buildId))!.compatibilityJson)).toMatchObject({
      schema: 'storyforge.game-build-compatibility', fromBuildNumber: 1, toBuildNumber: 2,
      level: 'compatible', migrationPolicy: 'identity',
    })
    expect(await db.gameBuilds.get(build.id!)).toMatchObject({
      status: 'released', previewHash: build.previewHash, releasedGameReleaseId: published.receipt.gameReleaseId,
    })
    expect(await db.simulationSessions.get(opened.sessionId)).toMatchObject({ gameBuildId: build.id })
    ...
    for (const [index, goal] of [
      ...
      const nextEvolution = await beginGameProductionEvolutionV1({
        scope: owned.scope, productionId: owned.productionId, userText: goal,
      })
      const row = await db.gameProductionBriefs
        .where('[productionId+revision]').equals([owned.productionId, nextEvolution.briefRevision]).first()
      const productionBeforeStart = (await db.gameProductions.get(owned.productionId))!
      await executeGameProductionCommand({
        scope: owned.scope, productionId: owned.productionId,
        command: {
      ...
      const parsedBrief = parseGameProductionBriefV3(row!.briefJson)
      const nextProjection = await runGameProductionUntilBlockedV1({
        scope: owned.scope, productionId: owned.productionId,
        executor: createConfiguredGameProductionExecutorV1({
          production: (await db.gameProductions.get(owned.productionId))!, brief: parsedBrief, runText,
        }),
        capabilityBindings,
      })
      expect(nextProjection.terminal).toBe(true)
      const nextBuild = (await db.gameBuilds.get(nextProjection.buildId))!
      expect(nextBuild).toMatchObject({
        buildNumber: previousBuildNumber + 1, parentBuildNumber: previousBuildNumber,
        sourceGameReleaseId: published.receipt.gameReleaseId, status: 'release-ready',
      })
      previousBuildNumber = nextBuild.buildNumber
    }
    expect(previousBuildNumber).toBe(4)
    expect(await db.gameBuilds.where('productionId').equals(owned.productionId).count()).toBe(4)
    expect(await db.gameReleases.get(published.receipt.gameReleaseId)).toMatchObject({
      contentHash: published.receipt.releaseContentHash,
    })
    expect(await db.simulationSessions.get(opened.sessionId)).toMatchObject({
      gameBuildId: build.id, runtimeSourceHash: build.packageHash,
    })
  }, 30_000)
  ...
  it('商业候选通过可信中继生成可验证 PNG/MP3，且执行请求与冻结证据不含 API Key', async () => {
    ...
    const textRequirement = owned.brief.capabilityRequirements.find(item => item.mediaClass === 'text')!
    const textBindingHash = await hashGameProductionValueV2({ provider: 'existing-global-config' })
    const outputs = modelOutputs(
      owned.brief.source.worldContentHash,
      'avg',
      owned.brief.source.selection.characterExportIds.length
    ...
    const projection = await runGameProductionUntilBlockedV1({
      scope: owned.scope, productionId: owned.productionId,
      executor: createConfiguredGameProductionExecutorV1({
        production: (await db.gameProductions.get(owned.productionId))!,
    ...
    const media = await db.gameBuildArtifacts.where('buildId').equals(build.id!).toArray()
    ...
    await expect(prepareGameProductionAdoption({
      scope: owned.scope, productionId: owned.productionId,
    })).rejects.toThrow(/商业候选缺少真实浏览器性能回执/)
    await recordGameBrowserPerformanceMeasurementV1({
      scope: owned.scope, gameBuildId: build.id!,
      measurement: {
    ...
    expect(await db.gameBuilds.get(build.id!)).toMatchObject({ status: 'preview-ready' })
    await expect(prepareGameProductionAdoption({
      scope: owned.scope, productionId: owned.productionId,
    ...
    await recordGameBuildMainRoutePlaythroughV1({
      scope: owned.scope, gameBuildId: build.id!, simulationSessionId: sessionId,
    ...
    const runtimeArtifact = await db.gameBuildArtifacts
      .where('[buildId+artifactKey]').equals([build.id!, 'runtime.package']).first()
    const runtime = parseGameRuntimePackageV2(runtimeArtifact!.payloadJson)
    await recordGameMediaRuntimeMeasurementV1({
      scope: owned.scope, gameBuildId: build.id!,
      measurement: {
    ...
    expect(await db.gameBuilds.get(build.id!)).toMatchObject({ status: 'release-ready' })
    await expect(prepareGameProductionAdoption({
      scope: owned.scope, productionId: owned.productionId,
  ...
  it('作者声明只改玩法模块时跨 Build 复用未受影响闭包，并重新执行装配与 QA', async () => {
    ...
    const first = await runGameProductionUntilBlockedV1({
      scope: owned.scope, productionId: owned.productionId,
      executor: createConfiguredGameProductionExecutorV1({
        production: (await db.gameProductions.get(owned.productionId))!, brief: owned.brief, runText,
      }), capabilityBindings,
    })
    ...
    const evolved = await beginGameProductionEvolutionV1({
      scope: owned.scope, productionId: owned.productionId,
    ...
    const nextBrief = parseGameProductionBriefV3(briefRow!.briefJson)
    ...
    const second = await runGameProductionUntilBlockedV1({
      scope: owned.scope, productionId: owned.productionId,
      executor: createConfiguredGameProductionExecutorV1({
        production: (await db.gameProductions.get(owned.productionId))!, brief: nextBrief, runText,
      }), capabilityBindings,
    ...
    const secondBuild = (await db.gameBuilds.get(second.buildId))!
    ...
    const artifacts = await db.gameBuildArtifacts.where('buildId').equals(second.buildId).toArray()
  ...
  it('六类文字游戏与 TTRPG 经过正式生产、可玩 Build Preview 与同包原子发布', async () => {
    ...
    for (const productType of products) {
      ...
      const bindingHash = await hashGameProductionValueV2({ provider: 'existing-global-config', productType })
      ...
      const projectedBuild = await db.gameBuilds.get(projection.buildId)
      expect(
        projection,
      ...
      if (productType === 'ttrpg') {
        ...
        expect(await db.gameBuildArtifacts
      ...
      const preview = await startGameProductionPreviewV1({
        scope: owned.scope, productionId: owned.productionId,
      })
      expect(preview.productType).toBe(productType)
      expect(await db.simulationSessions.get(preview.sessionId)).toMatchObject({
        gameBuildId: build.id, gameReleaseId: null, runtimeSourceHash: build.packageHash,
      })
      if (productType === 'ttrpg') {
        const beforeSessionZero = await readSimulationState(preview.sessionId)
        const beforeVersion = await readSimulationStateVersion(preview.sessionId)
        await db.gameBuilds.update(build.id!, { previewHash: 'f'.repeat(64) })
        ...
        await db.gameBuilds.update(build.id!, { previewHash: build.previewHash })
        ...
        let state = await readSimulationState(preview.sessionId)
        let version = await readSimulationStateVersion(preview.sessionId)
        ...
        state = await readSimulationState(preview.sessionId)
        version = await readSimulationStateVersion(preview.sessionId)
      ...
      const released = await resolvePlayableGameSource({
        scope: owned.scope, source: { kind: 'release', gameReleaseId: published.receipt.gameReleaseId },
      })
