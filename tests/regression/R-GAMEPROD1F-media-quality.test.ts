...
import { analyzeDecodedGameAudioPcmV1 } from '../../src/lib/game-production/audio-quality-analyzer'
import {
  evaluateGameMediaCommercialPolicyV2,
  GAME_COMMERCIAL_MEDIA_POLICY_V2,
} from '../../src/lib/game-production/media-quality-policy'
...
function runtimeWithAsset(asset: NonNullable<GameRuntimePackageV2['presentation']>['assets'][number], loop = false): GameRuntimePackageV2 {
  return {
    presentation: {
      version: 1,
      assets: [asset],
  ...
  } as GameRuntimePackageV2
}

describe('R-GAMEPROD-1F · commercial media quality analyzers', () => {
  it('对浏览器解码后的精确 PCM 计算版本化 LUFS、true peak 与循环接缝', () => {
    const channel = sine({ amplitude: 0.16, seconds: 2 })
    const analysis = analyzeDecodedGameAudioPcmV1({ channels: [channel, channel], sampleRateHz: 48_000 })
    ...
    expect(() => analyzeDecodedGameAudioPcmV1({
  ...
  it('把图片尺寸/透明度/bytes 与音频声道/采样率/响度/峰值/loop 变成商业硬门', () => {
    const image = runtimeWithAsset({
      ...
      mimeType: 'image/png', byteSize: GAME_COMMERCIAL_MEDIA_POLICY_V2.maximumImageBytes + 1,
    ...
    expect(evaluateGameMediaCommercialPolicyV2({
