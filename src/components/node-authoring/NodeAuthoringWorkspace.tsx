...
export default function NodeAuthoringWorkspace(props: { project: Project; worldGroupId: number | null }) {
  ...
  useEffect(() => {
    ...
    if (!flow) return
    try { const loadedGraph = parseAuthoringGraph(flow.graphJson).graph; setDraft(flow); setGraph(loadedGraph); setSavedGraph(structuredClone(loadedGraph)); setSelectedNodeId(null); setSelectedNodeIds([]); setDirty(false); void useNodeFlowStore.getState().loadRuns(projectId, selectedFlowId) } catch (error) { toast.error(`节点图读取失败：${error instanceof Error ? error.message : String(error)}`) }
