...
const CONSTITUTION_EXTRACT_CONTEXT_SOURCE_KEYS = [
  "constitutionScanSources",
] as const;
const CODEX_EXTRACT_CONTEXT_SOURCE_KEYS = [
  "manualText",
  "codexExtractionBaseline",
] as const;
...
const HISTORY_AGENT_CONTEXT_SOURCE_KEYS = [
  "worldview",
  "historyAgentBaseline",
] as const;
const REFERENCE_DERIVED_CONTEXT_SOURCE_KEYS = [
  "referenceDerivedBaseline",
] as const;
...
const CHARACTER_RELATIONSHIP_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      ...
      instruction: "没有足够的角色与剧情资料时返回空数组，不得编造关系。",
    },
    partial: {
      handling: "reference-and-create",
      instruction: "只提取现有资料逐字支持的关系，不补写缺失剧情。",
    },
    complete: {
      handling: "grounded-transform",
      instruction: "综合现有角色、关系、大纲与正文，输出可核对的关系候选。",
    },
  },
};
...
const REFERENCE_DERIVED_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      ...
      instruction: "没有已完成的版本化分块分析时不得生成派生结果。",
    },
    partial: {
      handling: "grounded-transform",
      instruction: "只整理已登记的分析维度或人物塑造文本，不补造来源内容。",
    },
    complete: {
      handling: "grounded-transform",
      instruction: "严格压缩版本化分析证据，保持来源、维度和角色归并可核查。",
    },
  },
};
...
const WORLD_LOCATION_EXTRACTION_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      handling: "require-upstream",
      instruction: "没有已写正文时不得提取重要地点。",
    },
    partial: {
      handling: "grounded-transform",
      instruction: "只从当前已写正文明确出现的地点中提取候选。",
    },
    complete: {
      handling: "grounded-transform",
      instruction: "综合全部已写章节去重，只保留有明确剧情作用的地点。",
    },
  },
};

const WORLD_LOCATION_EXTRACTION_COMPRESSION_POLICY = compressionPolicy([
  "chapterContent",
]);

const WORLD_MAP_CONTEXT_SOURCE_KEYS = [
  "worldview",
  "geography",
  "codex",
  "locations",
...
const WORLD_MAP_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      handling: "create-from-request",
      instruction:
        "当前世界资料为空；只生成最小可用地图，不得声称引用作者设定。",
    },
    partial: {
      handling: "reference-and-create",
      instruction: "锁定已有地理实体、名称与空间证据，只补足地图运行所需参数。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格依据当前世界的世界观、地理、词条和重要地点生成可核查地图配置。",
    },
  },
};
const WORLD_MAP_COMPRESSION_POLICY = compressionPolicy(
  WORLD_MAP_CONTEXT_SOURCE_KEYS,
);
...
const PROSE_INVENTORY_EXTRACTION_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      handling: "require-upstream",
      instruction: "没有所选章节正文时不得提取物品流水。",
    },
    partial: {
      handling: "grounded-transform",
      instruction: "只记录所选正文中明确发生、持有人明确的物品变动。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "逐章提取真实发生的物品变动，保持角色归属、方向和规范名称一致。",
    },
  },
};

const PROSE_INVENTORY_EXTRACTION_COMPRESSION_POLICY = compressionPolicy([
  "chapterContent",
]);
...
const PROSE_STORY_TIMELINE_EXTRACTION_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      handling: "require-upstream",
      instruction: "没有已写正文时不得提取故事年表。",
    },
    partial: {
      handling: "grounded-transform",
      instruction: "只记录正文中明确发生的剧情事件，不补写缺失时间或因果。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "逐章提取正文明确发生的剧情大事，保持故事时间、重要度和章节归属可核对。",
    },
  },
};

const PROSE_STORY_TIMELINE_EXTRACTION_COMPRESSION_POLICY = compressionPolicy([
  "chapterContent",
  "storyTimelineTarget",
]);
...
const PROSE_SELECTION_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      ...
      instruction: "没有作者冻结的选中文字时不得执行局部编辑或查漏。",
    },
    partial: {
      handling: "grounded-transform",
      instruction: "只处理作者冻结的选中文字，不推断或补写选区外正文。",
    },
    complete: {
      handling: "grounded-transform",
      instruction: "只处理作者冻结的选中文字，不推断或补写选区外正文。",
...
const FORESHADOW_SUGGESTION_COMPRESSION_POLICY = compressionPolicy(
  FORESHADOW_SUGGESTION_CONTEXT_SOURCE_KEYS,
);
const HISTORY_AGENT_COMPRESSION_POLICY = compressionPolicy(
  HISTORY_AGENT_CONTEXT_SOURCE_KEYS,
);
const REFERENCE_DERIVED_COMPRESSION_POLICY = compressionPolicy(
  REFERENCE_DERIVED_CONTEXT_SOURCE_KEYS,
);
...
const HISTORY_AGENT_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      handling: "require-author-input",
      instruction: "目标历史条目不存在时不得调用模型。",
    },
    partial: {
      handling: "grounded-transform",
      instruction:
        "只依据已登记目标条目和可用世界观提供咨询，不补写缺失 Canon。",
    },
    complete: {
      handling: "grounded-transform",
      instruction: "依据完整登记输入提供可审查建议，只写候选结果字段。",
    },
  },
};
...
const WORLD_FOUNDATION_INPUT_POLICY = {
  ...
  states: {
    empty: {
      handling: "create-from-request",
      instruction:
        "当前没有已填写的世界基础；只依据作者本轮要求创建候选，不得声称引用了既有设定。",
    },
    partial: {
      handling: "reference-and-create",
      instruction:
        "把已有世界字段视为约束，只补足缺失联系，不覆盖或擅改作者已填写内容。",
    },
    complete: {
      handling: "grounded-transform",
...
const STORY_CORE_INPUT_POLICY = {
  ...
  states: {
    empty: {
      handling: "create-from-request",
      instruction:
        "世界观与故事核心都为空；若已有角色、故事线或大纲，可把它们作为下游反推证据创建目标字段候选，否则只依据作者要求创建；任何反推都只是候选，不得冒充已确认设定。",
    },
    partial: {
      handling: "reference-and-create",
      instruction:
        "锁定已有世界观或故事核心字段，只补足当前目标字段；可用角色、故事线或大纲反推缺失联系，但不得覆盖作者已填写的其他字段，也不得把反推内容写入非目标字段。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格依据已填写世界观与故事核心生成目标字段，并用现有角色、故事线和大纲检查下游兼容性；下游证据不能覆盖已确认上游。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const CREATIVE_RULES_INPUT_POLICY = {
  ...
  states: {
    empty: {
      handling: "create-from-request",
      instruction:
        "世界观、故事核心和创作规则都为空；只依据项目概况与本轮目标创建单字段建议，不得声称引用了作者尚未填写的设定。",
    },
    partial: {
      handling: "reference-and-create",
      instruction:
        "锁定作者已经填写的世界、故事和其它创作规则，只补当前目标字段；缺失内容可以给出最小建议，不得顺手覆盖其它字段。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格依据已有世界观、故事核心和创作规则建议当前目标字段，并检查它与其它规则是否可共同执行。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const WORLDVIEW_FIELD_INPUT_POLICY = {
  ...
  states: {
    empty: {
      handling: "create-from-request",
      instruction:
        "世界基座和下游故事资料都为空；只依据作者本轮要求创建目标字段候选，不得声称引用了既有设定。",
    },
    partial: {
      handling: "reference-and-create",
      instruction:
        "锁定已有世界字段；缺失上游可依据故事核心、角色、故事线或大纲反推，但只能写当前目标字段，推断不得冒充已确认设定。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格依据已有世界基座并用下游故事资料核对兼容性；下游证据不能覆盖已确认的世界设定。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const CHARACTER_INPUT_POLICY = {
  ...
  states: {
    empty: {
      handling: "create-from-request",
      instruction:
        "当前没有可用设定或角色主档；依据作者要求创建角色候选，并避免伪造既有关系。",
    },
    partial: {
      handling: "reference-and-create",
      instruction:
        "保留已有设定与角色约束，在空白处创建角色；不得复制已有角色或臆造已确认关系。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格依据现有世界与角色阵容创建互补角色，保持术语、能力边界和关系一致。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const CHARACTER_SUPPLEMENT_INPUT_POLICY = {
  ...
  states: {
    empty: {
      handling: "require-author-input",
      instruction:
        "目标角色不存在或不属于当前世界；停止补全，不得创建替代角色。",
    },
    partial: {
      handling: "reference-and-create",
      instruction:
        "锁定目标角色已有设定，只补全作者本次选择的字段；不得修改身份轴、关系或其它字段。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格依据目标角色完整设定补全作者本次选择的字段；若启用剧情证据，还必须服从已确认事实与正文表现。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const OUTLINE_VOLUME_INPUT_POLICY = {
  ...
  states: {
    empty: {
      handling: "create-from-request",
      instruction:
        "上游设定为空；仅按作者本轮要求创建最小可用卷纲候选，不把推断冒充 Canon。",
    },
    partial: {
      handling: "reference-and-create",
      instruction:
        "锁定已有上游设定，围绕缺失部分补足卷级结构，不得改写已确认事实。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格把已有世界、故事核心、角色与故事线编排为卷纲，不新增冲突前提。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const WORLD_GAME_INPUT_POLICY = {
  ...
  states: {
    empty: {
      handling: "require-upstream",
      instruction:
        "缺少已发布的冻结世界或叙事时停止生成，先完成 WorldRelease。",
    },
    partial: {
      handling: "grounded-transform",
      instruction:
        "把已选择的冻结世界资产作为创作地基，补足新的危机、行动、分支后果和结局；不要逐字复刻来源剧情。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "使用完整冻结创作包演化新的可玩剧情；保留便携身份引用，同时让冲突、推进、选择和结局形成独立游戏体验。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const OUTLINE_STORY_ARC_INPUT_POLICY = {
  ...
  states: {
    empty: {
      handling: "create-from-request",
      instruction:
        "上游世界、故事核心和角色均为空；只按作者本轮要求创建最小故事线候选，所有补全都不得冒充已确认设定。",
    },
    partial: {
      handling: "reference-and-create",
      instruction:
        "锁定已有上游内容并围绕缺失部分补足故事线；清楚区分作者事实与规划推断，不覆盖已确认设定。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格把已有世界、故事核心和角色目标编排为主线/支线，保持规则、动机、因果和阶段一致。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const OUTLINE_STORYLINE_PROGRESS_INPUT_POLICY = {
  ...
  states: {
    empty: {
      handling: "require-author-input",
      instruction:
        "缺少已登记故事线或目标章节正文时停止映射；不得凭空创建进度事实。",
    },
    partial: {
      handling: "reference-and-create",
      instruction:
        "只把正文逐字证据映射到已登记故事线；缺少的阶段或交汇不得臆造，疑似新线只能作为待确认候选。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格依据当前章节正文、已登记故事线及作者确认的历史进度，输出可核查的进度、交汇和疑似新线候选。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const OUTLINE_CHARACTER_DRIVEN_INPUT_POLICY = {
  ...
  states: {
    empty: {
      handling: "require-author-input",
      instruction:
        "缺少可执行的角色弧光起点和终点时停止生成，先由作者选择角色并填写状态变化。",
    },
    partial: {
      handling: "reference-and-create",
      instruction:
        "锁定作者填写的角色弧光；以已有世界、故事核心和故事线为约束补足卷章编排，不得另起主线或改写角色终点。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格把作者填写的全部角色弧光编入既有主线/支线和卷章结构，并明确每章的因果、角色参与和弧光推进。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const OUTLINE_CHARACTER_REVISION_INPUT_POLICY = {
  ...
  states: {
    ...
    partial: {
      handling: "reference-and-create",
      instruction:
        "以现有章纲、作者保护边界和可用正文证据为准；证据缺失时只提出可审查建议，不把推断写成既成事实。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格依据已写进度、连续性交接、事实、角色和故事线分析影响；已写区只读，只有作者最终勾选的未来大纲 patch 可写。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const OUTLINE_IMPACT_REGENERATION_INPUT_POLICY = {
  ...
  states: {
    ...
    partial: {
      handling: "require-upstream",
      instruction:
        "来源正文与目标章纲必须同时存在；资料不完整时保持当前摘要并要求作者补齐。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格依据 H57 当前影响项与登记 Context 重建一个后续章纲摘要，只输出可确认候选，不改正文、事实或其它大纲字段。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const OUTLINE_CHAPTER_INPUT_POLICY = {
  ...
  states: {
    ...
    partial: {
      handling: "reference-and-create",
      instruction:
        "以现有卷纲和已填写设定为边界补足章纲；缺失设定只能作为候选假设，不得冒充事实。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格依据卷纲与完整上游设定拆分章纲，保持剧情线、角色阶段和因果顺序。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const OUTLINE_DETAIL_INPUT_POLICY = {
  ...
  states: {
    ...
    partial: {
      handling: "reference-and-create",
      instruction:
        "以现有章纲和已填写设定为硬边界补足场景；未填写的世界或角色细节只能留作候选，不得冒充 Canon。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格依据章纲、故事核心、角色、相邻章和当前细纲拆分场景，避免重复已有场景并保持因果与信息边界。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const PROSE_INPUT_POLICY = {
  ...
  states: {
    ...
    partial: {
      handling: "reference-and-create",
      instruction:
        "章纲和已有设定是硬边界；只在未规定处进行文学创作，不补写成新的已确认设定。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格落实章纲、细纲、故事核心和角色约束，同时遵守本章信息边界。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const PROSE_EMOTION_BEAT_INPUT_POLICY = {
  ...
  states: {
    empty: {
      handling: "require-upstream",
      instruction:
        "缺少当前章纲时不得规划情感节拍，先完成或确认该章标题与摘要。",
    },
    partial: {
      handling: "reference-and-create",
      instruction:
        "以现有章纲和已填写设定为硬边界规划节拍，未规定细节不得冒充已确认剧情。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格落实章纲、细纲、故事核心与角色约束，让 3–6 个节拍形成可执行的情感递进或反转。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const PROSE_POST_ADOPTION_INPUT_POLICY = {
  ...
  states: {
    ...
    partial: {
      handling: "grounded-transform",
      instruction:
        "只从当前已采纳正文抽取有逐字证据的变化，不把未完成片段补写成事实。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "严格依据当前已采纳正文抽取交接与记忆，计划和设定只能用于核对，不能冒充已发生事实。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
...
const WORLD_COMPRESSION_POLICY = compressionPolicy([
  "worldview",
  "powerSystem",
  "codex",
]);
...
const CONSTITUTION_EXTRACT_COMPRESSION_POLICY = compressionPolicy([
  "constitutionScanSources",
]);
const CODEX_EXTRACT_COMPRESSION_POLICY = compressionPolicy([
  "manualText",
  "codexExtractionBaseline",
]);
...
const INSPIRATION_COMPRESSION_POLICY = compressionPolicy([
...
const PROSE_CONSISTENCY_COMPRESSION_POLICY = compressionPolicy([
  "chapterContent",
]);
const INTERACTION_RUNTIME_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      handling: "require-upstream",
      instruction: "缺少角色可见运行时上下文时停止，不得臆造场景、知识或记忆。",
    },
    partial: {
      handling: "require-upstream",
      instruction: "角色运行时上下文不完整时停止，等待重新装配。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "只能依据该角色可见的运行时上下文生成候选，不得读取其他角色私有视角。",
    },
  },
};
const INTERACTION_RUNTIME_COMPRESSION_POLICY = compressionPolicy([
  "interactionRuntime",
]);
const ADVENTURE_RUNTIME_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      handling: "require-upstream",
      instruction: "缺少文字冒险运行时上下文时停止，不得臆造行动或结果。",
    },
    partial: {
      handling: "require-upstream",
      instruction: "文字冒险运行时上下文不完整时停止，等待重新装配。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "自由输入只能映射到登记的当前可执行行动；结果叙述只能改写已提交事件证据。",
    },
  },
};
const ADVENTURE_RUNTIME_COMPRESSION_POLICY = compressionPolicy([
  "adventureRuntime",
]);
const NARRATIVE_SIMULATION_RUNTIME_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      handling: "require-upstream",
      instruction: "缺少叙事模拟运行时上下文时停止，不得臆造局势、行动或结果。",
    },
    partial: {
      handling: "require-upstream",
      instruction: "叙事模拟运行时上下文不完整时停止，等待重新装配。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "只能依据玩家可见状态与正式事件证据生成表现候选，不得改写确定性结算。",
    },
  },
};
const NARRATIVE_SIMULATION_RUNTIME_COMPRESSION_POLICY = compressionPolicy([
  "narrativeSimulationRuntime",
]);
const OPEN_WORLD_RUNTIME_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      handling: "require-upstream",
      instruction:
        "缺少开放世界玩家视角时停止，不得臆造区域、人物、任务或事件。",
    },
    partial: {
      handling: "require-upstream",
      instruction: "开放世界运行时上下文不完整时停止，等待重新装配。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "只能润色已公开任务或叙述正式事件证据，不得改变确定性世界状态。",
    },
  },
};
const OPEN_WORLD_RUNTIME_COMPRESSION_POLICY = compressionPolicy([
  "openWorldRuntime",
]);
const TTRPG_GM_RUNTIME_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      handling: "require-upstream",
      instruction:
        "缺少正式 TTRPG 主持人运行视角时停止，不得臆造场景、裁定或线索。",
    },
    partial: {
      handling: "require-upstream",
      instruction: "TTRPG 主持人运行视角不完整时停止，等待重新装配。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "只能叙述已完成的正式 RulePack 行动，或为当前 GM 控制 NPC 从 availableActions 提出闭集行动意图；不得指定骰点、结果、资源变化、线索权限或场景事实。",
    },
  },
};
...
const TTRPG_PLAYER_RUNTIME_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      handling: "require-upstream",
      instruction:
        "缺少单角色 TTRPG 玩家视角时停止，不得从主持上下文或其他角色视角补全。",
    },
    partial: {
      handling: "require-upstream",
      instruction: "TTRPG 玩家视角不完整时停止，等待按同一角色重新装配。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "只能从 availableActions 与玩家可见目标提出行动意图；不得生成骰点、结果、资源变化、线索或场景推进。",
    },
  },
};
const TTRPG_PLAYER_RUNTIME_COMPRESSION_POLICY = compressionPolicy([
  "ttrpgPlayerRuntime",
]);
const TTRPG_CHARACTER_AUTHORING_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      handling: "require-upstream",
      instruction:
        "缺少冻结 RulePack、CampaignPack 或目标玩家角色时停止，不得凭空车卡。",
    },
    partial: {
      handling: "require-upstream",
      instruction: "单角色车卡上下文不完整时停止，等待重新装配。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "只能提出角色卡候选；数值必须遵守 RulePack，锁定字段不得改变，采用前必须由作者确认。",
    },
  },
};
const TTRPG_CHARACTER_AUTHORING_COMPRESSION_POLICY = compressionPolicy([
  "ttrpg.character-authoring",
]);
const AVG_AUTHORING_INPUT_POLICY: AgentSkillInputPolicyV1 = {
  ...
  states: {
    empty: {
      handling: "require-upstream",
      instruction: "缺少 AVG 媒资与演出上下文时停止，不得臆造媒资 key。",
    },
    partial: {
      handling: "grounded-transform",
      instruction: "只指出缺失项和可用改进，不得把建议直接写入演出模块。",
    },
    complete: {
      handling: "grounded-transform",
      instruction: "基于已登记媒资与 Beat 给出只读演出审阅建议。",
    },
  },
};
...
const PROSE_STYLE_LEARNING_INPUT_POLICY = {
  ...
  states: {
    empty: {
      handling: "require-author-input",
      instruction:
        "缺少合格章节或作者保存的改稿对照时停止学习，不得凭空生成文风画像。",
    },
    partial: {
      handling: "grounded-transform",
      instruction:
        "只依据登记的有限章节样本或改稿对照提炼；证据不足的维度必须明确标注。",
    },
    complete: {
      handling: "grounded-transform",
      instruction:
        "综合登记的章节样本、改稿对照与校准反馈，提炼具体可执行且不挪用剧情实体的文风画像。",
    },
  },
} as const satisfies AgentSkillInputPolicyV1;
const PROSE_STYLE_LEARNING_COMPRESSION_POLICY = compressionPolicy([
  "styleLearningBaseline",
]);

function gameProductionInputPolicy(
  sourceKeys: readonly string[],
): AgentSkillInputPolicyV1 {
  return {
    sourceKeys,
    states: {
      empty: {
        handling: "require-upstream",
        instruction:
          "缺少登记的生产来源时停止，不得从组件状态或其他 Work 猜测。",
      },
      partial: {
        handling: "require-upstream",
        instruction: "生产依赖不完整时停止并报告缺失 key，不得扩大读取范围。",
      },
      complete: {
        handling: "grounded-transform",
        instruction:
          "只能基于已授权 Brief 和显式 Artifact 依赖生成候选；正式发布仍需整包 adoption。",
      },
    },
  };
}
...
export const AGENT_SKILLS = [
  {
    ...
    regressionTests: [
      "R-AGENT1-chat-copilot-world-origin",
      "R-HARNESS2-master-terminal-verifier",
      "R-HARNESS16-semantic-context-compression",
      "R-HARNESS18-execution-version-freshness",
    ],
  },
  {
    version: 1,
    ...
    inputPolicy: {
      ...
      states: {
        empty: {
          handling: "create-from-request",
          instruction:
            "只依据作者本轮概念生成新世界候选，不得声称已有不存在的世界或故事核心。",
        },
        partial: {
          handling: "reference-and-create",
          instruction:
            "锁定已有世界目录或故事核心，只建议尚不存在且有差异化的新世界。",
        },
        complete: {
          handling: "grounded-transform",
          instruction:
            "严格依据当前世界目录和故事核心建议递进世界，不得改写已有世界。",
        },
      },
    },
    ...
    inputPolicy: {
      ...
      states: {
        empty: {
          handling: "create-from-request",
          instruction:
            "只依据作者已保存的世界草稿创建七字段候选，不得声称引用了不存在的设定。",
        },
        partial: {
          handling: "reference-and-create",
          instruction:
            "锁定已有世界观和故事核心，只补足本次七字段候选并保持其它世界差异化。",
        },
        complete: {
          handling: "grounded-transform",
          instruction:
            "严格依据当前世界正式设定扩写七字段，禁止覆盖非目标字段或其它世界。",
        },
      },
    },
  ...
  },
  {
    ...
    inputPolicy: {
      ...
      states: {
        empty: {
          handling: "require-upstream",
          instruction: "没有已登记非空设定源时不调用模型，不创造断言。",
        },
        partial: {
          handling: "grounded-transform",
          instruction: "只从当前登记源、主题、主体和逐字证据闭集抽取候选。",
        },
        complete: {
          handling: "grounded-transform",
          instruction: "只从完整登记闭集抽取候选，不确认或取代 Canon。",
        },
      },
    },
  ...
  },
  {
    ...
    inputPolicy: {
      ...
      states: {
        empty: {
          handling: "require-upstream",
          instruction: "缺少作者来源或登记分类基线时不调用模型。",
        },
        partial: {
          handling: "require-upstream",
          instruction: "必须同时冻结作者来源、目标分类 schema 和同类既有词条。",
        },
        complete: {
          handling: "grounded-transform",
          instruction: "只按当前分类字段闭集拆分可确认词条，不写入未选候选。",
        },
      },
    },
  ...
  },
  {
    ...
    lastVerifiedAt: "2026-08-09",
    regressionTests: [
      "R-AGENT1-chat-copilot-character",
      "R-HARNESS33-character-panel-ui",
      "R-HARNESS2-master-terminal-verifier",
      "R-HARNESS16-semantic-context-compression",
      "R-HARNESS18-execution-version-freshness",
    ],
  ...
  },
  {
    ...
    optionalContextSourceKeys:
      CHARACTER_SUPPLEMENT_OPTIONAL_CONTEXT_SOURCE_KEYS,
    ...
    lastVerifiedAt: "2026-08-09",
    regressionTests: [
      "R-HARNESS38-character-supplement-agent",
      "R-HARNESS38-character-supplement-ui",
    ],
  },
  {
    version: 1,
  ...
  },
  {
    ...
    writeTargets: [
      ...
      {
        table: "references",
        fields: ["analysisSummary"],
        adoptionExtension: "reference-analysis-reference-lifecycle",
      },
    ],
    lastVerifiedAt: "2026-08-14",
  ...
  },
  {
    ...
    lastVerifiedAt: "2026-08-09",
    regressionTests: [
      "R-AGENT1-chat-copilot-inspiration",
      "R-CM1-inspiration-fusion-ui",
      "R-HARNESS34-inspiration-entry",
      "R-HARNESS34-inspiration-panel-ui",
      "R-HARNESS2-master-terminal-verifier",
      "R-HARNESS16-semantic-context-compression",
      "R-HARNESS18-execution-version-freshness",
    ],
  ...
  },
  {
    ...
    writeTargets: [
      { table: "storyArcs", fields: ["name", "type", "stages", "description"] },
    ],
    lastVerifiedAt: "2026-08-09",
    regressionTests: [
  ...
  },
  {
    ...
    writeTargets: [
      {
        ...
        fields: [
          ...
          "progressNote",
          "lastActiveChapterId",
          "lastActiveChapterTitle",
          "involvedEntities",
          "evidenceQuote",
        ],
      },
      {
        table: "storylineCrossings",
        fields: [
          "arcIdA",
          "arcIdB",
          "chapterId",
    ...
    lastVerifiedAt: "2026-08-10",
    regressionTests: [
      "R-HARNESS40-storyline-progress-agent",
      "R-PHASE39-storyline-progress-ui",
    ],
  },
  {
    version: 1,
    ...
    writeTargets: [
      { table: "characterDrivenPlans", fields: ["generatedVolumes", "status"] },
    ],
    lastVerifiedAt: "2026-08-09",
    regressionTests: [
  ...
  },
  {
    ...
    writeTargets: [
      {
        table: "narrativeModules",
        fields: [],
        adoptionExtension: "world-game-narrative-modules",
      },
      {
        table: "narrativeNodes",
        fields: [],
        adoptionExtension: "world-game-narrative-nodes",
      },
      {
        table: "narrativeBeats",
        fields: [],
        adoptionExtension: "world-game-narrative-beats",
      },
      {
        table: "narrativeChoices",
        fields: [],
        adoptionExtension: "world-game-narrative-choices",
      },
      {
        table: "gameDefinitions",
        fields: [],
        adoptionExtension: "world-game-definitions",
      },
      {
        table: "adventureModules",
        fields: [],
        adoptionExtension: "world-game-adventure-modules",
      },
      {
        table: "interactionCharacterProfiles",
        fields: [],
        adoptionExtension: "world-game-interaction-profiles",
      },
      {
        table: "interactionSceneTemplates",
        fields: [],
        adoptionExtension: "world-game-interaction-scenes",
      },
      {
        table: "avgPresentationModules",
        fields: [],
        adoptionExtension: "world-game-avg-presentations",
      },
      {
        table: "avgMediaAssets",
        fields: [],
        adoptionExtension: "world-game-avg-media-assets",
      },
      {
        table: "avgMediaBlobs",
        fields: [],
        adoptionExtension: "world-game-avg-media-blobs",
      },
    ],
  ...
  },
  {
    ...
    lastVerifiedAt: "2026-08-08",
    regressionTests: [
      "R-AGENT1-chat-copilot-outline",
      "R-HARNESS11-outline-batch-durable",
      "R-HARNESS16-semantic-context-compression",
      "R-HARNESS18-execution-version-freshness",
    ],
  },
  {
    ...
    lastVerifiedAt: "2026-08-08",
    regressionTests: [
      "R-HARNESS14-workflow-classifier",
      "R-AGENT1-chat-copilot-outline",
      "R-HARNESS16-semantic-context-compression",
      "R-HARNESS18-execution-version-freshness",
    ],
  ...
  },
  {
    ...
    lastVerifiedAt: "2026-08-08",
    regressionTests: [
      "R-HARNESS7-prose-generation-durable",
      "R-HARNESS9-information-boundary",
      "R-HARNESS16-semantic-context-compression",
      "R-HARNESS18-execution-version-freshness",
    ],
  },
  {
    ...
    regressionTests: [
      ...
      "R-HARNESS7-prose-generation-durable",
      "R-HARNESS16-semantic-context-compression",
      "R-HARNESS17-context-compression-eval",
      "R-HARNESS18-execution-version-freshness",
    ],
  },
  {
    ...
    inputPolicy: {
      ...
      states: {
        empty: {
          handling: "create-from-request",
          instruction:
            "项目设定为空时只创建最小伏笔候选，不得声称引用了既有 Canon。",
        },
        partial: {
          handling: "reference-and-create",
          instruction:
            "锁定已有世界、角色与伏笔，新增候选不得覆盖或重复作者记录。",
        },
        complete: {
          handling: "grounded-transform",
          instruction:
            "依据完整登记上下文设计可埋设、可兑现且不重复的伏笔候选。",
        },
      },
    },
  ...
  },
  {
    ...
    lastVerifiedAt: "2026-08-08",
    regressionTests: [
      "R-AGENT5-chapter-organization",
      "R-HARNESS20-chapter-post-adoption-durable",
    ],
  },
  {
    version: 1,
  ...
  },
  {
    ...
    optionalContextSourceKeys: [],
    inputPolicy: gameProductionInputPolicy([
      "game-production.consultation-source",
    ]),
  ...
  },
  {
    ...
    contextCompression: compressionPolicy([
      ...
      "game-production.artifact-inputs",
    ]),
    maxOutputTokens: 16_000,
    writeTargets: [
      {
        table: "gameBuildArtifacts",
        fields: ["payloadJson"],
  ...
  },
  {
    ...
    lastVerifiedAt: "2026-08-21",
    regressionTests: [
      "R-GAMEPROD1E-media-adapters",
      "R-GAMEPROD1F-production-executor",
    ],
  },
  {
    version: 1,
    ...
    writeTargets: [
      {
        table: "gameBuilds",
        fields: ["manifestJson", "previewManifestJson"],
        adoptionExtension: "game-production-builds",
      },
    ],
    lastVerifiedAt: "2026-08-21",
  ...
  },
  {
    ...
    contextSourceKeys: [
      "game-production.brief",
      "game-production.quality-feedback",
    ],
    optionalContextSourceKeys: [],
    inputPolicy: gameProductionInputPolicy([
      "game-production.brief",
  ...
  },
  {
    ...
    writeTargets: [
      {
        table: "gameBuilds",
        fields: ["compatibilityJson"],
        adoptionExtension: "game-production-builds",
      },
    ],
    lastVerifiedAt: "2026-08-21",
    regressionTests: [
      "R-GAMEPROD1B-commands",
      "R-GAMEPROD1D-scheduler",
      "R-GAMEPROD1F-production-executor",
    ],
  },
...
export const AGENT_SKILL_IDS: readonly AgentSkillId[] = AGENT_SKILLS.map(
  (skill) => skill.id,
);
...
export function resolveAgentSkillContextSourceKeysV1(
  ...
  const toolSources = skill.readToolNames.flatMap(
    (name) => AGENT_TOOL_BY_NAME.get(name)?.sourceKeys ?? [],
  );
  return [
    ...new Set([
      ...toolSources,
...
export function buildAgentSkillInputGuidanceV1(
  ...
  const available = state.availableSourceKeys.length
    ? state.availableSourceKeys.join("、")
    : "无";
  const missing = state.missingSourceKeys.length
    ? state.missingSourceKeys.join("、")
    : "无";
  const degraded = [
    ...new Set([...state.truncatedSourceKeys, ...state.trimmedSourceKeys]),
  ];
  return [
    ...
    ...(degraded.length
      ? [`受预算影响的来源：${degraded.join("、")}；不得假装已看到其全文。`]
      : []),
  ].join("\n");
}
...
export function validateAgentSkillContextEvidenceV1(
  ...
  const included = assertUniqueStringArray(
    evidence.included,
    "上下文 included",
  );
  ...
  const authorized = new Set(
    resolveAgentSkillContextSourceKeysV1(skill, { includeOptional: true }),
  );
  for (const key of [...included, ...omitted, ...trimmed]) {
    if (!authorized.has(key))
      throw new Error(`Agent Skill ${skill.id} 上下文来源越权 ${key}`);
  }
  if (
  ...
  if (evidence.sourceEvidence) {
    ...
    for (const source of evidence.sourceEvidence) {
      if (keys.has(source.key))
        throw new Error(`Agent Skill ${skill.id} 来源证据重复 ${source.key}`);
      keys.add(source.key);
      if (!authorized.has(source.key))
        throw new Error(`Agent Skill ${skill.id} 来源证据越权 ${source.key}`);
      if (
        ...
        throw new Error(
          `Agent Skill ${skill.id} 来源 ${source.key} token 证据无效`,
        );
      if (source.status === "included") {
        if (
          !included.includes(source.key) ||
          source.delivery === "none" ||
          source.inputTokens < 1
        ) {
          throw new Error(
            `Agent Skill ${skill.id} 来源 ${source.key} included 证据不一致`,
          );
        }
        if (
          ...
          throw new Error(
            `Agent Skill ${skill.id} 来源 ${source.key} full 证据不一致`,
          );
        }
        if (
          ...
          throw new Error(
            `Agent Skill ${skill.id} 来源 ${source.key} truncated 证据不一致`,
          );
        }
        if (
          source.delivery === "compressed" &&
          source.originalTokens <= source.inputTokens
        ) {
          throw new Error(
            `Agent Skill ${skill.id} 来源 ${source.key} compressed 证据不一致`,
          );
        }
        ...
        throw new Error(
          `Agent Skill ${skill.id} 来源 ${source.key} 未交付证据不一致`,
        );
      }
      if (source.compression) {
        ...
        if (
          ...
          compression.attempts >
            skill.contextCompression.maxAttemptsPerSource ||
          ...
          throw new Error(
            `Agent Skill ${skill.id} 来源 ${source.key} 压缩证据无效`,
          );
        if (!skill.contextCompression.sourceKeys.includes(source.key)) {
          throw new Error(
            `Agent Skill ${skill.id} 来源 ${source.key} 不允许压缩`,
          );
        }
        if (compression.outcome === "verified") {
          if (
            ...
            compression.coveredAnchorCount !==
              compression.requiredAnchorCount ||
            (source.status === "included" && source.delivery !== "compressed")
          )
            throw new Error(
              `Agent Skill ${skill.id} 来源 ${source.key} 已验证压缩证据不一致`,
            );
        } else if (
          ...
          throw new Error(
            `Agent Skill ${skill.id} 来源 ${source.key} 压缩回退证据不一致`,
          );
      } else if (source.delivery === "compressed") {
        throw new Error(
          `Agent Skill ${skill.id} 来源 ${source.key} 缺少压缩证据`,
        );
      }
    }
    if (deliveredTokens !== evidence.estimatedInputTokens) {
      throw new Error(
        `Agent Skill ${skill.id} 来源 token 合计与上下文证据不一致`,
      );
    }
  }
...
export function validateAgentSkillDefinitionsV1(
  ...
  const executionModesByAgent: Record<
    DomainAgentId,
    ReadonlySet<AgentSkillExecutionModeV1>
  > = {
    "world-origin": new Set([
      "complete",
      "worldview-field",
      "world-suggest",
      "worldview-expand",
  ...
  for (const skill of definitions) {
    if (skill.version !== 1)
      throw new Error(`Agent Skill ${skill.id} 版本不受支持`);
    if (ids.has(skill.id)) throw new Error(`Agent Skill ID 重复：${skill.id}`);
    ids.add(skill.id);
    if (skill.defaultForAgent) {
      if (defaultAgents.has(skill.agentId))
        throw new Error(`Agent ${skill.agentId} 存在多个默认 Skill`);
      defaultAgents.add(skill.agentId);
    }
    if (!skill.owner.trim())
      throw new Error(`Agent Skill ${skill.id} 缺少 owner`);
    ...
    if (skill.regressionTests.length === 0)
      throw new Error(`Agent Skill ${skill.id} 缺少回归证据`);
    for (const toolName of skill.readToolNames) {
      const tool = AGENT_TOOL_BY_NAME.get(toolName);
      if (!tool || tool.risk !== "read")
        throw new Error(
          `Agent Skill ${skill.id} 引用了未知只读工具 ${toolName}`,
        );
    }
    ...
    for (const sourceKey of authorizedSourceKeys) {
      if (!CONTEXT_SOURCE_BY_KEY.has(sourceKey)) {
        throw new Error(
          `Agent Skill ${skill.id} 引用了未登记上下文源 ${sourceKey}`,
        );
      }
    }
    ...
    if (
      new Set(skill.inputPolicy.sourceKeys).size !==
      skill.inputPolicy.sourceKeys.length
    ) {
    ...
    for (const sourceKey of skill.inputPolicy.sourceKeys) {
      if (!authorizedSourceKeys.includes(sourceKey)) {
        throw new Error(
          `Agent Skill ${skill.id} 的输入状态来源未获读取授权 ${sourceKey}`,
        );
      }
    }
    ...
    if (compression.version !== 1)
      throw new Error(`Agent Skill ${skill.id} 的压缩策略版本无效`);
    if (
      !compression.sourceKeys.length ||
      new Set(compression.sourceKeys).size !== compression.sourceKeys.length
    ) {
    ...
    for (const sourceKey of compression.sourceKeys) {
      if (!authorizedSourceKeys.includes(sourceKey)) {
        throw new Error(
          `Agent Skill ${skill.id} 的压缩来源未获读取授权 ${sourceKey}`,
        );
      }
    }
    ...
    if (
      !Number.isFinite(compression.maxFullTextBudgetScale) ||
      compression.maxFullTextBudgetScale < 1
    ) {
    ...
    for (const target of skill.writeTargets) {
      if (!REGISTRY_BY_NAME.has(target.table)) {
        throw new Error(
          `Agent Skill ${skill.id} 引用了未登记表 ${target.table}`,
        );
      }
      const registered = new Set(
        (FIELD_BY_TARGET.get(target.table) ?? []).map((field) => field.field),
      );
      const extension = target.adoptionExtension
        ? ADOPTION_EXTENSIONS.find(
            (item) => item.id === target.adoptionExtension,
          )
        : undefined;
      if (
        target.adoptionExtension &&
        (!extension || extension.target !== target.table)
      ) {
        throw new Error(
          `Agent Skill ${skill.id} 引用了未登记或目标不匹配的采纳扩展 ${target.adoptionExtension}`,
        );
      }
      if (target.fields.length === 0 && !target.adoptionExtension) {
        throw new Error(
          `Agent Skill ${skill.id} 的写目标 ${target.table} 未声明字段或采纳扩展`,
        );
      }
      for (const field of target.fields) {
        if (!registered.has(field)) {
          throw new Error(
            `Agent Skill ${skill.id} 引用了未登记写字段 ${target.table}.${field}`,
          );
        }
      }
    }
  ...
  for (const agentId of DOMAIN_AGENT_IDS) {
    if (!defaultAgents.has(agentId))
      throw new Error(`Agent ${agentId} 缺少默认 Skill`);
  }
}

validateAgentSkillDefinitionsV1(AGENT_SKILLS);

export const AGENT_SKILL_BY_ID: ReadonlyMap<string, AgentSkillDefinitionV1> =
  new Map(AGENT_SKILLS.map((skill) => [skill.id, skill]));

export const DEFAULT_AGENT_SKILL_BY_AGENT: ReadonlyMap<
  DomainAgentId,
  AgentSkillDefinitionV1
> = new Map(
  AGENT_SKILLS.filter((skill) => skill.defaultForAgent).map((skill) => [
    skill.agentId,
    skill,
  ]),
);

export function getDefaultAgentSkillV1(
  agentId: DomainAgentId,
): AgentSkillDefinitionV1 {
...
export function resolveAgentSkillV1(
  ...
  return skillId
    ? getAgentSkillV1(skillId, agentId)
    : getDefaultAgentSkillV1(agentId);
}
