...
export const GAME_PLATFORM_DEPLOYMENT_SCENARIOS_V1 = [
...
export type GamePlatformDeploymentScenarioV1 = typeof GAME_PLATFORM_DEPLOYMENT_SCENARIOS_V1[number]

export interface GamePlatformDeploymentScenarioEvidenceV1 {
  evidenceHash: string
  assertionCount: number
}

export interface GamePlatformDeploymentScenarioResultV1 {
  scenario: GamePlatformDeploymentScenarioV1
...
export interface GamePlatformDeploymentConformanceReceiptV1 {
  schema: 'storyforge.game-platform-deployment-conformance'
  version: 1
  ...
  startedAt: number
  completedAt: number
  scenarios: GamePlatformDeploymentScenarioResultV1[]
  readyForPromotion: boolean
  receiptHash: string
}

export type GamePlatformDeploymentScenarioRunnersV1 = Record<
  GamePlatformDeploymentScenarioV1,
  (input: {
    scenario: GamePlatformDeploymentScenarioV1
    signal: AbortSignal
  }) => Promise<GamePlatformDeploymentScenarioEvidenceV1>
>

export class GamePlatformDeploymentScenarioErrorV1 extends Error {
  constructor(public readonly code: string) {
    super(`[game-platform-deployment:${code}] scenario failed`)
    this.name = 'GamePlatformDeploymentScenarioErrorV1'
  }
}
...
function stableId(value: unknown, label: string, maximum: number): string {
  if (typeof value !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._:/-]*$/.test(value)
    ...
    throw new Error(`[game-platform-deployment:protocol] ${label} 无效`)
  }
  return value
}
...
function exactKeys(value: Record<string, unknown>, expected: string[], label: string): void {
  ...
  if (actual.length !== expected.length || actual.some(key => !expected.includes(key))) {
    throw new Error(`[game-platform-deployment:protocol] ${label} 字段不符合协议`)
  }
}

function scenarioCode(error: unknown): string {
  return error instanceof GamePlatformDeploymentScenarioErrorV1
 ...
 * from running.
 */
export async function runGamePlatformDeploymentConformanceV1(input: {
  environment: 'staging' | 'production'
  deploymentId: string
  deploymentTargetHash: string
  serviceVersion: string
  runners: GamePlatformDeploymentScenarioRunnersV1
  scenarioTimeoutMs?: number
  now?: () => number
}): Promise<GamePlatformDeploymentConformanceReceiptV1> {
  if (!['staging', 'production'].includes(String(input.environment))) {
    throw new Error('[game-platform-deployment:protocol] environment 无效')
  }
  ...
  if (!Number.isInteger(scenarioTimeoutMs) || scenarioTimeoutMs < 100 || scenarioTimeoutMs > 10 * 60_000) {
    throw new Error('[game-platform-deployment:configuration] scenarioTimeoutMs 无效')
  }
  const now = input.now ?? (() => Date.now())
  const readNow = (label: string) => timestamp(now(), label)
  const startedAt = readNow('startedAt')
  const scenarios: GamePlatformDeploymentScenarioResultV1[] = []
  for (const scenario of GAME_PLATFORM_DEPLOYMENT_SCENARIOS_V1) {
    ...
    try {
      ...
      if (typeof runner !== 'function') throw new GamePlatformDeploymentScenarioErrorV1('runner-missing')
      ...
      if (!evidence || typeof evidence !== 'object' || Array.isArray(evidence)
        ...
        throw new GamePlatformDeploymentScenarioErrorV1('evidence-invalid')
      }
      ...
      if (!Number.isInteger(evidence.assertionCount) || evidence.assertionCount < 1 || evidence.assertionCount > 1_000_000) {
        throw new GamePlatformDeploymentScenarioErrorV1('assertion-count-invalid')
      }
      const completedAt = readNow(`${scenario}.completedAt`)
      if (completedAt < scenarioStartedAt) {
        throw new GamePlatformDeploymentScenarioErrorV1('clock-regressed')
      }
      scenarios.push({
  ...
  const body = {
    schema: 'storyforge.game-platform-deployment-conformance' as const,
    version: 1 as const,
...
export async function verifyGamePlatformDeploymentConformanceReceiptV1(
  input: unknown,
): Promise<GamePlatformDeploymentConformanceReceiptV1> {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw new Error('[game-platform-deployment:protocol] receipt 必须是对象')
  }
  ...
  if (receipt.schema !== 'storyforge.game-platform-deployment-conformance' || receipt.version !== 1
    || !['staging', 'production'].includes(String(receipt.environment))) {
    throw new Error('[game-platform-deployment:protocol] receipt 版本无效')
  }
  ...
  if (!Number.isInteger(receipt.startedAt) || !Number.isInteger(receipt.completedAt)
    ...
    || receipt.scenarios.length !== GAME_PLATFORM_DEPLOYMENT_SCENARIOS_V1.length) {
    throw new Error('[game-platform-deployment:protocol] receipt 状态无效')
  }
  const scenarios = receipt.scenarios.map((value, index) => {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
      throw new Error('[game-platform-deployment:protocol] scenario 无效')
    }
    ...
    if (row.scenario !== GAME_PLATFORM_DEPLOYMENT_SCENARIOS_V1[index]
      ...
      throw new Error('[game-platform-deployment:protocol] scenario 状态无效')
    }
    if (row.status === 'passed') {
      if (row.code !== 'ok' || Number(row.assertionCount) < 1) {
        throw new Error('[game-platform-deployment:protocol] passed scenario 证据无效')
      }
      sha(row.evidenceHash, 'scenario.evidenceHash')
    } else if (row.evidenceHash !== null || row.assertionCount !== 0 || row.code === 'ok') {
      throw new Error('[game-platform-deployment:protocol] failed scenario 证据无效')
    }
    return row
  })
  ...
  for (let index = 1; index < scenarios.length; index += 1) {
    if (Number(scenarios[index].startedAt) < Number(scenarios[index - 1].completedAt)) {
      throw new Error('[game-platform-deployment:protocol] scenario 时间顺序无效')
    }
  }
  ...
  if (new Set(evidenceHashes).size !== evidenceHashes.length) {
    throw new Error('[game-platform-deployment:protocol] scenario 证据不得重用')
  }
  if (receipt.readyForPromotion !== ready) {
    throw new Error('[game-platform-deployment:protocol] promotion 裁决与 scenario 不一致')
  }
  const { receiptHash, ...body } = receipt
  if (await hashCanonicalValue(body) !== receiptHash) {
    throw new Error('[game-platform-deployment:integrity] receipt hash 不匹配')
  }
  return structuredClone(input) as GamePlatformDeploymentConformanceReceiptV1
}
