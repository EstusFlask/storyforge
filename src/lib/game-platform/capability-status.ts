export const GAME_PLATFORM_ROLLOUT_STAGES_V1 = [
  'disabled',
  'developer',
...
export type GamePlatformRolloutStageV1 = typeof GAME_PLATFORM_ROLLOUT_STAGES_V1[number]
export type GamePlatformEnvironmentV1 = 'test' | 'development' | 'preview' | 'production'
export type GamePlatformEvidenceStatusV1 = 'proved' | 'partial' | 'pending'

export type GamePlatformCapabilityIdV1 =
  | 'playable-world-bundle'
  | 'game-production-v3'
  | 'ttrpg-formal-local'
  | 'ttrpg-ai-gm'
...
export interface GamePlatformCapabilityStatusV1 {
  id: GamePlatformCapabilityIdV1
  rollout: GamePlatformRolloutStageV1
  evidence: GamePlatformEvidenceStatusV1
  userLabel: string
  reason: string
  requiresOnlineService: boolean
  requiresAiGmBetaGate: boolean
  allowedEnvironments: GamePlatformEnvironmentV1[]
}

export interface GamePlatformCapabilityContextV1 {
  environment: GamePlatformEnvironmentV1
...
export interface GamePlatformCapabilityDecisionV1 {
  enabled: boolean
  capability: GamePlatformCapabilityStatusV1
  blockers: string[]
}
...
export const GAME_PLATFORM_CAPABILITIES_V1: readonly GamePlatformCapabilityStatusV1[] = Object.freeze([
  {
    id: 'playable-world-bundle', rollout: 'default', evidence: 'proved', userLabel: '世界游戏出口',
    reason: '不可变 WorldRelease 可编译为带来源证据的 PlayableWorldBundle。',
    requiresOnlineService: false, requiresAiGmBetaGate: false,
    allowedEnvironments: ['test', 'development', 'preview', 'production'],
  },
  {
    id: 'game-production-v3', rollout: 'author-opt-in', evidence: 'proved', userLabel: '自动游戏制作',
    reason: '七类游戏均进入 Brief、Build、Preview、质量门与原子发布链；TTRPG 使用九步 Brief、提案选择、验证后的 CampaignPack 与冻结 RulePack，不再依赖固定模板。',
    requiresOnlineService: false, requiresAiGmBetaGate: false,
    allowedEnvironments: ['test', 'development', 'preview', 'production'],
  },
  {
...
const CAPABILITY_BY_ID = new Map(GAME_PLATFORM_CAPABILITIES_V1.map(item => [item.id, item]))

export function getGamePlatformCapabilityStatusV1(
  id: GamePlatformCapabilityIdV1,
): GamePlatformCapabilityStatusV1 {
  const capability = CAPABILITY_BY_ID.get(id)
  if (!capability) throw new Error(`[game-platform-capability] 未登记能力:${id}`)
  return structuredClone(capability)
}

export function evaluateGamePlatformCapabilityV1(
  id: GamePlatformCapabilityIdV1,
  context: GamePlatformCapabilityContextV1,
): GamePlatformCapabilityDecisionV1 {
  const capability = getGamePlatformCapabilityStatusV1(id)
...
export function currentGamePlatformEnvironmentV1(): GamePlatformEnvironmentV1 {
