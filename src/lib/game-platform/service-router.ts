export interface GamePlatformReadinessV1 {
  ready: boolean
  serviceVersion: string
  checks: Record<string, 'ready' | 'missing' | 'development-only' | 'unhealthy'>
  /** Probe time, not service boot time. Omitted by the legacy/static evaluator. */
  checkedAt?: number
}

type FetchHandlerV1 = (request: Request) => Promise<Response>

export type GamePlatformServiceDomainV1 = 'commercial' | 'community' | 'online' | 'operations'

export interface GamePlatformRateLimitDecisionV1 {
  allowed: boolean
  limit: number
  remaining: number
  retryAfterMs: number
...
export interface GamePlatformRateLimiterV1 {
  consume(input: {
    ...
    cost: number
    now: number
  }): GamePlatformRateLimitDecisionV1 | Promise<GamePlatformRateLimitDecisionV1>
}

export interface GamePlatformRequestGuardAuditV1 {
  requestId: string
  domain: GamePlatformServiceDomainV1
  method: string
...
export interface GamePlatformRequestGuardV1 {
  /**
   ...
   */
  resolveSubject(request: Request, domain: GamePlatformServiceDomainV1): Promise<{
    subjectKey: string
    subjectKind: GamePlatformRequestGuardAuditV1['subjectKind']
  }>
  rateLimiter: GamePlatformRateLimiterV1
  audit(entry: GamePlatformRequestGuardAuditV1): void | Promise<void>
  now?: () => number
}

export interface GamePlatformRequestAuthorityV1 {
  /**
...
function domainForPath(pathname: string): GamePlatformServiceDomainV1 | null {
...
function guardedResponse(response: Response, input: {
  requestId: string
  decision: GamePlatformRateLimitDecisionV1
}): Response {
...
export function createGamePlatformServiceRouterV1(input: {
  commercial: FetchHandlerV1
  community: FetchHandlerV1
  online: FetchHandlerV1
  operations: FetchHandlerV1
  readiness: (input?: { force?: boolean }) => GamePlatformReadinessV1 | Promise<GamePlatformReadinessV1>
  guard: GamePlatformRequestGuardV1
...
export function createMemoryGamePlatformRateLimiterV1(input: {
  limit: number
  windowMs: number
  maximumSubjects?: number
}): GamePlatformRateLimiterV1 {
  if (!Number.isInteger(input.limit) || input.limit <= 0) throw new Error('[game-platform-rate-limit] limit 无效')
  if (!Number.isInteger(input.windowMs) || input.windowMs <= 0) throw new Error('[game-platform-rate-limit] windowMs 无效')
  const maximumSubjects = input.maximumSubjects ?? 10_000
  if (!Number.isInteger(maximumSubjects) || maximumSubjects <= 0) throw new Error('[game-platform-rate-limit] maximumSubjects 无效')
  const windows = new Map<string, { startedAt: number; used: number }>()
  return {
    consume({ subjectKey, bucket, cost, now }) {
      if (!Number.isInteger(cost) || cost <= 0) throw new Error('[game-platform-rate-limit] cost 无效')
      ...
      if (!window || now - window.startedAt >= input.windowMs) {
        if (!window && windows.size >= maximumSubjects) {
          ...
          if (windows.size >= maximumSubjects) throw new Error('[game-platform-rate-limit] subject capacity reached')
        }
...
export type GamePlatformProductionDependencyV1 =
...
export const GAME_PLATFORM_PRODUCTION_DEPENDENCIES_V1: readonly GamePlatformProductionDependencyV1[] = [
  'identity-provider', 'transactional-commercial-store', 'transactional-community-store',
  'transactional-online-store', 'transactional-operations-store', 'object-storage',
  'payment-provider', 'webhook-secret-manager', 'realtime-fanout', 'rate-limiter',
  'single-writer-coordination',
...
export function evaluateGamePlatformProductionReadinessV1(input: {
  serviceVersion: string
  environment: 'development' | 'production'
  dependencies: Partial<Record<GamePlatformProductionDependencyV1, 'configured' | 'memory'>>
}): GamePlatformReadinessV1 {
  if (!input.serviceVersion.trim() || input.serviceVersion.length > 100 || /[\r\n]/.test(input.serviceVersion)) throw new Error('[game-platform-router:configuration] serviceVersion 无效')
  const checks = Object.fromEntries(GAME_PLATFORM_PRODUCTION_DEPENDENCIES_V1.map(key => [key, input.dependencies[key] === 'configured' ? 'ready' : input.dependencies[key] === 'memory' ? 'development-only' : 'missing'])) as GamePlatformReadinessV1['checks']
