...
import {
  createGamePlatformServiceRouterV1,
  type GamePlatformRateLimiterV1,
  type GamePlatformProductionDependencyV1,
  type GamePlatformRequestAuthorityV1,
  type GamePlatformRequestGuardV1,
...
export interface HostedGamePlatformIdentityV1 {
...
export interface HostedGamePlatformAuditSinksV1 {
...
export interface HostedGamePlatformNamespacesV1 {
  commercial: string
  community: string
  operations: string
  online: string
...
export interface HostedGamePlatformServiceV1 {
  fetch(request: Request): Promise<Response>
  readiness: GamePlatformReadinessV1
  readinessNow(input?: { force?: boolean }): Promise<GamePlatformReadinessV1>
...
export interface HostedGamePlatformBootstrapV1 {
  fetch(request: Request): Promise<Response>
  current(): HostedGamePlatformServiceV1 | null
}

export type HostedGamePlatformProductionStorageV1 = TransactionalKeyValueStorageV1 & {
  dependencyAdapters: {
    'transactional-commercial-store': GamePlatformProductionDependencyAdapterV1<'transactional-commercial-store'>
    'transactional-community-store': GamePlatformProductionDependencyAdapterV1<'transactional-community-store'>
    'transactional-online-store': GamePlatformProductionDependencyAdapterV1<'transactional-online-store'>
    'transactional-operations-store': GamePlatformProductionDependencyAdapterV1<'transactional-operations-store'>
  }
  lfgHandoffPersistence: CommunityLfgRoomHandoffPersistenceV1
}

export type HostedGamePlatformProductionSecretManagerV1 = CommercialWebhookSecretProviderV1
  & CommunityLfgRoomSecretVaultV1
  & HostedOnlineRoomCredentialIssuerV1
  & GamePlatformProductionDependencyAdapterV1<'webhook-secret-manager'>

export interface HostedGamePlatformProductionRuntimeV1 {
  identity: HostedGamePlatformIdentityV1
    & GamePlatformProductionDependencyAdapterV1<'identity-provider'>
  /** The exact transactional storage used by all four authorities and LFG handoff state. */
  storage: HostedGamePlatformProductionStorageV1
  releaseDeliveryPersistence: CommercialReleaseDeliveryPersistenceV1
    & GamePlatformProductionDependencyAdapterV1<'object-storage'>
  checkoutProvider: CommercialCheckoutProviderV1
    & GamePlatformProductionDependencyAdapterV1<'payment-provider'>
  /** One deployment-owned secret boundary for webhook, LFG and room credentials. */
  secretManager: HostedGamePlatformProductionSecretManagerV1
  realtime: OnlineRoomRealtimeCoordinatorV1
    & GamePlatformProductionDependencyAdapterV1<'realtime-fanout'>
  rateLimiter: GamePlatformRateLimiterV1
    & GamePlatformProductionDependencyAdapterV1<'rate-limiter'>
  requestAuthority: GamePlatformRequestAuthorityV1
    & GamePlatformProductionDependencyAdapterV1<'single-writer-coordination'>
}

export interface HostedGamePlatformServiceInputV1 {
  ...
  requestGuard: GamePlatformRequestGuardV1
  productionRuntime?: HostedGamePlatformProductionRuntimeV1
  productionProbeTimeoutMs?: number
  productionReadinessCacheTtlMs?: number
  productionProbeAudit?: (entry: GamePlatformProductionProbeAuditV1) => void | Promise<void>
  namespaces?: Partial<HostedGamePlatformNamespacesV1>
  audits?: HostedGamePlatformAuditSinksV1
  now?: () => number
...
const DEFAULT_NAMESPACES: HostedGamePlatformNamespacesV1 = {
...
class CatalogTtrpgReleaseStoreV1 implements HostedTtrpgReleaseStoreV1 {
  ...
  async loadByContentHash(contentHash: string): Promise<HostedTtrpgReleaseRecordV1 | null> {
    ...
    if (!delivered || delivered.releaseHash !== contentHash
      || delivered.bundle.gameRelease.contentHash !== contentHash) return null
    ...
    return {
      contentHash,
      manifestJson: JSON.stringify(delivered.bundle.gameRelease.manifest),
 ...
 */
export async function createHostedGamePlatformServiceV1(
  input: HostedGamePlatformServiceInputV1,
): Promise<HostedGamePlatformServiceV1> {
  ...
  if (input.environment === 'development' && !input.webhookSecret && !input.productionRuntime) {
    throw new Error('[hosted-game-platform:configuration] 开发环境缺少 webhookSecret')
  }
  const productionAdapters: GamePlatformProductionDependencyAdaptersV1 | undefined = input.productionRuntime
    ? {
    ...
    : undefined
  const activeReadiness = createGamePlatformActiveReadinessV1({
  ...
  const router = createGamePlatformServiceRouterV1({
 ...
 */
export function createHostedGamePlatformBootstrapV1(input: HostedGamePlatformServiceInputV1 & {
  bootstrapRetryBackoffMs?: number
}): HostedGamePlatformBootstrapV1 {
  ...
  if (!Number.isInteger(retryBackoffMs) || retryBackoffMs < 100 || retryBackoffMs > 60_000) {
    throw new Error('[hosted-game-platform:configuration] bootstrapRetryBackoffMs 无效')
  }
  if (!input.serviceVersion.trim() || input.serviceVersion.length > 100 || /[\r\n]/.test(input.serviceVersion)) {
    throw new Error('[hosted-game-platform:configuration] serviceVersion 无效')
  }
  const now = input.now ?? (() => Date.now())
  let service: HostedGamePlatformServiceV1 | null = null
  let initializing: Promise<HostedGamePlatformServiceV1 | null> | null = null
  let lastFailureAt: number | null = null

  const ensure = async (): Promise<HostedGamePlatformServiceV1 | null> => {
    if (service) return service
    if (initializing) return initializing
    const observedAt = now()
    if (lastFailureAt != null && observedAt - lastFailureAt < retryBackoffMs) return null
    initializing = createHostedGamePlatformServiceV1(input)
      .then(created => {
        service = created
        lastFailureAt = null
        return created
