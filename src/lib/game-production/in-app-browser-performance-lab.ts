...
import { createBuildGameMediaResolver } from './media-resolver'
...
export async function collectBrowserPerformanceSamplesV1(input: {
  ...
  if (!Number.isFinite(input.durationMs) || input.durationMs < 1
    ...
    throw new Error('[game-browser-performance-lab] 采样参数无效')
  }
  ...
  for (let index = 0; index < input.minimumLatencySamples; index += 1) {
    ...
    if (!Number.isFinite(sample.sceneLatencyMs) || sample.sceneLatencyMs < 0
      ...
      throw new Error('[game-browser-performance-lab] 浏览器延迟样本无效')
    }
  ...
  while (true) {
    ...
    if (elapsedMs >= nextMemoryAt) {
      ...
      if (!Number.isFinite(latestHeapBytes) || latestHeapBytes < 0) {
        throw new Error('[game-browser-performance-lab] 浏览器堆内存样本无效')
      }
...
async function readBrowserHeapBytes(): Promise<number> {
  ...
  if (!Number.isFinite(used) || Number(used) < 0) {
    throw new Error('[game-browser-performance-lab] 当前浏览器没有可验证的 JS 堆内存接口')
  }
  return Number(used)
}
...
export async function runInAppBrowserPerformanceLabV1(input: {
  ...
  durationMs?: number
}): Promise<GameBrowserPerformanceMeasurementV1> {
  ensureActive(input.signal)
  const durationMs = input.durationMs ?? GAME_BROWSER_PERFORMANCE_POLICY_V1.minimumLongRunDurationMs
  ...
  if (!build || !await assertRecordInScope(scope, 'gameBuilds', build, { owner: 'work' })) {
    throw new Error('[game-browser-performance-lab] Build 不存在或跨 Work')
  }
  if (!['preview-ready', 'release-ready', 'released'].includes(build.status)) {
    throw new Error('[game-browser-performance-lab] Build 尚未达到浏览器验收状态')
  }
  ...
  if (preview.packageHash !== build.packageHash || preview.previewHash !== build.previewHash) {
    throw new Error('[game-browser-performance-lab] Preview 与 Build hash 不一致')
  }
  const artifacts = await db.gameBuildArtifacts.where('buildId').equals(build.id!).toArray()
  for (const artifact of artifacts) {
    if (!await assertRecordInScope(scope, 'gameBuildArtifacts', artifact, { owner: 'work' })) {
      throw new Error('[game-browser-performance-lab] Build Artifact 跨 Work')
    }
  }
  ...
  const resolver = await createBuildGameMediaResolver({ scope, gameBuildId: build.id!, preview })
  ...
  if (catalog.failures.length) {
    resolver.dispose()
    throw new Error(`[game-browser-performance-lab] 媒资预加载失败:${catalog.failures.map(row => row.assetKey).join(',')}`)
  }
  ...
  try {
    ...
    const samples = await collectBrowserPerformanceSamplesV1({
      ...
      minimumLatencySamples: GAME_BROWSER_PERFORMANCE_POLICY_V1.minimumLatencySamples,
