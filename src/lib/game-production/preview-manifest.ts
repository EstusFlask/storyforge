import type { GameBuildPreviewManifestV1 } from '../types'
...
function fail(message: string): never {
  throw new Error(`[game-build-preview] ${message}`)
}
...
function previewHashBody(manifest: Omit<GameBuildPreviewManifestV1, 'previewHash'>) {
  return {
...
export function parseGameBuildPreviewManifestV1(value: string | unknown): GameBuildPreviewManifestV1 {
  ...
  if (manifest.schema !== 'storyforge.game-build-preview' || manifest.version !== 1) fail('schema/version 无效')
  ...
  if (canonicalGameProductionJsonV2(sortedBindings) !== canonicalGameProductionJsonV2(mediaBindings)) {
...
export async function createGameBuildPreviewManifestV1(input: {
  productionKey: string
  buildNumber: number
  buildManifestHash: string
  runtimePackage: GameBuildPreviewManifestV1['runtimePackage']
  mediaBindings?: GameBuildPreviewManifestV1['mediaBindings']
  fallbackSummary?: string[]
}): Promise<GameBuildPreviewManifestV1> {
  const runtimePackage = parseGameRuntimePackageV2(input.runtimePackage)
  const body: Omit<GameBuildPreviewManifestV1, 'previewHash'> = {
    schema: 'storyforge.game-build-preview',
    version: 1,
  ...
  return parseGameBuildPreviewManifestV1({
    ...body,
    previewHash: await hashGameProductionValueV2(previewHashBody(body)),
  })
}

export async function verifyGameBuildPreviewManifestV1(value: string | unknown): Promise<GameBuildPreviewManifestV1> {
  const manifest = parseGameBuildPreviewManifestV1(value)
  ...
  if (await hashGameProductionValueV2(previewHashBody(body)) !== manifest.previewHash) fail('previewHash 校验失败')
  return manifest
}
