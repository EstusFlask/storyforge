import type { GameProductionBriefV3, GameRuntimePackageV2 } from '../types'
import { validateTtrpgCampaignForPublicationV1 } from '../ttrpg/campaign'

export interface GameProductQualityGateV1 {
  gateId: string
  passed: boolean
  evidence: string[]
}

export interface GameProductQualityReportV1 {
  productType: GameRuntimePackageV2['productType']
  passed: boolean
  gates: GameProductQualityGateV1[]
  warnings: string[]
}

function gate(gateId: string, passed: boolean, evidence: string[]): GameProductQualityGateV1 {
 ...
 */
export function evaluateGameRuntimeProductQualityV1(input: {
  runtimePackage: GameRuntimePackageV2
  brief: GameProductionBriefV3
}): GameProductQualityReportV1 {
  ...
  const gates: GameProductQualityGateV1[] = [
  ...
  if (runtimePackage.productType === 'ttrpg') {
    ...
    gates.push(
      ...
      gate('product.ttrpg.counterexample-simulation', campaignValidation?.valid === true
  ...
  } else if (runtimePackage.productType === 'narrative-simulation') {
    const simulation = runtimePackage.simulation
    gates.push(
      gate('product.simulation.system', !!simulation && simulation.resources.length >= 1
        && simulation.metrics.length >= 2 && simulation.issues.length >= 1,
      [`resources=${simulation?.resources.length ?? 0}`, `metrics=${simulation?.metrics.length ?? 0}`, `issues=${simulation?.issues.length ?? 0}`]),
      gate('product.simulation.agency', !!simulation && simulation.actors.length >= 1
        && simulation.actions.length >= 2 && simulation.endings.length >= 1,
      [`actors=${simulation?.actors.length ?? 0}`, `actions=${simulation?.actions.length ?? 0}`, `endings=${simulation?.endings.length ?? 0}`]),
    )
  ...
  } else if (runtimePackage.productType === 'storygame') {
    gates.push(gate('product.storygame.branching', narrative.choices.some(choice => (
      narrative.choices.filter(candidate => candidate.sourceNodeKey === choice.sourceNodeKey).length >= 2
