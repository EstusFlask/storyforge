...
import type {
  ...
  GameProductionPlanV3,
  GameTaskBudgetReservationV1,
  WorkspaceScope,
} from '../types'
import { assertRecordInScope, resolveScope, scopeTransactionTables } from '../world-engine/scope'
import {
  acceptGameBuildArtifact,
  carryForwardGameBuildArtifactsAcrossBuildsV1,
  carryForwardGameBuildArtifactsToEpochV1,
} from './artifact-store'
import { parseGameBuildQualityReportV1 } from './adoption'
...
import { createGameProductionPlanV3, parseGameProductionPlanV3 } from './plan'
...
export interface GameProductionTaskArtifactV1 {
  artifactKey: string
  requirementKey?: string | null
  kind: GameBuildArtifactKindV1
  mediaKind?: GameBuildArtifactRecordV1['mediaKind']
  payload: unknown
  metadata?: unknown
  quality?: unknown
  rights?: unknown
...
export interface GameProductionTaskUsageV1 {
  modelCalls: number
  inputTokens: number
  outputTokens: number
  mediaCalls: number
...
export interface GameProductionTaskExecutionResultV1 {
  artifacts: GameProductionTaskArtifactV1[]
  passedGateIds: string[]
  usage: GameProductionTaskUsageV1
}

export interface GameProductionTaskExecutionInputV1 {
  ...
  contextText: string
  inputArtifacts: GameBuildArtifactRecordV1[]
  capabilityBindings: GameProductionCapabilityBindingV1[]
  signal: AbortSignal
}

export type GameProductionTaskExecutorV1 = (
  input: GameProductionTaskExecutionInputV1,
) => Promise<GameProductionTaskExecutionResultV1>

export type GameProductionSchedulerBoundaryV1 =
  | 'task.claimed'
  | 'provider.requested'
  | 'provider.responded'
...
interface LedgerTaskV1 {
  ...
  passedGateIds: string[]
  usage: GameProductionTaskUsageV1 | null
  errorCode: string | null
}

interface SchedulerLedgerV1 {
  schema: 'storyforge.game-production-budget-ledger'
  version: 1
...
interface ResumeCandidateV1 {
  schema: 'storyforge.game-production-task-candidate'
  version: 1
  taskKey: string
  attempt: number
  controlEpoch: number
  ...
  result: GameProductionTaskExecutionResultV1
}

export interface GameProductionTaskProjectionV1 {
  taskKey: string
  lane: GameProductionPlanTaskV3['lane']
...
export interface GameProductionSchedulerProjectionV1 {
  productionId: number
  buildId: number
  buildNumber: number
  buildStatus: string
  ...
  budget: {
    usage: GameProductionTaskUsageV1
    limits: GameProductionBriefV3['productionBudget']
  }
  tasks: GameProductionTaskProjectionV1[]
}
...
function ledgerRecord(value: unknown, label: string): Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error(`[game-production-scheduler] ${label} 必须是对象`)
  }
...
function exactLedgerKeys(row: Record<string, unknown>, keys: string[], label: string): void {
  ...
  if (Object.keys(row).some(key => !expected.has(key)) || keys.some(key => !(key in row))) {
    throw new Error(`[game-production-scheduler] ${label} 字段不精确`)
  }
}

function ledgerInteger(value: unknown, label: string, minimum = 0): number {
  if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < minimum) {
    throw new Error(`[game-production-scheduler] ${label} 整数无效`)
  }
  return value
}
...
function ledgerHash(value: unknown, label: string, allowEmpty = false): string {
  if (typeof value !== 'string' || (!isSha256Hash(value) && !(allowEmpty && value === ''))) {
    throw new Error(`[game-production-scheduler] ${label} hash 无效`)
  }
  return value
}

function parseLedgerUsage(value: unknown, label: string): GameProductionTaskUsageV1 {
  ...
  if (costUsd !== null && (typeof costUsd !== 'number' || !Number.isFinite(costUsd) || costUsd < 0)) {
    throw new Error(`[game-production-scheduler] ${label}.costUsd 无效`)
  }
  return {
...
function parseLedger(value: string): SchedulerLedgerV1 {
  ...
  try { candidate = JSON.parse(value) } catch { throw new Error('[game-production-scheduler] budget ledger JSON 损坏') }
  const row = ledgerRecord(candidate, 'budget ledger')
  exactLedgerKeys(row, ['schema', 'version', 'rootRunId', 'rootClaim', 'tasks'], 'budget ledger')
  if (row.schema !== 'storyforge.game-production-budget-ledger' || row.version !== 1
    || !row.tasks || typeof row.tasks !== 'object' || Array.isArray(row.tasks)) {
    throw new Error('[game-production-scheduler] budget ledger 基础字段无效')
  }
  ...
  if (row.rootClaim !== null) {
    ...
    if (typeof claim.owner !== 'string' || !claim.owner || claim.owner.length > 300) {
      throw new Error('[game-production-scheduler] rootClaim.owner 无效')
    }
  ...
  for (const [taskKey, rawTask] of Object.entries(row.tasks as Record<string, unknown>)) {
    if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,199}$/.test(taskKey)) {
      throw new Error('[game-production-scheduler] ledger taskKey 无效')
    }
    ...
    if (!['claimed', 'settled', 'failed'].includes(String(task.status))) {
      throw new Error(`[game-production-scheduler] tasks.${taskKey}.status 无效`)
    }
    if (!Array.isArray(task.passedGateIds) || task.passedGateIds.length > 100
      ...
      throw new Error(`[game-production-scheduler] tasks.${taskKey}.passedGateIds 无效`)
    }
    ...
    if (errorCode !== null && (typeof errorCode !== 'string' || !errorCode || errorCode.length > 300)) {
      throw new Error(`[game-production-scheduler] tasks.${taskKey}.errorCode 无效`)
    }
    tasks[taskKey] = {
...
export function assertGameProductionBudgetLedgerV1(value: string): void {
  parseLedger(value)
}

function zeroUsage(): GameProductionTaskUsageV1 {
...
function boundedUsage(usage: GameProductionTaskUsageV1, reservation: GameTaskBudgetReservationV1): void {
  ...
  if (integers.some(value => !Number.isInteger(value) || value < 0)
    ...
    throw new Error('[game-production-scheduler] task usage 超出 Plan 预算预留')
  }
}

function normalizedBindings(
  task: GameProductionPlanTaskV3,
  bindings: readonly GameProductionCapabilityBindingV1[],
): GameProductionCapabilityBindingV1[] {
  ...
  return task.capabilityRequirementKeys.map(requirementKey => {
    ...
    if (!binding || !isSha256Hash(binding.bindingHash) || !binding.adapterId.trim()) {
      throw new Error(`[game-production-scheduler] 缺少非密钥 capability binding:${requirementKey}`)
    }
    return { requirementKey, bindingHash: binding.bindingHash, adapterId: binding.adapterId.trim() }
  })
}

function artifactWriteTargets(task: GameProductionPlanTaskV3) {
  if (task.skillId) {
  ...
  return [{
    table: 'gameBuildArtifacts', fields: [], mode: 'candidate-only' as const,
    adoptionExtension: 'game-production-artifacts',
  }]
}

function taskContextSourceKeys(task: GameProductionPlanTaskV3): string[] {
  if (task.skillId) {
    const skill = getAgentSkillV1(task.skillId)
    return [
...
function rootContract(scope: WorkspaceScope, build: { id: number; buildNumber: number; controlEpoch: number; planHash: string }) {
  return {
    version: 1 as const,
    objective: `负责 GameBuild ${build.buildNumber} 的有界 DAG 所有权、预算与 terminal join`,
    ...
    permissions: {
      ...
      writeTargets: [{
        table: 'gameBuilds', fields: [], mode: 'candidate-only' as const,
        adoptionExtension: 'game-production-builds',
      }],
    },
    dependencyReceiptPolicy: { requiredForJoin: true as const, verifierSetVersion: 'game-production-root-v1' },
    budget: {
      maxModelCalls: 1, maxToolCalls: 0, maxInputTokens: 1, maxOutputTokens: 1,
      maxAttemptsPerStep: 1,
    },
    acceptance: [
      { id: 'game-production.children', kind: 'deterministic-check' as const, required: true },
      { id: 'game-production.package', kind: 'gate-passed' as const, required: true },
    ],
    verificationPlan: [{
      id: 'game-production.root-terminal', kind: 'terminal' as const,
...
function taskContract(input: {
  ...
  return {
    version: 1 as const,
    objective: `执行 GameBuild ${input.build.buildNumber} 的任务 ${input.task.taskKey}`,
    workflowKind: 'long-running-resumable' as const,
    ...
    dependencyReceiptPolicy: { requiredForJoin: true as const, verifierSetVersion: 'game-production-task-v1' },
    budget: {
    ...
    verificationPlan: [{
      ...
      verifier: 'game-production-task-v1',
...
async function currentProductionBuild(scope: WorkspaceScope, productionId: number) {
  ...
  if (!production || !await assertRecordInScope(scope, 'gameProductions', production, { owner: 'work' })
    ...
    throw new Error('[game-production-scheduler] Production/当前 Build 不存在')
  }
  ...
  if (!build || !briefRow || !await assertRecordInScope(scope, 'gameBuilds', build, { owner: 'work' })
    ...
    throw new Error('[game-production-scheduler] Build/Brief 不满足调度条件')
  }
  const brief = parseGameProductionBriefV3(briefRow.briefJson)
  if (await hashGameProductionValueV2(brief) !== briefRow.briefHash) {
    throw new Error('[game-production-scheduler] Brief hash 校验失败')
  }
...
async function applyCrossBuildEvolutionReuse(input: {
  ...
  plan: GameProductionPlanV3
}): Promise<{ plan: GameProductionPlanV3; reusableArtifactKeys: string[]; sourceBuildId: number | null }> {
  ...
  if (!parentBuild || parentBuild.id == null
    || !await assertRecordInScope(input.scope, 'gameBuilds', parentBuild, { owner: 'work' })) {
    throw new Error('[game-production-scheduler] 演化 parent Build 缺失或跨 Work')
  }
  if (evolution.base.kind === 'build' && evolution.base.buildNumber !== parentBuild.buildNumber) {
    throw new Error('[game-production-scheduler] 演化 impact 与 parent Build 不一致')
  }
  const parentBriefRow = await db.gameProductionBriefs
    .where('[productionId+revision]').equals([input.build.productionId, parentBuild.briefRevision]).first()
  if (!parentBriefRow) throw new Error('[game-production-scheduler] 演化 parent Brief 缺失')
  const parentBrief = parseGameProductionBriefV3(parentBriefRow.briefJson)
  const immutableEnvelope = (brief: GameProductionBriefV3) => ({
  ...
  if (canonicalGameProductionJsonV2(immutableEnvelope(parentBrief))
    !== canonicalGameProductionJsonV2(immutableEnvelope(input.brief))) {
    return { plan: input.plan, reusableArtifactKeys: [], sourceBuildId: parentBuild.id }
  }
  let parentPlan: GameProductionPlanV3
  try { parentPlan = parseGameProductionPlanV3(parentBuild.planJson, parentBrief, parentBriefRow.briefHash) } catch {
    return { plan: input.plan, reusableArtifactKeys: [], sourceBuildId: parentBuild.id }
  }
  const parentTasks = new Map(parentPlan.tasks.map(task => [task.taskKey, task]))
  const parentArtifacts = (await db.gameBuildArtifacts.where('buildId').equals(parentBuild.id).toArray())
  ...
  const tasks: GameProductionPlanV3['tasks'] = []
  ...
  for (const task of input.plan.tasks) {
    ...
    const requirementsUnchanged = parentTask && canonicalGameProductionJsonV2(
    ...
    const canReuse = task.executionMode !== 'deterministic' && lane != null && !affected.has(lane)
      ...
      && canonicalGameProductionJsonV2(parentTask.outputArtifactKeys) === canonicalGameProductionJsonV2(task.outputArtifactKeys)
    ...
    const reuseKey = await hashGameProductionValueV2({
      schema: 'storyforge.game-production-cross-build-reuse', version: 1,
  ...
  const plan = parseGameProductionPlanV3({ ...input.plan, tasks }, input.brief, input.plan.briefHash)
...
async function ensurePlan(input: {
  scope: WorkspaceScope
  productionId: number
  suppliedPlan?: GameProductionPlanV3
}) {
  ...
  if (['paused', 'cancelled', 'failed', 'archived', 'released'].includes(state.build.status)) {
    throw new Error(`[game-production-scheduler] Build 状态 ${state.build.status} 不允许调度`)
  }
  let currentPlan: GameProductionPlanV3 | null = null
  try {
    currentPlan = parseGameProductionPlanV3(state.build.planJson, state.brief, state.briefRow.briefHash)
  } catch { currentPlan = null }
  if (currentPlan && currentPlan.controlEpoch === state.build.controlEpoch && state.build.planHash === await hashGameProductionValueV2(currentPlan)) {
    return { ...state, plan: currentPlan }
  }
  let plan = input.suppliedPlan
    ? parseGameProductionPlanV3(input.suppliedPlan, state.brief, state.briefRow.briefHash)
    : await createGameProductionPlanV3({
        brief: state.brief,
  ...
  if (!input.suppliedPlan) {
    ...
    if (reuse.sourceBuildId != null && reuse.reusableArtifactKeys.length > 0) {
      await carryForwardGameBuildArtifactsAcrossBuildsV1({
  ...
  if (plan.controlEpoch !== state.build.controlEpoch || plan.buildNumber !== state.build.buildNumber) {
    throw new Error('[game-production-scheduler] Plan 与 Build epoch/number 不一致')
  }
  ...
  if (currentPlan && currentPlan.controlEpoch < plan.controlEpoch
    ...
    const reusableArtifactKeys = plan.tasks.flatMap(task => {
      ...
      return task.executionMode !== 'deterministic' && previous
        && canonicalGameProductionJsonV2(previous.outputArtifactKeys) === canonicalGameProductionJsonV2(task.outputArtifactKeys)
        ? task.outputArtifactKeys : []
    })
    if (reusableArtifactKeys.length > 0) await carryForwardGameBuildArtifactsToEpochV1({
  ...
  await db.transaction('rw', scopeTransactionTables(db.gameBuilds, db.gameBuildArtifacts), async () => {
    const build = await db.gameBuilds.get(state.build.id!)
    if (!build || build.controlEpoch !== state.build.controlEpoch || build.stateRevision !== state.build.stateRevision) {
      throw new Error('[game-production-scheduler] Plan CAS 已过期')
    }
    await db.gameBuildArtifacts.where('buildId').equals(build.id!).filter(row => (
    ...
    await db.gameBuilds.update(build.id!, {
      ...
      planJson: canonicalGameProductionJsonV2(plan), planHash,
      budgetLedgerJson: canonicalGameProductionJsonV2(emptyLedger()),
...
async function ensureRootRun(input: {
  ...
  if (ledger.rootRunId != null) {
    ...
    if (root.run.gameBuildId !== state.build.id
      || root.contract.scope.gameProduction?.taskKey !== ROOT_TASK_KEY
      || root.contract.scope.gameProduction.controlEpoch !== state.build.controlEpoch
      || root.contract.scope.gameProduction.planHash !== state.build.planHash) {
      throw new Error('[game-production-scheduler] root Run 与 Build 不一致')
    }
    return { ...state, root, ledger }
  }

  const claimOwner = `scheduler:${crypto.randomUUID()}`
  await db.transaction('rw', db.gameBuilds, async () => {
    const current = await db.gameBuilds.get(state.build.id!)
    if (!current || current.controlEpoch !== state.build.controlEpoch || current.planHash !== state.build.planHash) {
      throw new Error('[game-production-scheduler] root claim 已过期')
    }
    ...
    if (next.rootClaim && next.rootClaim.owner !== claimOwner && next.rootClaim.expiresAt > Date.now()) {
      throw new Error('[game-production-scheduler] root Run 正由其他调度器创建')
    }
    next.rootClaim = { owner: claimOwner, expiresAt: Date.now() + CLAIM_TTL_MS }
    await db.gameBuilds.update(current.id!, { budgetLedgerJson: canonicalGameProductionJsonV2(next), updatedAt: Date.now() })
  })
  const refreshed = await db.gameBuilds.get(state.build.id!)
  ...
  if (ledger.rootClaim?.owner !== claimOwner) throw new Error('[game-production-scheduler] root claim 丢失')
  let root = await createAgentRunV1({
    scope: input.scope, gameBuildId: state.build.id!,
  ...
  await db.transaction('rw', db.gameBuilds, async () => {
    const current = await db.gameBuilds.get(state.build.id!)
    if (!current) throw new Error('[game-production-scheduler] Build 在 root 提交前消失')
    const next = parseLedger(current.budgetLedgerJson)
    if (next.rootRunId != null && next.rootRunId !== root.run.id) {
      throw new Error('[game-production-scheduler] 检测到并发 root Run')
    }
    if (next.rootClaim?.owner !== claimOwner) throw new Error('[game-production-scheduler] root claim 已被替换')
...
async function childSnapshots(scope: WorkspaceScope, buildId: number, rootRunId: number) {
  const rows = await db.agentRuns.where('gameBuildId').equals(buildId).toArray()
  ...
  return new Map(snapshots.map(snapshot => [snapshot.contract.scope.gameProduction!.taskKey, snapshot]))
}
...
async function settleCarriedTask(input: {
  ...
  if (!receiptHash) throw new Error('[game-production-scheduler] carried task 缺少 terminal receipt')
  await db.transaction('rw', scopeTransactionTables(db.gameBuilds, db.gameBuildArtifacts), async () => {
    const build = await db.gameBuilds.get(input.buildId)
    if (!build || build.controlEpoch !== input.controlEpoch) {
      throw new Error('[game-production-scheduler] carried task settlement epoch 已过期')
    }
    ...
    if (artifacts.length !== input.task.outputArtifactKeys.length) {
      throw new Error(`[game-production-scheduler] carried task 输出不完整:${input.task.taskKey}`)
    }
    const now = Date.now()
    for (const artifact of artifacts) await db.gameBuildArtifacts.update(artifact.id!, {
    ...
    await db.gameBuilds.update(build.id!, {
      budgetLedgerJson: canonicalGameProductionJsonV2(ledger), updatedAt: now,
    })
  })
}
...
async function ensureCarriedForwardTaskRuns(input: {
  ...
  plan: GameProductionPlanV3
  capabilityBindings: GameProductionCapabilityBindingV1[]
  onDurableBoundary?: (boundary: GameProductionSchedulerBoundaryV1, snapshot: AgentRunSnapshotV1) => void | Promise<void>
}): Promise<void> {
  let progressed = true
  while (progressed) {
    progressed = false
    ...
    const artifacts = (await db.gameBuildArtifacts.where('buildId').equals(input.build.id).toArray())
    ...
    for (const task of input.plan.tasks) {
      ...
      const candidateHash = await hashGameProductionValueV2(outputs.map(row => ({
      ...
      const inputHash = await hashGameProductionValueV2({
        schema: 'storyforge.game-production-carried-task-input', version: 1,
      ...
      const capabilityBindingHash = bindings.length > 0 ? await hashGameProductionValueV2(bindings) : undefined
      let snapshot: AgentRunSnapshotV1
      try {
        snapshot = await createAgentRunV1({
          scope: input.scope, gameBuildId: input.build.id,
      ...
      snapshot = await append(input.scope, snapshot, 'verification.started', {
        verifierSetVersion: 'game-production-carried-task-v1',
      })
      const receiptHash = await hashGameProductionValueV2({
        schema: 'storyforge.game-production-carried-task-receipt', version: 1,
...
async function acceptedInputs(buildId: number, controlEpoch: number, keys: string[]) {
  ...
  const rows = (await db.gameBuildArtifacts.where('buildId').equals(buildId).toArray())
  ...
  if (rows.length !== wanted.size || new Set(rows.map(row => row.artifactKey)).size !== wanted.size) {
    throw new Error('[game-production-scheduler] task 输入 Artifact 缺失、重复或 epoch 过期')
  }
  return rows.sort((a, b) => a.artifactKey.localeCompare(b.artifactKey))
}

function validateExecutionResult(task: GameProductionPlanTaskV3, result: GameProductionTaskExecutionResultV1): void {
  if (!result || !Array.isArray(result.artifacts) || !Array.isArray(result.passedGateIds)) {
    throw new Error('[game-production-scheduler] executor 返回合同无效')
  }
  ...
  if (expected.length !== actual.length || expected.some((key, index) => key !== actual[index])
    ...
    throw new Error(`[game-production-scheduler] ${task.taskKey} 输出或 acceptance gates 不完整`)
  }
  boundedUsage(result.usage, task.budgetReservation)
}

function parseResumeCandidate(value: unknown, task: GameProductionPlanTaskV3, epoch: number): ResumeCandidateV1 {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('[game-production-scheduler] checkpoint candidate 缺失')
  const candidate = value as ResumeCandidateV1
  if (candidate.schema !== 'storyforge.game-production-task-candidate' || candidate.version !== 1
    ...
    throw new Error('[game-production-scheduler] checkpoint candidate 无效或过期')
  }
...
async function settleLedger(input: {
  ...
  await db.transaction('rw', db.gameBuilds, async () => {
    const build = await db.gameBuilds.get(input.buildId)
    if (!build || build.controlEpoch !== input.controlEpoch) throw new Error('[game-production-scheduler] ledger epoch 已过期')
    const ledger = parseLedger(build.budgetLedgerJson)
    ledger.tasks[input.taskKey] = input.entry
    await db.gameBuilds.update(build.id!, { budgetLedgerJson: canonicalGameProductionJsonV2(ledger), updatedAt: Date.now() })
  })
}
...
async function acceptCandidate(input: {
  ...
  const receiptHash = input.snapshot.projection.terminalReceiptHash
  if (!receiptHash) throw new Error('[game-production-scheduler] task Run 尚无 terminal receipt')
...
async function finishCandidateRun(input: {
  ...
  snapshot = await append(input.scope, snapshot, 'verification.started', { verifierSetVersion: 'game-production-task-v1' })
  const receiptHash = await hashGameProductionValueV2({
    schema: 'storyforge.game-production-task-receipt', version: 1,
...
async function runClaimedTask(input: {
  ...
  task: GameProductionPlanTaskV3
  snapshot: AgentRunSnapshotV1
  executor: GameProductionTaskExecutorV1
  capabilityBindings: GameProductionCapabilityBindingV1[]
  signal: AbortSignal
  onDurableBoundary?: (boundary: GameProductionSchedulerBoundaryV1, snapshot: AgentRunSnapshotV1) => void | Promise<void>
}): Promise<void> {
  ...
  const bindingHash = snapshot.contract.runtimeBindingHash
    ?? await hashGameProductionValueV2(snapshot.contract.executionBindings ?? { deterministic: input.task.kind })
  ...
  let result: GameProductionTaskExecutionResultV1
  try {
    ...
    if (code === 'task-aborted' || code === 'provider-safety-refusal' || attempt >= input.task.maxAttempts) {
      ...
      await db.transaction('rw', scopeTransactionTables(db.gameBuilds, db.gameProductions), async () => {
        const build = await db.gameBuilds.get(input.build.id)
        ...
        await db.gameBuilds.update(input.build.id, {
          status,
          failureJson: canonicalGameProductionJsonV2({
            taskKey: input.task.taskKey, code, attempt, detail: safeExecutorError(error),
          }),
          updatedAt,
        })
        if (status === 'failed') await db.gameProductions.update(input.productionId, {
  ...
  const candidateHash = await hashGameProductionValueV2(result)
  if (input.task.executionMode === 'model') {
  ...
  const resumePayload: ResumeCandidateV1 = {
    schema: 'storyforge.game-production-task-candidate', version: 1,
...
async function compileTerminalBuild(input: {
  ...
  if (!build || !production || build.controlEpoch !== input.plan.controlEpoch) {
    throw new Error('[game-production-scheduler] terminal join Build 已过期')
  }
  const artifacts = (await db.gameBuildArtifacts.where('buildId').equals(build.id!).toArray())
  ...
  if (new Set(artifacts.map(row => row.artifactKey)).size !== artifacts.length) {
    throw new Error('[game-production-scheduler] terminal Artifact key 不唯一')
  }
  ...
  if (!packageArtifact || !qualityArtifact) throw new Error('[game-production-scheduler] terminal package/quality Artifact 缺失')
  const runtimePackage = parseGameRuntimePackageV2(packageArtifact.payloadJson)
  const packageHash = await hashGameProductionValueV2(runtimePackage)
  if (packageArtifact.contentHash !== packageHash) throw new Error('[game-production-scheduler] package Artifact hash 不一致')
  ...
  if (build.parentBuildNumber != null) {
    ...
    if (!parentBuild?.id || !parentBuild.packageHash) {
      throw new Error('[game-production-scheduler] compatibility parent Build 缺失')
    }
    const parentArtifact = await db.gameBuildArtifacts
    ...
    if (!parentArtifact || !['accepted', 'carried-forward'].includes(parentArtifact.status)) {
      throw new Error('[game-production-scheduler] compatibility parent package Artifact 缺失')
    }
    ...
    if (await hashGameProductionValueV2(parentRuntimePackage) !== parentBuild.packageHash) {
      throw new Error('[game-production-scheduler] compatibility parent package hash 不一致')
    }
    previousPackage = {
  ...
  const quality = parseGameBuildQualityReportV1(qualityArtifact.payloadJson)
  const qualityReportHash = await hashGameProductionValueV2(quality)
  if (qualityArtifact.contentHash !== qualityReportHash || quality.packageHash !== packageHash) {
    throw new Error('[game-production-scheduler] quality Artifact/package hash 不一致')
  }
  const ledger = parseLedger(build.budgetLedgerJson)
  ...
  const taskReceipts = input.plan.tasks.map(task => {
    ...
    if (!child?.projection.terminalReceiptHash || entry?.status !== 'settled'
      ...
      throw new Error(`[game-production-scheduler] terminal child receipt 未结算:${task.taskKey}`)
    }
  ...
  const manifest = {
    schema: 'storyforge.game-build-manifest' as const, version: 1 as const,
  ...
  const mediaBindings = (runtimePackage.presentation?.assets ?? []).map(asset => {
    ...
    if (!artifact || artifact.mimeType !== asset.mimeType || artifact.byteSize !== asset.byteSize) {
      throw new Error(`[game-production-scheduler] RuntimePackage 媒资未绑定 accepted Artifact:${asset.assetKey}`)
    }
  ...
  if (root.projection.state !== 'completed') {
    root = await append(input.scope, root, 'step.succeeded', {
      ...
      outputHash: await hashGameProductionValueV2({ manifestHash, taskReceipts }),
    })
    root = await append(input.scope, root, 'verification.started', { verifierSetVersion: 'game-production-root-v1' })
    ...
    throw new Error('[game-production-scheduler] root Run terminal receipt 与 Build join 不一致')
  }
  await db.transaction('rw', scopeTransactionTables(db.gameBuilds, db.gameProductions, db.gameBuildArtifacts), async () => {
    const current = await db.gameBuilds.get(build.id!)
    const currentProduction = await db.gameProductions.get(production.id!)
    if (!current || !currentProduction || current.controlEpoch !== build.controlEpoch || current.planHash !== build.planHash) {
      throw new Error('[game-production-scheduler] terminal commit CAS 已过期')
    }
    const currentArtifacts = (await db.gameBuildArtifacts.where('buildId').equals(build.id!).toArray())
      .filter(row => row.controlEpoch === build.controlEpoch && (row.status === 'accepted' || row.status === 'carried-forward'))
    if (currentArtifacts.length !== artifacts.length) throw new Error('[game-production-scheduler] terminal commit Artifact 集合变化')
    ...
    await db.gameBuilds.update(build.id!, {
      ...
      manifestJson: canonicalGameProductionJsonV2(manifest), manifestHash, packageHash,
      previewManifestJson: canonicalGameProductionJsonV2(preview), previewHash: preview.previewHash,
      qualityReportJson: canonicalGameProductionJsonV2(quality), qualityReportHash,
...
export async function projectGameProductionSchedulerV1(input: {
  scope: WorkspaceScope
  productionId: number
  suppliedPlan?: GameProductionPlanV3
  ...
  let plan: GameProductionPlanV3 | null = null
  try {
    plan = input.suppliedPlan
      ? parseGameProductionPlanV3(input.suppliedPlan, current.brief, current.briefRow.briefHash)
      : parseGameProductionPlanV3(current.build.planJson, current.brief, current.briefRow.briefHash)
  } catch { plan = null }
  ...
  const tasks = (plan?.tasks ?? []).map(task => {
    ...
    let status: GameProductionTaskProjectionV1['status'] = dependenciesReady ? 'ready' : 'waiting'
    let blocker: string | null = null
    if (child) {
      const step = child.projection.steps[task.taskKey]
      if (child.contract.scope.gameProduction?.controlEpoch !== build.controlEpoch) status = 'stale'
  ...
  const usage: GameProductionTaskUsageV1 = {
...
function costBearing(task: GameProductionPlanTaskV3): boolean {
  return task.budgetReservation.modelCalls > 0 || task.budgetReservation.mediaCalls > 0
    || (task.budgetReservation.maximumCostUsd ?? 0) > 0
}

export async function runGameProductionSchedulerCycleV1(input: {
  scope: WorkspaceScope
  productionId: number
  executor: GameProductionTaskExecutorV1
  suppliedPlan?: GameProductionPlanV3
  ...
  const selected: GameProductionPlanTaskV3[] = []
  ...
  const claimed: Array<{ task: GameProductionPlanTaskV3; snapshot: AgentRunSnapshotV1 }> = []
  for (const task of selected) {
    ...
    try {
      const snapshot = await createAgentRunV1({
        scope, gameBuildId: state.build.id!,
        contract: taskContract({
...
export async function runGameProductionUntilBlockedV1(input: {
  scope: WorkspaceScope
  productionId: number
  executor: GameProductionTaskExecutorV1
  suppliedPlan?: GameProductionPlanV3
  ...
  let projection!: GameProductionSchedulerProjectionV1
  for (let cycle = 0; cycle < maximumCycles; cycle++) {
    projection = await runGameProductionSchedulerCycleV1(input)
    ...
    const signature = canonicalGameProductionJsonV2(projection.tasks.map(task => ({
  ...
  throw new Error('[game-production-scheduler] 超出最大调度 cycle，拒绝隐藏循环')
}
