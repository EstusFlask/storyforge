...
export async function resolveConfiguredTextCapabilityV1(input: {
  ...
  const capabilityHash = await hashGameProductionValueV2({
    requirementKey: input.requirementKey,
    ...identity,
  })
  if (input.expectedCapabilityHash && capabilityHash !== input.expectedCapabilityHash) {
    throw new Error('[game-production-capability] 文本 provider binding 与授权 capability 不一致')
  }
  const body = {
  ...
  const receipt = { ...body, receiptHash: await hashGameProductionValueV2(body) }
  if (/api[-_]?key|authorization|bearer/i.test(JSON.stringify(receipt))) {
    throw new Error('[game-production-capability] provider receipt 含敏感字段')
  }
...
export async function runConfiguredProductionTextV1(input: {
  ...
  if (!Number.isInteger(input.maximumOutputTokens) || input.maximumOutputTokens < 1) {
    throw new Error('[game-production-capability] maximumOutputTokens 无效')
  }
