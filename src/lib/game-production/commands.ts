...
export interface GameProductionCommandReceiptV1 {
  ok: boolean
  commandId: string
  commandType: GameProductionCommandV1['type']
  productionId: number
  stateRevision: number
  result: Record<string, unknown>
  errorCode: GameProductionErrorCodeV1 | null
  replayed: boolean
}

class ExpectedCommandFailure extends Error {
  constructor(readonly code: GameProductionErrorCodeV1, message: string) {
    super(message)
  }
}

function reject(code: GameProductionErrorCodeV1, message: string): never {
  throw new ExpectedCommandFailure(code, message)
}

function safeJson(value: unknown): string {
  const json = canonicalGameProductionJsonV2(value)
  if (json.length > 100_000) throw new Error('[game-production] command result 超出安全上限')
  return json
}
...
async function productionInScope(scope: WorkspaceScope, productionId: number): Promise<GameProductionRecordV1 & { id: number }> {
  ...
  return production as GameProductionRecordV1 & { id: number }
}

async function currentBuild(production: GameProductionRecordV1 & { id: number }): Promise<GameBuildRecordV1 & { id: number }> {
  if (production.currentBuildNumber == null) reject('invalid-state-transition', 'Production 尚无当前 Build')
  const build = await db.gameBuilds
  ...
  return build as GameBuildRecordV1 & { id: number }
}

async function assertWorldRelease(scope: WorkspaceScope, worldReleaseId: number, expectedHash?: string) {
  const release = await db.worldReleases.get(worldReleaseId)
  if (!release || !await assertRecordInScope(scope, 'worldReleases', release, { owner: 'world' })) {
    reject('source-stale', 'WorldRelease 不存在或跨 World')
  }
  if (expectedHash && release.contentHash !== expectedHash) reject('source-stale', 'WorldRelease content hash 已变化')
  return release
}

async function nextBuildNumber(productionId: number): Promise<number> {
  const rows = await db.gameBuilds.where('productionId').equals(productionId).toArray()
  return Math.max(0, ...rows.map(row => row.buildNumber)) + 1
}

async function nextBriefRevision(productionId: number): Promise<number> {
  const rows = await db.gameProductionBriefs.where('productionId').equals(productionId).toArray()
...
async function applyCommand(input: {
  ...
  if (command.type === 'create-intent') {
    return { production, result: { status: production.status, created: production.createdAt === now } }
  }
  ...
  if (command.type === 'save-brief-revision') {
    ...
    await db.gameProductions.update(production.id, { currentBriefRevision: revision, status, stateRevision, updatedAt: now })
  ...
  if (command.type === 'authorize-start') {
    ...
    const build = stampNewRecord(scope, 'gameBuilds', {
      ...
      parentBuildNumber: production.currentBuildNumber, sourceGameReleaseId: production.currentGameReleaseId,
  ...
  if (command.type === 'pause') {
    ...
    await db.gameProductions.update(production.id, { status: 'paused', controlEpoch, stateRevision, updatedAt: now })
  ...
  if (command.type === 'resume') {
    ...
    await db.gameProductions.update(production.id, { status: productionStatus, controlEpoch, stateRevision, updatedAt: now })
  ...
  if (command.type === 'stop') {
    ...
    if (command.retention === 'discard-unreleased') {
      await db.gameBuildArtifacts.where('buildId').equals(build.id).filter(row => row.status !== 'orphaned').modify({ status: 'orphaned', updatedAt: now })
    }
    await db.gameProductions.update(production.id, { status: 'stopped', controlEpoch, stateRevision, updatedAt: now })
  ...
  if (command.type === 'restore') {
    ...
    const allowedStatuses: GameProductionRecordV1['status'][] = [
    ...
    if (archiveState.code !== 'user-archived'
      ...
      || !allowedStatuses.includes(archiveState.previousStatus as GameProductionRecordV1['status'])
    ...
    const restoredStatus = archiveState.previousStatus as GameProductionRecordV1['status']
    const controlEpoch = production.controlEpoch + 1
    let restoredBuildStatus: GameBuildRecordV1['status'] | null = null
    if (production.currentBuildNumber != null) {
      const build = await currentBuild(production)
  ...
  if (command.type === 'request-preview') {
    const build = await db.gameBuilds.where('[productionId+buildNumber]').equals([production.id, command.buildNumber]).first()
  ...
  if (command.type === 'publish') {
    reject('publication-transaction-failed', '发布必须由 game-production/adoption.ts 原子事务入口执行')
  }
  ...
  if (command.base.kind === 'build') {
    const base = await db.gameBuilds.where('[productionId+buildNumber]').equals([production.id, command.base.buildNumber]).first()
    ...
    const release = await db.gameReleases.get(command.base.gameReleaseId)
  ...
  const previous = await db.gameProductionBriefs
  ...
  const priorBrief = parseGameProductionBriefV3(previous.briefJson)
  ...
  const baseRef = command.base.kind === 'build'
    ...
    : `game-release:${command.base.gameReleaseId}:${command.base.contentHash}`
  const nextBrief = parseGameProductionBriefV3({
    ...priorBrief,
  ...
  const revision = await nextBriefRevision(production.id)
  const briefHash = await Dexie.waitFor(hashGameProductionValueV2(nextBrief))
  await db.gameProductionBriefs.add(stampNewRecord(scope, 'gameProductionBriefs', {
    projectId: scope.projectId, worldId: scope.worldId, workId: scope.workId,
  ...
  const stateRevision = production.stateRevision + 1
  await db.gameProductions.update(production.id, { status: 'brief-ready', currentBriefRevision: revision, stateRevision, updatedAt: now })
...
async function executeTransaction(input: {
  ...
  return db.transaction('rw', scopeTransactionTables(
    ...
    let production: GameProductionRecordV1 & { id: number }
    if (command.type === 'create-intent') {
      await assertWorldRelease(scope, command.worldReleaseId)
      const existing = await db.gameProductions.where('[workId+productionKey]').equals([scope.workId, command.productionKey]).first()
      if (existing) {
        if (!await assertRecordInScope(scope, 'gameProductions', existing, { owner: 'work' })) reject('production-state-conflict', 'productionKey 跨 Work 冲突')
        production = existing as GameProductionRecordV1 & { id: number }
      } else {
        const row = stampNewRecord(scope, 'gameProductions', {
          projectId: scope.projectId, worldId: scope.worldId, workId: scope.workId,
    ...
    const existingCommand = await db.gameProductionCommands
    ...
    if (existingCommand) {
      ...
      return {
        ...
        errorCode: existingCommand.errorCode as GameProductionErrorCodeV1 | null, replayed: true,
      }
    }
...
function notifyProductionChanged(receipt: GameProductionCommandReceiptV1): void {
  if (typeof BroadcastChannel === 'undefined') return
  const channel = new BroadcastChannel('storyforge-game-production')
...
export async function executeGameProductionCommand(input: {
  scope: WorkspaceScope
  productionId?: number
  command: GameProductionCommandV1 | unknown
  now?: number
}): Promise<GameProductionCommandReceiptV1> {
