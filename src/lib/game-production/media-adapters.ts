...
export interface GameMediaProviderCapabilityV1 {
  adapterId: string
  version: 1
  mediaClasses: GameMediaClassV1[]
...
export interface GameMediaRequestV1 {
...
export interface GameMediaEstimateV1 {
  requestId: string
  outputCount: number
...
export interface GameMediaCandidateV1 {
  schema: 'storyforge.game-media-candidate'
  version: 1
  adapterId: string
  requestId: string
...
export interface GameMediaProviderAdapterV1 {
...
function fail(message: string): never {
  throw new Error(`[game-media-adapter] ${message}`)
}
...
function assertRequest(request: GameMediaRequestV1, capability: GameMediaProviderCapabilityV1): void {
  if (request.schema !== 'storyforge.game-media-request' || request.version !== 1
...
export function detectGameMediaMimeTypeV1(data: ArrayBuffer): string | null {
...
export function detectGameImageDimensionsV1(data: ArrayBuffer): { width: number; height: number } | null {
  const bytes = new Uint8Array(data)
...
async function candidate(input: {
  ...
  return {
    schema: 'storyforge.game-media-candidate', version: 1,
...
async function verifyCandidate(value: unknown, adapterId: string): Promise<GameMediaCandidateV1> {
  const item = row(value, 'candidate') as unknown as GameMediaCandidateV1
  if (item.schema !== 'storyforge.game-media-candidate' || item.version !== 1
    ...
    || detectGameMediaMimeTypeV1(item.data) !== item.mimeType) fail('候选验证失败')
  return structuredClone(item)
}

function estimate(request: GameMediaRequestV1, bytesPerOutput: number, durationMs: number): GameMediaEstimateV1 {
  return {
...
const OPENAI_IMAGE_CAPABILITY: GameMediaProviderCapabilityV1 = {
...
const AGNES_IMAGE_CAPABILITY: GameMediaProviderCapabilityV1 = {
 ...
 */
export const agnesImage21FlashAdapterV1: GameMediaProviderAdapterV1 = {
...
export const openAIGptImage2AdapterV1: GameMediaProviderAdapterV1 = {
...
function elevenAdapter(input: {
  ...
  endpoint: string
}): GameMediaProviderAdapterV1 {
  const capability: GameMediaProviderCapabilityV1 = {
...
const PROCEDURAL_CAPABILITY: GameMediaProviderCapabilityV1 = {
...
export const proceduralAudioAdapterV1: GameMediaProviderAdapterV1 = {
...
const UNIMPLEMENTED_CAPABILITIES: GameMediaProviderCapabilityV1[] = [
  {
...
const IMPLEMENTED_ADAPTERS = new Map<string, GameMediaProviderAdapterV1>([
...
export function listGameMediaProviderCapabilitiesV1(): GameMediaProviderCapabilityV1[] {
...
export function resolveGameMediaProviderAdapterV1(adapterId: string): GameMediaProviderAdapterV1 {
