...
import { detectGameImageDimensionsV1, type GameMediaClassV1, type GameMediaRequestV1 } from './media-adapters'
import { ensureGeneratedCharacterAlphaV1 } from './character-alpha-matting'
import type { ResolvedGameMediaCapabilityV1 } from './media-transport'
import { parseGameRuntimePackageV2 } from './runtime-package'
import { evaluateGameRuntimeProductQualityV1 } from './product-quality'
...
import type {
  ...
  GameProductionTaskArtifactV1,
  GameProductionTaskExecutionInputV1,
  GameProductionTaskExecutionResultV1,
  GameProductionTaskExecutorV1,
  GameProductionTaskUsageV1,
} from './scheduler'
...
type ProductionExecutorOptionsV1 = {
  ...
  mediaCapabilities: ReadonlyMap<string, ResolvedGameMediaCapabilityV1>
}
...
function fail(message: string): never {
  throw new Error(`[game-production-executor] ${message}`)
}
...
export function parseProductionModelJsonObjectV1(output: string, label: string): JsonRecord {
  ...
  try {
    ...
    if (cause instanceof Error && cause.message.startsWith('[game-production-executor]')) throw cause
  }
...
function parseDesign(value: unknown, brief: GameProductionBriefV3): GameDesignArtifactV1 {
  ...
  if (row.schema !== 'storyforge.game-design-artifact' || row.version !== 1) fail('design schema/version 无效')
  ...
  return {
    schema: 'storyforge.game-design-artifact', version: 1,
...
function jsonValue(value: unknown, label: string, expected: 'object' | 'array'): string {
  ...
  return canonicalGameProductionJsonV2(value)
}

function parseNarrative(value: unknown, brief: GameProductionBriefV3): NarrativeArtifactV1 {
  ...
  if (row.schema !== 'storyforge.game-narrative-artifact' || row.version !== 1
  ...
  const nodes: FrozenGameNarrativeNode[] = nodeRows.filter(node => reachable.has(node.key)).map(node => {
  ...
  return {
    schema: 'storyforge.game-narrative-artifact', version: 1,
...
function parseAcceptedNarrative(value: unknown, brief: GameProductionBriefV3): NarrativeArtifactV1 {
  ...
  if (canonicalGameProductionJsonV2(storedSuccessors) !== canonicalGameProductionJsonV2(
    parsed.nodes.map(node => ({ key: node.key, successorKeys: node.successorKeys })),
  )) fail('acceptedNarrative successorKeys 与 Choice 图不一致')
  return parsed
}

function parseProductModule(value: unknown, brief: GameProductionBriefV3): ProductModuleArtifactV1 {
  const row = record(value, 'productModule')
  exactKeys(row, [
    'schema', 'version', 'productType', 'interfaceStyle', 'interactionNotes', 'presentationPolicy',
  ], 'productModule')
  if (row.schema !== 'storyforge.game-product-module-artifact' || row.version !== 1) {
...
function expectedVisualKeys(brief: GameProductionBriefV3): string[] {
...
function expectedAudioKeys(brief: GameProductionBriefV3): string[] {
...
export function parseGameMediaRequirementsArtifactV2(
  value: unknown,
  brief: GameProductionBriefV3,
  ...
  if (row.schema !== 'storyforge.game-media-requirements-artifact' || row.version !== 2
  ...
  return { schema: 'storyforge.game-media-requirements-artifact', version: 2, visual, audio }
}

function artifactPayload(input: GameProductionTaskExecutionInputV1, artifactKey: string): unknown {
...
function zeroUsage(durationMs: number): GameProductionTaskUsageV1 {
  return { modelCalls: 0, inputTokens: 0, outputTokens: 0, mediaCalls: 0, costUsd: 0, durationMs, storageBytes: 0 }
}

function textSystem(taskKey: string, brief: GameProductionBriefV3): string {
  const common = `你是 StoryForge 已登记的游戏生产执行器。任务=${taskKey}。\n` +
  ...
  if (taskKey === 'content.design') return `${common}\n输出字段必须精确为：` +
    '{"schema":"storyforge.game-design-artifact","version":1,"title":"...","logline":"...","playerGoal":"...","coreLoop":["..."],"sourceAnchors":["..."],"invariants":["..."],"tone":["..."],"targetPlayMinutes":1,"targetEndingCount":1}。' +
  ...
  if (taskKey === 'content.narrative') {
    ...
    '{"schema":"storyforge.game-narrative-artifact","version":1,"moduleKind":"main|side|quest|opening|free","moduleTitle":"...","entryNodeKey":"...","nodes":[{"key":"...","kind":"entry|scene|choice|ending","title":"...","summary":"...","condition":{},"effects":[]}],"beats":[{"beatKey":"...","nodeKey":"...","kind":"narration|dialogue|action|system","speakerKey":null,"text":"...","order":0}],"choices":[{"choiceKey":"...","sourceNodeKey":"...","text":"...","description":"","unavailableReason":"","targetNodeKey":"...","displayCondition":{},"availableCondition":{},"effects":[],"tags":[],"order":0}]}。' +
  ...
  if (taskKey === 'content.product-module') return `${common}\n输出字段必须精确为：` +
    '{"schema":"storyforge.game-product-module-artifact","version":1,"productType":"storygame|character-interaction|text-adventure|avg|narrative-simulation|text-open-world|ttrpg","interfaceStyle":"...","interactionNotes":["..."],"presentationPolicy":{"pacing":"slow|balanced|fast","transitionMs":500,"backgroundStrategy":"none|key-scenes"}}。' +
...
async function executeModelTask(input: GameProductionTaskExecutionInputV1, options: {
  brief: GameProductionBriefV3
  category: string
  runText: ProductionTextRunnerV1
  ...
  let payload: unknown
  let kind: GameProductionTaskArtifactV1['kind']
  let quality: unknown
  if (input.task.taskKey === 'content.design') {
    payload = parseDesign(raw, options.brief); kind = 'game-design'
    ...
    payload = parseGameMediaRequirementsArtifactV2(raw, options.brief); kind = 'asset-manifest'
...
function runtimeEnvironment(): GameMediaRequestV1['environment'] {
...
function resolvedMediaCapability(
  ...
  if (!resolved.adapter.capability.mediaClasses.includes(requirement.mediaClass as GameMediaClassV1)) {
...
async function generateProviderMedia(input: {
  execution: GameProductionTaskExecutionInputV1
  options: MediaExecutorOptionsV1
  requirement: ProviderCapabilityRequirementV1
  artifactKey: string
  mediaClass: GameMediaClassV1
  ...
  candidate: Awaited<ReturnType<ResolvedGameMediaCapabilityV1['adapter']['parseAndVerify']>>
  ...
  const request: GameMediaRequestV1 = {
    schema: 'storyforge.game-media-request', version: 1, requestId,
  ...
  return {
    candidate,
    request,
    receiptHash: await hashGameProductionValueV2({
      ...
      usageHash: await hashGameProductionValueV2(candidate.providerReceipt.usage),
      costUsd: candidate.providerReceipt.costUsd,
    }),
  }
}

async function executeVisualTask(input: GameProductionTaskExecutionInputV1, options: {
  production: GameProductionRecordV1
  brief: GameProductionBriefV3
  mediaCapabilities: ReadonlyMap<string, ResolvedGameMediaCapabilityV1>
}): Promise<GameProductionTaskExecutionResultV1> {
  const startedAt = performance.now()
  const requirements = parseGameMediaRequirementsArtifactV2(artifactPayload(input, 'media.requirements'), options.brief)
  ...
  const artifacts: GameProductionTaskArtifactV1[] = []
  let storageBytes = 0
  let totalCostUsd = 0
  let costKnown = true
  ...
  for (const [index, artifactKey] of input.task.outputArtifactKeys.entries()) {
    ...
    const anchorRulesHash = await hashGameProductionValueV2({
    ...
    storageBytes += blob.byteSize
    const dimensions = detectGameImageDimensionsV1(candidateData)
...
async function executeAudioTask(input: GameProductionTaskExecutionInputV1, options: {
  production: GameProductionRecordV1
  brief: GameProductionBriefV3
  mediaCapabilities: ReadonlyMap<string, ResolvedGameMediaCapabilityV1>
}): Promise<GameProductionTaskExecutionResultV1> {
  ...
  for (const [index, artifactKey] of input.task.outputArtifactKeys.entries()) {
    ...
    const mediaClass: GameMediaClassV1 = requirement.mediaKind === 'bgm' ? 'music' : 'sfx'
...
function mediaAsset(row: GameProductionTaskExecutionInputV1['inputArtifacts'][number]) {
...
async function executeIntegrationTask(input: GameProductionTaskExecutionInputV1, options: {
  production: GameProductionRecordV1
  brief: GameProductionBriefV3
}): Promise<GameProductionTaskExecutionResultV1> {
  const startedAt = performance.now()
  ...
  let ttrpg: NonNullable<GameRuntimePackageV2['ttrpg']> | undefined
  ...
  const runtimePackage: GameRuntimePackageV2 = {
    schema: 'storyforge.game-runtime-package', version: 2, productType: options.brief.intent.productType,
    definition: {
      gameKey: options.production.productionKey, title: options.production.title,
  ...
  if (options.brief.intent.productType === 'avg' || options.brief.intent.productType === 'ttrpg') {
    ...
    const cues: NonNullable<GameRuntimePackageV2['presentation']>['cues'] = []
  ...
  const ttrpgArtifacts: GameProductionTaskArtifactV1[] = parsed.ttrpg ? [
    {
  ...
  return {
    artifacts: [...ttrpgArtifacts, {
      ...
      quality: { parser: 'parseGameRuntimePackageV2', graphValidated: true },
...
async function executeQualityTask(input: GameProductionTaskExecutionInputV1, options: {
  brief: GameProductionBriefV3
}): Promise<GameProductionTaskExecutionResultV1> {
  const startedAt = performance.now()
  const runtimePackage = parseGameRuntimePackageV2(artifactPayload(input, 'runtime.package'))
  if (runtimePackage.productType === 'ttrpg') {
    ...
    const ruleHash = await hashGameProductionValueV2(rulePack)
    if (ruleHash !== runtimePackage.ttrpg.rulePack.contentHash
      || canonicalGameProductionJsonV2(rulePack) !== canonicalGameProductionJsonV2(runtimePackage.ttrpg.rulePack.content)) {
      fail('独立 RulePack Artifact 与 RuntimePackage 不一致')
    }
    if (canonicalGameProductionJsonV2(campaign) !== canonicalGameProductionJsonV2(runtimePackage.ttrpg.campaign)) {
  ...
  const productQuality = evaluateGameRuntimeProductQualityV1({ runtimePackage, brief: options.brief })
  ...
  const quality: GameBuildQualityReportV1 = {
    schema: 'storyforge.game-build-quality-report', version: 1, buildNumber: input.buildNumber,
...
}): Promise<GameProductionCapabilityBindingV1> {
  return {
    requirementKey: input.requirementKey, adapterId: input.adapterId,
    bindingHash: await hashGameProductionValueV2({
...
export function createConfiguredGameProductionExecutorV1(input: {
  production: GameProductionRecordV1
  brief: GameProductionBriefV3
  category?: string
  runText?: ProductionTextRunnerV1
  ...
  const supportedProducts = new Set<GameProductType>([
    'storygame', 'character-interaction', 'text-adventure', 'avg',
    'narrative-simulation', 'text-open-world', 'ttrpg',
  ])
  ...
  const options = {
    ...
    category: input.category ?? 'game-production', runText: input.runText ?? defaultTextRunner,
    mediaCapabilities: input.mediaCapabilities ?? new Map<string, ResolvedGameMediaCapabilityV1>(),
  }
