...
function stableKey(value: string, label: string): string {
  ...
  if (!STABLE_KEY.test(normalized)) throw new Error(`[game-production-artifact] ${label} 无效`)
  return normalized
}

function boundedJson(value: unknown, label: string): string {
  const json = canonicalGameProductionJsonV2(value)
  if (json.length > 2_000_000) throw new Error(`[game-production-artifact] ${label} 超出 2MB 上限`)
  return json
}
 ...
 */
export async function acceptGameBuildArtifact(input: {
  ...
  inputHash: string
}): Promise<GameBuildArtifactRecordV1> {
  ...
  if (!isSha256Hash(input.inputHash)) throw new Error('[game-production-artifact] inputHash 无效')
  if (input.producerReceiptHash != null && !isSha256Hash(input.producerReceiptHash)) {
    throw new Error('[game-production-artifact] producerReceiptHash 无效')
  }
  ...
  const contentHash = input.contentHash ?? await hashGameProductionValueV2(JSON.parse(payloadJson))
  if (!isSha256Hash(contentHash)) throw new Error('[game-production-artifact] contentHash 无效')
  ...
  return db.transaction('rw', scopeTransactionTables(
    ...
    if (input.producerRunId != null) {
      ...
      try {
        const contract = run ? JSON.parse(run.contractJson) as {
          scope?: { gameProduction?: { controlEpoch?: unknown } }
        } : null
        runControlEpoch = contract?.scope?.gameProduction?.controlEpoch ?? null
      } catch { /* malformed contracts are rejected below */ }
      if (!run || run.gameBuildId !== build.id || run.projectId !== scope.projectId
        || run.workId !== scope.workId || runControlEpoch !== input.controlEpoch) {
        throw new Error('[game-production-artifact] producer Run 不属于当前 Build/epoch')
      }
    }
    if (input.blobObjectId != null) {
      ...
      if (!blob || !await assertRecordInScope(scope, 'mediaBlobObjects', blob, { owner: 'work' })
        ...
        throw new Error('[game-production-artifact] 媒资对象与 Artifact 不一致')
      }
    } else if (input.mimeType != null) {
      throw new Error('[game-production-artifact] 非媒资 Artifact 不能声明 mimeType')
    }
    const rows = await db.gameBuildArtifacts.where('buildId').equals(build.id!).toArray()
    ...
    const now = Date.now()
    const artifact = stampNewRecord(scope, 'gameBuildArtifacts', {
    ...
    } satisfies GameBuildArtifactRecordV1, { owner: 'work' })
    const id = await db.gameBuildArtifacts.add(artifact) as number
    if (priorAcceptedIds.length > 0) {
      await db.gameBuildArtifacts.where('id').anyOf(priorAcceptedIds).modify({ status: 'invalid', updatedAt: now })
    }
    return { ...artifact, id }
  })
}
 ...
 */
export async function carryForwardGameBuildArtifactsToEpochV1(input: {
  scope: WorkspaceScope
  buildId: number
  ...
  if (!Number.isInteger(input.fromControlEpoch) || !Number.isInteger(input.toControlEpoch)
    ...
    throw new Error('[game-production-artifact] carry-forward epoch 无效')
  }
  const keys = [...new Set(input.artifactKeys.map(value => stableKey(value, 'artifactKey')))]
  if (keys.length !== input.artifactKeys.length) throw new Error('[game-production-artifact] carry-forward keys 重复')
  return db.transaction('rw', scopeTransactionTables(
    ...
    if (!build || !await assertRecordInScope(scope, 'gameBuilds', build, { owner: 'work' })
      ...
      throw new Error('[game-production-artifact] carry-forward Build/epoch 不可写')
    }
    const allRows = await db.gameBuildArtifacts.where('buildId').equals(build.id!).toArray()
    ...
    if (new Set(sourceRows.map(row => row.artifactKey)).size !== sourceRows.length) {
      throw new Error('[game-production-artifact] carry-forward 来源 key 不唯一')
    }
    const carried: GameBuildArtifactRecordV1[] = []
    const now = Date.now()
    for (const source of sourceRows) {
      ...
      if (current) {
        if (current.contentHash !== source.contentHash) {
          throw new Error(`[game-production-artifact] carry-forward 当前 epoch 内容冲突:${source.artifactKey}`)
        }
      ...
      if (source.blobObjectId != null) {
        ...
        if (!blob || !await assertRecordInScope(scope, 'mediaBlobObjects', blob, { owner: 'work' })
          ...
          throw new Error(`[game-production-artifact] carry-forward 媒资对象损坏:${source.artifactKey}`)
        }
      }
      const version = Math.max(0, ...allRows.filter(row => row.artifactKey === source.artifactKey)
        .map(row => row.version)) + 1
      const next = stampNewRecord(scope, 'gameBuildArtifacts', {
        projectId: scope.projectId, worldId: scope.worldId, workId: scope.workId,
      ...
      const id = await db.gameBuildArtifacts.add(next) as number
      await db.gameBuildArtifacts.update(source.id!, { status: 'invalid', updatedAt: now })
 ...
 */
export async function carryForwardGameBuildArtifactsAcrossBuildsV1(input: {
  scope: WorkspaceScope
  ...
  if (input.sourceBuildId === input.targetBuildId || !Number.isInteger(input.targetControlEpoch)
    ...
    throw new Error('[game-production-artifact] cross-build carry-forward 参数无效')
  }
  const keys = [...new Set(input.artifactKeys.map(value => stableKey(value, 'artifactKey')))]
  if (!keys.length || keys.length !== input.artifactKeys.length) {
    throw new Error('[game-production-artifact] cross-build carry-forward keys 为空或重复')
  }
  return db.transaction('rw', scopeTransactionTables(
    db.gameBuilds, db.gameBuildArtifacts, db.mediaBlobObjects,
  ), async () => {
    const [sourceBuild, targetBuild] = await Promise.all([
      db.gameBuilds.get(input.sourceBuildId), db.gameBuilds.get(input.targetBuildId),
    ])
    if (!sourceBuild || !targetBuild
      || !await assertRecordInScope(scope, 'gameBuilds', sourceBuild, { owner: 'work' })
      || !await assertRecordInScope(scope, 'gameBuilds', targetBuild, { owner: 'work' })
      ...
      throw new Error('[game-production-artifact] cross-build 来源/目标关系不可复用')
    }
    const [sourceRows, targetRows] = await Promise.all([
      db.gameBuildArtifacts.where('buildId').equals(sourceBuild.id!).toArray(),
      db.gameBuildArtifacts.where('buildId').equals(targetBuild.id!).toArray(),
    ])
    ...
    if (sources.length !== keys.length || new Set(sources.map(row => row.artifactKey)).size !== keys.length) {
      throw new Error('[game-production-artifact] cross-build 来源 Artifact 不完整或不唯一')
    }
    const carried: GameBuildArtifactRecordV1[] = []
    const now = Date.now()
    for (const source of sources) {
      ...
      if (existing) {
        if (existing.contentHash !== source.contentHash || existing.parentArtifactHash !== source.contentHash) {
          throw new Error(`[game-production-artifact] cross-build 当前内容冲突:${source.artifactKey}`)
        }
      ...
      if (source.blobObjectId != null) {
        ...
        if (!blob || !await assertRecordInScope(scope, 'mediaBlobObjects', blob, { owner: 'work' })
          || blob.storageState !== 'ready' || blob.contentHash !== source.contentHash
          || blob.mimeType !== source.mimeType || blob.byteSize !== source.byteSize) {
          throw new Error(`[game-production-artifact] cross-build 媒资对象损坏:${source.artifactKey}`)
        }
      }
