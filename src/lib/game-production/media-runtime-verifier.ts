import type { GamePlaythroughBrowserEnvironmentV1, GameMediaRuntimeMeasurementV1 } from './quality-receipts'
import { analyzeDecodedGameAudioPcmV1, type GameAudioQualityAnalysisV1 } from './audio-quality-analyzer'

export interface GameMediaRuntimeAssetDescriptorV1 {
  assetKey: string
  contentHash: string
  mimeType: string
  width: number | null
...
export interface GameMediaRuntimeDecoderV1 {
  image(url: string, timeoutMs: number): Promise<{ width: number; height: number; hasAlpha: boolean }>
  audio(url: string, timeoutMs: number): Promise<GameAudioQualityAnalysisV1>
}
...
const browserDecoder: GameMediaRuntimeDecoderV1 = {
  image(url, timeoutMs) {
    return withTimeout({
  ...
  async audio(url, timeoutMs) {
    ...
    try {
      ...
      return await Promise.race([timeout, (async () => {
        ...
        return analyzeDecodedGameAudioPcmV1({ channels, sampleRateHz: decoded.sampleRate })
      })()])
    } catch (cause) {
 ...
 */
export async function verifyGameMediaRuntimeUrlsV1(input: {
  assets: GameMediaRuntimeAssetDescriptorV1[]
  urls: Record<string, string>
  environment: GamePlaythroughBrowserEnvironmentV1
  measuredAt?: number
  timeoutMs?: number
  decoder?: GameMediaRuntimeDecoderV1
}): Promise<GameMediaRuntimeMeasurementV1> {
