import type { GameBuildCompatibilityReportV1, GameRuntimePackageV2 } from '../types'
...
function fail(message: string): never {
  throw new Error(`[game-production-compatibility] ${message}`)
}

function stableValues(pkg: GameRuntimePackageV2): Map<string, string> {
  ...
  const add = (prefix: string, key: string, value: unknown) => {
    ...
    values.set(stableKey, canonicalGameProductionJsonV2(value))
  }
  ...
  for (const resource of pkg.simulation?.resources ?? []) add('simulation.resource', resource.key, resource)
  for (const metric of pkg.simulation?.metrics ?? []) add('simulation.metric', metric.key, metric)
  for (const actor of pkg.simulation?.actors ?? []) add('simulation.actor', actor.key, actor)
  for (const issue of pkg.simulation?.issues ?? []) add('simulation.issue', issue.key, issue)
  for (const action of pkg.simulation?.actions ?? []) add('simulation.action', action.key, action)
  for (const ending of pkg.simulation?.endings ?? []) add('simulation.ending', ending.key, ending)
...
export async function createGameBuildCompatibilityReportV1(input: {
  previous: RuntimePackageBuildRefV1 | null
  current: RuntimePackageBuildRefV1
}): Promise<GameBuildCompatibilityReportV1> {
  validateRef(input.current, 'current')
  if (input.previous) validateRef(input.previous, 'previous')
  if (!input.previous) {
    const core = {
      schema: 'storyforge.game-build-compatibility' as const, version: 1 as const,
    ...
    return { ...core, reportHash: await hashGameProductionValueV2(core) }
  }
  ...
  const reasons = breaking
    ...
    : exactPackage
      ...
      : addedStableKeys.length
        ...
        : ['变化只涉及不进入存档语义的表现内容。']
  const core = {
    schema: 'storyforge.game-build-compatibility' as const, version: 1 as const, level,
