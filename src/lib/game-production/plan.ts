...
function fail(message: string): never { throw new Error(`[game-production-plan] ${message}`) }
...
function parseBudget(value: unknown, label: string): GameTaskBudgetReservationV1 {
...
function parseTask(value: unknown, index: number): GameProductionPlanTaskV3 {
  ...
  let reuse: GameProductionPlanTaskV3['reuse'] = null
  if (row.reuse != null) {
...
export function parseGameProductionPlanV3(
  value: string | unknown,
  briefInput?: GameProductionBriefV3 | string,
  ...
  if (row.schema !== 'storyforge.game-production-plan' || row.version !== 3 || !isSha256Hash(row.briefHash)) {
  ...
  const brief = briefInput ? parseGameProductionBriefV3(briefInput) : null
  ...
  return {
    schema: 'storyforge.game-production-plan', version: 3,
...
const ZERO_BUDGET: GameTaskBudgetReservationV1 = {
  modelCalls: 0, inputTokens: 0, outputTokens: 0, mediaCalls: 0,
  maximumCostUsd: 0, durationMs: 5_000, storageBytes: 1_000_000,
}
...
export function createVerticalSliceGameProductionPlanV3(input: {
  brief: GameProductionBriefV3
  briefHash: string
  controlEpoch: number
  buildNumber: number
}): GameProductionPlanV3 {
  const brief = parseGameProductionBriefV3(input.brief)
  if (brief.intent.productType !== 'storygame' && brief.intent.productType !== 'avg') {
    fail('1C 纵切仅接受 storygame 或 avg')
  }
  const tasks = [
  ...
  return parseGameProductionPlanV3({
    schema: 'storyforge.game-production-plan', version: 3, buildNumber: input.buildNumber,
    productType: brief.intent.productType, briefHash: input.briefHash, controlEpoch: input.controlEpoch,
    concurrency: {
      maximumCostBearingTasks: 1, maximumTextProviderTasks: 1, maximumMediaProviderTasks: 1,
    },
    tasks, terminalTaskKey: 'quality.verify',
  }, brief, input.briefHash)
}

function reservation(input: Partial<GameTaskBudgetReservationV1>): GameTaskBudgetReservationV1 {
  return {
    modelCalls: 0, inputTokens: 0, outputTokens: 0, mediaCalls: 0,
...
function productionTask(
  value: Omit<GameProductionPlanTaskV3, 'requiredReceipts' | 'reuse'>,
): GameProductionPlanTaskV3 {
  return {
    ...value,
 ...
 */
export async function createGameProductionPlanV3(input: {
  buildNumber: number
  controlEpoch?: number
  briefHash: string
  brief: GameProductionBriefV3 | string
}): Promise<GameProductionPlanV3> {
  const brief = parseGameProductionBriefV3(input.brief)
  ...
  if (await hashGameProductionValueV2(brief) !== input.briefHash) fail('briefHash 与 Brief 内容不一致')
  const controlEpoch = input.controlEpoch ?? 0
  if (!Number.isInteger(controlEpoch) || controlEpoch < 0) fail('controlEpoch 无效')

  const capabilityKeys = (classes: Array<GameProductionBriefV3['capabilityRequirements'][number]['mediaClass']>) => (
  ...
  const tasks: GameProductionPlanTaskV3[] = [
    productionTask({
      taskKey: 'content.design', lane: 'content', kind: 'game-design',
      skillId: 'game-production.content.v1', executionMode: 'model', dependsOn: [],
    ...
    productionTask({
      ...
      skillId: 'game-production.content.v1', executionMode: 'model', dependsOn: ['content.design'],
    ...
    productionTask({
      ...
      skillId: 'game-production.media-requirements.v1', executionMode: 'model', dependsOn: ['content.design'],
  ...
  if (activeVisual) {
    ...
    tasks.push(productionTask({
      ...
      skillId: 'game-production.media-request.v1', executionMode: 'media-provider',
