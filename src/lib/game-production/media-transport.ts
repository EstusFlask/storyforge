...
import type { GameProductionCapabilityBindingV1 } from './scheduler'
...
export type GameMediaCapabilityBindingReceiptV1 =
  | MediaRelayBindingReceiptV1
  | ConfiguredAgnesImageBindingReceiptV1

export interface ResolvedGameMediaCapabilityV1 {
  adapter: GameMediaProviderAdapterV1
  transport: MediaProviderTransportV1
  binding: GameProductionCapabilityBindingV1
  receipt: GameMediaCapabilityBindingReceiptV1
}
...
function fail(message: string): never {
  throw new Error(`[game-media-relay] ${message}`)
}
...
function configuredAgnesConfig(projectId: number, config?: AIConfig): AIConfig {
  return config ?? resolveRequestConfig(useAIConfigStore.getState().config, {
    category: 'game-production', projectId,
  }).config
}
...
export async function resolveConfiguredAgnesImageCapabilityV1(input: {
  ...
  const capabilityHash = await hashGameProductionValueV2(identity)
  const body = {
