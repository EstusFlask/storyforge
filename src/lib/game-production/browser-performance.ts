...
export const GAME_BROWSER_PERFORMANCE_POLICY_V1 = {
  policyId: 'storyforge.game-browser-performance.v1',
...
export interface GameBrowserPerformanceMeasurementV1 {
...
export interface GameBrowserPerformanceReceiptV1 {
  schema: 'storyforge.game-browser-performance-receipt'
  version: 1
  policyId: typeof GAME_BROWSER_PERFORMANCE_POLICY_V1.policyId
  packageHash: string
  previewHash: string
  environment: Pick<GameBrowserPerformanceMeasurementV1, 'browserName' | 'browserVersion' | 'platform' | 'viewport'>
  metrics: {
    firstInteractiveBytes: number
...
export async function createGameBrowserPerformanceReceiptV1(
  measurement: GameBrowserPerformanceMeasurementV1,
): Promise<GameBrowserPerformanceReceiptV1> {
  if (!validHash(measurement.packageHash) || !validHash(measurement.previewHash)) {
    throw new Error('[game-browser-performance] packageHash/previewHash 无效')
  }
  if (measurement.runtimeVerifier != null
    && !['playwright-cdp', 'in-app-browser-lab'].includes(measurement.runtimeVerifier)) {
    throw new Error('[game-browser-performance] runtimeVerifier 无效')
  }
  if (!measurement.browserName.trim() || !measurement.browserVersion.trim() || !measurement.platform.trim()
    ...
    || measurement.cachedSceneLatenciesMs.length > GAME_BROWSER_PERFORMANCE_POLICY_V1.maximumLatencySamples
    || measurement.choiceInputLatenciesMs.length > GAME_BROWSER_PERFORMANCE_POLICY_V1.maximumLatencySamples
    || measurement.memorySamples.length > GAME_BROWSER_PERFORMANCE_POLICY_V1.maximumMemorySamples
    ...
    throw new Error('[game-browser-performance] measurement 无效')
  }
  ...
  const warmup = memorySamples.find(sample => sample.elapsedMs >= GAME_BROWSER_PERFORMANCE_POLICY_V1.warmupDurationMs)
  const final = longRunDurationMs >= GAME_BROWSER_PERFORMANCE_POLICY_V1.minimumLongRunDurationMs
  ...
  if (measurement.firstInteractiveBytes > GAME_BROWSER_PERFORMANCE_POLICY_V1.maximumFirstInteractiveBytes) failures.push('first-interactive-bytes')
  if (measurement.cachedSceneLatenciesMs.length < GAME_BROWSER_PERFORMANCE_POLICY_V1.minimumLatencySamples) failures.push('scene-sample-count')
  else if (cachedSceneP95Ms == null || cachedSceneP95Ms > GAME_BROWSER_PERFORMANCE_POLICY_V1.maximumCachedSceneP95Ms) failures.push('cached-scene-p95')
  if (measurement.choiceInputLatenciesMs.length < GAME_BROWSER_PERFORMANCE_POLICY_V1.minimumLatencySamples) failures.push('input-sample-count')
  else if (choiceInputP95Ms == null || choiceInputP95Ms > GAME_BROWSER_PERFORMANCE_POLICY_V1.maximumChoiceInputP95Ms) failures.push('choice-input-p95')
  if (peakUsedHeapBytes == null) failures.push('heap-measurement-missing')
  else if (peakUsedHeapBytes > GAME_BROWSER_PERFORMANCE_POLICY_V1.maximumDesktopHeapBytes) failures.push('desktop-heap-peak')
  if (longRunDurationMs < GAME_BROWSER_PERFORMANCE_POLICY_V1.minimumLongRunDurationMs || longRunGrowthRatio == null) failures.push('long-run-incomplete')
  else if (longRunGrowthRatio > GAME_BROWSER_PERFORMANCE_POLICY_V1.maximumLongRunGrowthRatio) failures.push('long-run-memory-growth')
  const body = {
    schema: 'storyforge.game-browser-performance-receipt' as const,
    version: 1 as const,
    policyId: GAME_BROWSER_PERFORMANCE_POLICY_V1.policyId,
  ...
  return { ...body, receiptHash: await hashGameProductionValueV2(body) }
}
