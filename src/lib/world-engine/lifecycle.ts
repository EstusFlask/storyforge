...
export async function cascadeRegisteredReferences(
  ...
  if (!source) throw new Error(`[world-lifecycle] 未登记表 ${sourceTableName}`)
  for (const ref of source.refs ?? []) {
    if (ref.kind !== 'simple' || ref.onDelete === 'keep') continue
    const target = parseSimpleTarget(ref.target)
    if (!target) throw new Error(`[world-lifecycle] 无效引用 ${sourceTableName} -> ${ref.target}`)
    const targetSpec = REGISTRY_BY_NAME.get(target.tableName)
    if (!targetSpec) throw new Error(`[world-lifecycle] 引用目标未登记 ${target.tableName}`)
...
export async function deleteWork(workId: number): Promise<void> {
  ...
  await db.transaction('rw', transactionTablesFor('deleteProject'), async () => {
    ...
    if (project?.activeWorkId === workId) {
      ...
      if (!world) throw new Error('[world-lifecycle] Work 的 World 根在删除过程中丢失')
      await db.projects.update(work.projectId, {
        ...(nextWork ? projectCompatibilityMirror(world, nextWork) : projectCompatibilityWithoutWork(world)),
        updatedAt: Date.now(),
      })
    }
  })
...
export async function deleteWorld(worldId: number, options: { confirm: boolean } = { confirm: false }): Promise<void> {
  ...
  if (works.length && !options.confirm) {
    throw new Error('[world-lifecycle] 删除 World 前必须显式确认其 Works 将一并删除')
  }
  ...
  await db.transaction('rw', transactionTablesFor('deleteProject'), async () => {
    ...
    if (project?.activeWorldId === worldId) {
      ...
      await db.projects.update(world.projectId, {
        ...(nextWorld
          ? nextWork
            ? projectCompatibilityMirror(nextWorld, nextWork)
            : projectCompatibilityWithoutWork(nextWorld)
          : projectCompatibilityWithoutWorld()),
        updatedAt: Date.now(),
      })
    }
