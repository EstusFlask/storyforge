...
export const ADOPTION_EXTENSIONS: readonly AdoptionExtensionSpec[] = Object.freeze([
  ...
  },
  {
    ...
    entrypoints: ['src/lib/media/blob-store.ts', 'src/lib/comic/media-service.ts'],
    policyRegistry: 'PROJECT_TABLES portable binary + SHA-256/MIME/dimension checks + two-phase reference-counted GC',
    reason: '共享二进制对象必须 Work 隔离、hash 可验、备份可恢复；删除经 pending receipt 和引用复查，避免孤儿或误删。',
    reviewAfter: '2027-08-01',
  },
  {
