...
const ADOPTION_SCHEMAS_RAW: CollectionAdoptionSpec[] = [
  ...
  {
    target: 'ttrpgCampaignModules',
    identity: { kind: 'composite', fields: ['campaignKey'] },
    duplicatePolicy: 'update',
    required: ['campaignKey', 'title', 'status', 'sourceWorldReleaseId', 'rulePackId', 'contentJson', 'contentHash'],
    autoStamps: ['projectId', 'worldId', 'workId', 'createdAt', 'updatedAt'],
    ownerFrom: 'work',
    fkChecks: [
      { field: 'sourceWorldReleaseId', target: 'worldReleases' },
      { field: 'rulePackId', target: 'gameRulePacks' },
    ],
  },
]
...
export const ADOPTION_EXTENSIONS: readonly AdoptionExtensionSpec[] = Object.freeze([
  {
    id: 'ttrpg-product-authoring',
    target: 'gameRulePacks',
    entrypoints: ['src/lib/ttrpg/authoring.ts'],
    policyRegistry: 'TTRPG_RULE_PACK_V1 + TTRPG_CAMPAIGN_V1 + PROJECT_TABLES',
    reason: 'RulePack 必须在写入前通过闭集 DSL、许可、引用、fixture 和内容 hash 验证；不能把 raw JSON 从组件直接散写。',
    reviewAfter: '2027-08-21',
  },
  {
    id: 'ttrpg-campaign-authoring',
    target: 'ttrpgCampaignModules',
    entrypoints: ['src/lib/ttrpg/authoring.ts'],
    policyRegistry: 'TTRPG_RULE_PACK_V1 + TTRPG_CAMPAIGN_V1 + PROJECT_TABLES',
    reason: 'CampaignPack 必须校验冻结世界来源、规则引用、线索冗余、角色数值映射和内容 hash，发布前不得绕过预检。',
    reviewAfter: '2027-08-21',
  },
  {
  ...
  ...([
    ...
    policyRegistry: 'WORLD_GAME_AUTHORING_POLICY + GAME_PRODUCTION_PACKAGE_ADOPTION_V1 + PROJECT_TABLES + product validators',
    reason: '主 Agent 只产出严格便携剧情候选；作者确认或 Production publish 后由登记的整包落库器原子校验来源、图结构和产品模块，不能拆成通用字段 adopt。',
    reviewAfter: '2027-02-15',
  })),
  {
    id: 'character-interaction-production-roots', target: 'characterInteractionProductions',
    entrypoints: ['src/lib/character-interaction/production.ts', 'src/lib/character-interaction/production-pipeline.ts'],
    policyRegistry: 'CHARACTER_INTERACTION_PRODUCTION_CONTRACT_V1 + PROJECT_TABLES',
    reason: '角色互动生产根只允许产品专属来源、Brief、状态与发布 CAS 边界推进。', reviewAfter: '2027-08-25',
  },
  {
    id: 'character-interaction-production-steps', target: 'characterInteractionProductionSteps',
    entrypoints: ['src/lib/character-interaction/production-pipeline.ts', 'src/lib/character-interaction/production-harness.ts'],
    policyRegistry: 'CHARACTER_INTERACTION_PRODUCTION_CONTRACT_V1 + PROJECT_TABLES',
    reason: '步骤尝试必须校验冻结输入、前置 hash、恢复状态和人工确认。', reviewAfter: '2027-08-25',
  },
  {
    id: 'character-interaction-production-artifacts', target: 'characterInteractionArtifacts',
    entrypoints: ['src/lib/character-interaction/production-pipeline.ts', 'src/lib/character-interaction/production-harness.ts'],
    policyRegistry: 'CHARACTER_INTERACTION_PRODUCTION_CONTRACT_V1 + PROJECT_TABLES',
    reason: '候选、确认、世界升级和运行反馈产物只允许产品 parser/hash/人工确认边界写入。', reviewAfter: '2027-08-25',
  },
  {
    id: 'character-interaction-production-media', target: 'characterInteractionMediaAssets',
    entrypoints: ['src/lib/character-interaction/production-pipeline.ts'],
    policyRegistry: 'CHARACTER_INTERACTION_MEDIA_CONTRACT_V1 + PROJECT_TABLES',
    reason: '角色互动媒资必须经过规格、权利、内容寻址完整性或显式降级校验。', reviewAfter: '2027-08-25',
  },
  {
    id: 'character-interaction-product-releases', target: 'characterInteractionProductReleases',
    entrypoints: ['src/lib/character-interaction/production-pipeline.ts'],
    policyRegistry: 'CHARACTER_INTERACTION_PRODUCT_RELEASE_V1 + PROJECT_TABLES',
    reason: 'Product Release 只能原子冻结来源、Brief、确认产物、媒资和共享 GameRelease。', reviewAfter: '2027-08-25',
  },
  {
    id: 'game-production-roots', target: 'gameProductions',
    entrypoints: [
      'src/lib/game-production/commands.ts',
      'src/lib/game-production/artifact-store.ts',
      'src/lib/game-production/media-blob-store.ts',
      'src/lib/game-production/vertical-slice.ts',
      'src/lib/game-production/adoption.ts',
      'src/lib/game-production/scheduler.ts',
    ],
    policyRegistry: 'GAME_PRODUCTION_COMMAND_POLICY + GAME_PRODUCTION_ARTIFACT_POLICY + PROJECT_TABLES',
    reason: 'Production 状态、Brief 授权、幂等命令、Build/Artifact 和共享媒资必须维持 CAS、hash、scope、epoch 与引用完整性，不能由组件或 provider 直接散写。',
    reviewAfter: '2027-08-21',
  },
  {
    id: 'game-production-briefs', target: 'gameProductionBriefs',
    entrypoints: ['src/lib/game-production/commands.ts', 'src/lib/game-production/adoption.ts'],
    policyRegistry: 'GAME_PRODUCTION_COMMAND_POLICY + PROJECT_TABLES',
    reason: 'Brief revision 只能由命令 CAS 保存和授权，正式采用事务只能读取冻结 revision。',
    reviewAfter: '2027-08-21',
  },
  {
    id: 'game-production-commands', target: 'gameProductionCommands',
    entrypoints: ['src/lib/game-production/commands.ts', 'src/lib/game-production/adoption.ts'],
    policyRegistry: 'GAME_PRODUCTION_COMMAND_POLICY + PROJECT_TABLES',
  ...
  {
    id: 'game-production-builds', target: 'gameBuilds',
    entrypoints: [
      'src/lib/game-production/commands.ts',
      'src/lib/game-production/artifact-store.ts',
      'src/lib/game-production/vertical-slice.ts',
      'src/lib/game-production/adoption.ts',
      'src/lib/game-production/scheduler.ts',
      'src/lib/game-production/quality-receipts.ts',
    ],
    policyRegistry: 'GAME_PRODUCTION_COMMAND_POLICY + GAME_PRODUCTION_ARTIFACT_POLICY + GAME_QUALITY_GATE_RECEIPT_V1 + PROJECT_TABLES',
    reason: 'Build 状态、epoch、hash 与发布指针必须通过登记的控制、Artifact 或整包采用事务变更；商业可发布状态只能由不可变性能与作者主路线回执的最新投影推进或降级。',
    reviewAfter: '2027-08-21',
  },
  {
    id: 'game-production-artifacts', target: 'gameBuildArtifacts',
    entrypoints: [
      'src/lib/game-production/commands.ts',
      'src/lib/game-production/artifact-store.ts',
      'src/lib/game-production/adoption.ts',
    ...
    policyRegistry: 'GAME_PRODUCTION_ARTIFACT_POLICY + PROJECT_TABLES',
    reason: '候选、验收、沿用与 orphan 状态必须校验 Build epoch、receipt、hash、scope 和唯一版本。',
    reviewAfter: '2027-08-21',
  },
  {
    id: 'game-production-media-objects', target: 'mediaBlobObjects',
    entrypoints: ['src/lib/game-production/media-blob-store.ts', 'src/lib/game-production/artifact-store.ts'],
    policyRegistry: 'GAME_PRODUCTION_MEDIA_OBJECT_POLICY + PROJECT_TABLES',
    reason: '共享二进制只允许内容寻址存储边界写入、校验、租约与引用感知回收。',
    reviewAfter: '2027-08-21',
  },
  {
    id: 'game-production-release-adoption', target: 'gameReleases',
    entrypoints: [
    ...
    policyRegistry: 'GAME_PRODUCTION_PACKAGE_ADOPTION_V1 + GAME_DISTRIBUTION_BUNDLE_V1 + PROJECT_TABLES + product validators',
    reason: 'Production publish 只允许在单一事务中物化经 QA 的完整运行包；市场导入必须逐字验证分发包、媒资、世界来源、授权与来源回执，禁止分表发布或绕过产品 parser。',
    reviewAfter: '2027-08-21',
  },
  {
    id: 'game-production-textsim-adoption', target: 'narrativeSimulationModules',
    entrypoints: [
      'src/lib/narrative-simulation/authoring.ts',
      'src/lib/open-world/authoring.ts',
      'src/lib/game-production/adoption.ts',
    ],
    policyRegistry: 'GAME_PRODUCTION_PACKAGE_ADOPTION_V1 + PROJECT_TABLES + product validators',
    reason: 'Production publish 只允许在单一事务中物化经 QA 的完整运行包；禁止分表发布或绕过产品 parser。',
    reviewAfter: '2027-08-21',
  },
  {
    id: 'game-production-openworld-adoption', target: 'openWorldModules',
    entrypoints: ['src/lib/open-world/authoring.ts', 'src/lib/game-production/adoption.ts'],
    policyRegistry: 'GAME_PRODUCTION_PACKAGE_ADOPTION_V1 + PROJECT_TABLES + product validators',
    reason: 'Production publish 只允许在单一事务中物化经 QA 的完整运行包；禁止分表发布或绕过产品 parser。',
    reviewAfter: '2027-08-21',
  },
